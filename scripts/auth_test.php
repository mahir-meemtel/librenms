...
if (isset($options['v'])) {
    ...
    ObzoraConfig::set('auth_ad_debug', 1);
    ObzoraConfig::set('auth_ldap_debug', 1);
}

echo 'Authentication Method: ' . ObzoraConfig::get('auth_mechanism') . PHP_EOL;

// if ldap like, check selinux
if (ObzoraConfig::get('auth_mechanism') == 'ldap' || ObzoraConfig::get('auth_mechanism') == 'active_directory') {
...
try {
    ...
    // AD bind tests
    if ($authorizer instanceof \ObzoraNMS\Authentication\ActiveDirectoryAuthorizer) {
        ...
        if (ObzoraConfig::has('auth_ad_binduser') && ObzoraConfig::has('auth_ad_bindpassword')) {
            ...
            if (! $bind_success) {
                ...
                if ($ldap_error == 'Invalid credentials') {
                    print_error('AD bind failed for user ' . ObzoraConfig::get('auth_ad_binduser') . '@' . ObzoraConfig::get('auth_ad_domain') .
