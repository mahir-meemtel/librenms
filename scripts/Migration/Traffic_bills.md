...
This Document describes how to migrate the **traffic bills** from Observium to obzora

### Assumptions

* The obzora installation is complete and migration has taken place except for the traffic bills and traffic bill history.
* The old DB is called ``observium`` and new DB is called ``obzora``. If both DBs are not on the same DB Server, create a DB called ``observium`` on the target DB-Server run mysqldump & co to copy the data.
*  **No traffic bills** have been created in obzora.
*  The scripts have been tested on obzora version Version 22.1.0 and DB Schema "2021_11_29_165436_improve_ports_search_index (229)"

### Precaution
backup your databases first:
*  ``mysqldump observium > observium.sql``
*  ``mysqldump obzora > obzora.sql``

### Warning: traffic bills will be deleted from obzora and imported from observium!
...
LOCK TABLES obzora.bill_history obzora.bills obzora.bill_data WRITE, observium.bill_data WRITE, observium.bill_history READ, observium.bills READ ;

--
-- The columns bill_polled, bill_contact, bill_threshold and bill_notify are not present in the observium data model
--
TRUNCATE TABLE obzora.bills;
INSERT INTO obzora.bills
...
--
TRUNCATE TABLE obzora.bill_history;
INSERT INTO obzora.bill_history
...
TRUNCATE TABLE obzora.bill_data;
INSERT INTO obzora.bill_data
    ...
    ON DUPLICATE KEY UPDATE
        obzora.bill_data.delta=
            IF ( obzora.bill_data.delta >= VALUES(obzora.bill_data.delta), obzora.bill_data.delta, VALUES(obzora.bill_data.delta) ),
        obzora.bill_data.in_delta=
            IF ( obzora.bill_data.delta >= VALUES(obzora.bill_data.delta), obzora.bill_data.in_delta, VALUES(obzora.bill_data.in_delta) ),
        obzora.bill_data.out_delta=
            IF ( obzora.bill_data.delta >= VALUES(obzora.bill_data.delta), obzora.bill_data.out_delta, VALUES(obzora.bill_data.out_delta) ),
        obzora.bill_data.period=
            IF ( obzora.bill_data.delta >= VALUES(obzora.bill_data.delta), obzora.bill_data.period, VALUES(obzora.bill_data.period) )
    ;
COMMIT;
SELECT COUNT(bill_id) FROM obzora.bill_data;
UNLOCK TABLES;
...
LOCK TABLES obzora.bill_data WRITE, observium.bill_data WRITE ;
SELECT COUNT(bill_id) FROM observium.bill_data;
TRUNCATE TABLE obzora.bill_data;
