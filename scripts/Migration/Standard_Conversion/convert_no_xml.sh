...
####################### SCRIPT DESCRIPTION ########################
# First we SSH to ObzoraNMS and create necessary directories with  #
# the mkdir script. Then the script enters each Observium RRD dir #
# and SCPs the RRD files to the correct directory on ObzoraNMS.    #
# After that we add all of the devices to ObzoraNMS using the      #
# destwork script.                                                #
###################################################################
...
# Enter ObzoraNMS IP or hostname here
DEST=10.0.253.35
# Enter ObzoraNMS RRD path here
...
# This line SSHs to ObzoraNMS server and makes directories based on node list text file
...
while read line; 
	...
	do cd $O_RRDPATH"${line%/*}" 
		# Transfer RRD files to ObzoraNMS Server
		scp *.rrd root@$DEST:$L_RRDPATH"${line%/*}"/ 
		# Exit to parent dir
		cd ..
	done < $NODELIST

# This line SSHs to ObzoraNMS server and runs the destwork script to finish conversion
