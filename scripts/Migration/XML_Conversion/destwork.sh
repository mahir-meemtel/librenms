...
# files for use with ObzoraNMS. It then adds the device using the  #
# Addhost function of ObzoraNMS                                    #
###################################################################
...
# Enter user and group of ObzoraNMS installation
L_USRGRP=obzora
...
while read -r line
	...
	do cd $L_RRDPATH"${line%/*}" || return 1
		...
		rm ./*.xml;
		# Change ownership to ObzoraNMS user and group
		chown -R $L_USRGRP:$L_USRGRP .;
		# Add the host to ObzoraNMS
