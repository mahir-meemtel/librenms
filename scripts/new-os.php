...
if ($options['h'] && $options['o'] && $options['t'] && $options['v']) {
    ...
    $test_file = ObzoraConfig::get('install_dir') . "/tests/snmpsim/{$options['o']}.snmprec";
...
");
        ...
        if (! Str::contains($continue, 'y', ignoreCase: true)) {
            ...
            if (filter_var($icon, FILTER_VALIDATE_URL)) {
                ...
                file_put_contents(ObzoraConfig::get('temp_dir') . "/{$options['o']}", $icon_data);
                $file_info = mime_content_type(ObzoraConfig::get('temp_dir') . "/{$options['o']}");
                if ($file_info === 'image/png') {
                    $ext = '.png';
                } elseif ($file_info === 'image/svg+xml') {
                    $ext = '.svg';
                }
                rename(ObzoraConfig::get('temp_dir') . "/{$options['o']}", ObzoraConfig::get('install_dir') . "/html/images/os/$vendor$ext");
                $icon = $vendor;
            }
...
";
    ...
    if (filter_var($mib_name, FILTER_VALIDATE_URL)) {
        ...
        file_put_contents(ObzoraConfig::get('temp_dir') . "/{$options['o']}.mib", $mib_data);
        $file_info = mime_content_type(ObzoraConfig::get('temp_dir') . "/{$options['o']}.mib");
        ...
        if (file_exists(ObzoraConfig::get('install_dir') . "/mibs/$vendor/") == false) {
            mkdir(ObzoraConfig::get('install_dir') . "/mibs/$vendor/");
        }
        rename(ObzoraConfig::get('temp_dir') . "/{$options['o']}.mib", ObzoraConfig::get('install_dir') . "/mibs/$vendor/$mib_name");
    } elseif ($mib_name) {
        $tmp_mib = explode('/', $mib_name);
        $mib_name = array_pop($tmp_mib);
    }

    $translate_cmd = ObzoraConfig::get('snmptranslate') . ' -M ' . ObzoraConfig::get('mib_dir') . ':' . ObzoraConfig::get('mib_dir') . "/$vendor -m $mib_name -TB '.*Table$' -Os";
    ...
    foreach (explode(PHP_EOL, $tables) as $table_name) {
        if ($table_name) {
            ...
            if ($continue === 'y' || $continue === 'Y') {
                $mib2c_cmd = 'env MIBDIRS=' . ObzoraConfig::get('mib_dir') . ':' . ObzoraConfig::get('mib_dir') . "/$vendor/ env MIBS=\"$mib_name\" mib2c -q -c misc/mib2c.conf $table_name";
...
Usage:
    -h Is the device ID or hostname of the device in ObzoraNMS detected as generic
