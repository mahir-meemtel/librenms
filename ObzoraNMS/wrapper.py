...
"""
 ...
 Authors:       Orsiris de Jong <contact@netpower.fr>
                Neil Lathwood <neil@obzora.meemtel.com>
...
import ObzoraNMS
from ObzoraNMS.command_runner import command_runner
...
def wrapper(
    ...
    **kwargs,  # Type: dict, may contain modules
    ...
    db_connection = ObzoraNMS.DB(sconfig)
...
if __name__ == "__main__":
    parser = ArgumentParser(
        ...
        description="Spawn multiple obzora php processes in parallel.",
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    config = ObzoraNMS.get_config_data(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    )
    log_dir = config["log_dir"]
    log_file = os.path.join(log_dir, wrapper_type + ".log")
    logger = ObzoraNMS.logger_get_logger(log_file, debug=debug)
