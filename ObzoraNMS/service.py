...
import ObzoraNMS
from ObzoraNMS.config import DBConfig
...
class ServiceConfig(DBConfig):
    def __init__(self):
        """
        Stores all of the configuration variables for the ObzoraNMS service in a common object
    ...
    watchdog_logfile = "logs/obzora.log"
    health_file = ""  # disabled by default

    def populate(self):
        config = ObzoraNMS.get_config_data(self.BASE_DIR)
        ...
        self.watchdog_logfile = config.get("log_file", self.logdir + "/obzora.log")
...
class Service:
    ...
    def __init__(self):
        ...
        self._db = ObzoraNMS.DB(self.config)
        ...
        self.daily_timer = ObzoraNMS.RecurringTimer(
            self.config.update_frequency, self.run_maintenance, "maintenance"
        )
        self.stats_timer = ObzoraNMS.RecurringTimer(
        ...
        if self.config.watchdog_enabled:
            ...
            self.watchdog_timer = ObzoraNMS.RecurringTimer(
        ...
        self.systemd_watchdog_timer = ObzoraNMS.RecurringTimer(
    ...
    def start(self):
        ...
        self.poller_manager = ObzoraNMS.PollerQueueManager(self.config, self._lm)
        self.queue_managers["poller"] = self.poller_manager
        self.discovery_manager = ObzoraNMS.DiscoveryQueueManager(self.config, self._lm)
        self.queue_managers["discovery"] = self.discovery_manager
        self.queue_managers["alerting"] = ObzoraNMS.AlertQueueManager(
            self.config, self._lm
        )
        self.queue_managers["services"] = ObzoraNMS.ServicesQueueManager(
            self.config, self._lm
        )
        self.queue_managers["billing"] = ObzoraNMS.BillingQueueManager(
            self.config, self._lm
        )
        self.queue_managers["ping"] = ObzoraNMS.PingQueueManager(self.config, self._lm)
        ...
        logger.info("ObzoraNMS Service: {} started!".format(self.config.unique_name))
        logger.info(
            "Poller group {}. Using Python {} and {} locks and queues".format(
                "0 (default)" if self.config.group == [0] else self.config.group,
                python_version(),
                "redis" if isinstance(self._lm, ObzoraNMS.RedisLock) else "internal",
            )
        )
        logger.info(
    ...
    def run_maintenance(self):
        ...
        exit_code, output = ObzoraNMS.call_script("daily.sh")
        if exit_code == 0:
    ...
    def create_lock_manager(self):
        ...
        return ObzoraNMS.ThreadingLock()
