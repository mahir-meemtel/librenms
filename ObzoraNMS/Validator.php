...
class Validator
{
    ...
    public function __construct()
    {
        ...
        $pattern = $this->getBaseDir() . '/ObzoraNMS/Validations/*.php';

        foreach (glob($pattern) as $file) {
            $class_name = basename($file, '.php');
            $class = '\ObzoraNMS\Validations\\' . $class_name;
    ...
    public function result(ValidationResult $result, ?string $group = null): void
    {
        ...
        if (empty($group)) {
            ...
            $bt = debug_backtrace();
            foreach ($bt as $entry) {
                if (Str::startsWith($entry['class'], 'ObzoraNMS\Validations')) {
                    $group = str_replace('ObzoraNMS\Validations\\', '', $entry['class']);
                    break;
                }
            }
        }
    ...
    /**
     * Execute a command, but don't run it as root.  If we are root, run as the ObzoraNMS user.
    ...
    public function execAsUser(string $command, ?array &$output = null, ?int &$code = null): void
    {
        if (self::getUsername() === 'root') {
            $command = 'su ' . \config('obzora.user') . ' -s /bin/sh -c "' . $command . '"';
        }
    ...
    /**
     * Get the base url for this ObzoraNMS install, this will only work from web pages.
    ...
    {
        $url = function_exists('get_url') ? get_url() : ObzoraConfig::get('base_url');
