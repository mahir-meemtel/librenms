...
class QueueManager:
    def __init__(
        ...
        :param config: ObzoraNMS.ServiceConfig reference to the service config object
        :param lock_manager: A ObzoraNMS.Lock instance to help with locks
        ...
        self.config = config
        self.performance = ObzoraNMS.PerformanceCounter()
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if (
                    ...
                    with ObzoraNMS.TimeitContext.start() as t:
    ...
    def get_poller_config(self):
        """
        Returns the ObzoraNMS.PollerConfig for this QueueManager
        :return: ObzoraNMS.PollerConfig
        """
    ...
    def _create_queue(self, queue_type, group):
        ...
        try:
            return ObzoraNMS.RedisUniqueQueue(
...
class TimedQueueManager(QueueManager):
    def __init__(
        ...
        self.timer = ObzoraNMS.RecurringTimer(
...
class BillingQueueManager(TimedQueueManager):
    def __init__(self, config, lock_manager):
        ...
        self.calculate_timer = ObzoraNMS.RecurringTimer(
    ...
    def do_work(self, run_type, group):
        if run_type == "poll":
            ...
            exit_code, output = ObzoraNMS.call_script("poll-billing.php", args)
        else:  # run_type == 'calculate'
            logger.info("Calculating billing")
            args = ("-d", "-f") if self.config.debug else ("-f")
            exit_code, output = ObzoraNMS.call_script("billing-calculate.php", args)
    ...
    def do_work(self, context, group):
        if self.lock(group, "group", timeout=self.config.ping.frequency):
            try:
                ...
                exit_code, output = ObzoraNMS.call_script("ping.php", args)
...
class ServicesQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(device_id, timeout=self.config.services.frequency):
            ...
            exit_code, output = ObzoraNMS.call_script("check-services.php", args)
...
class AlertQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        ...
        args = ("-d", "-f") if self.config.debug else ("-f",)
        exit_code, output = ObzoraNMS.call_script("alerts.php", args)
...
class PollerQueueManager(QueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(device_id, timeout=self.config.poller.frequency):
            ...
            exit_code, output = ObzoraNMS.call_script("lnms", args)
...
class DiscoveryQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(
            device_id, timeout=ObzoraNMS.normalize_wait(self.config.discovery.frequency)
        ):
            logger.info("Discovering device {}".format(device_id))

            args = ("-d", "-h", device_id) if self.config.debug else ("-h", device_id)
            exit_code, output = ObzoraNMS.call_script("discovery.php", args)
