...
class LegacyModule implements Module
{
    ...
    {
        if (! \ObzoraNMS\Util\Module::legacyDiscoveryExists($this->name)) {
    ...
    {
        if (! \ObzoraNMS\Util\Module::legacyPollingExists($this->name)) {
            Log::error("Module $this->name does not exist, please remove it from your configuration");
    ...
    {
        if ($type == 'discovery' && ! \ObzoraNMS\Util\Module::legacyDiscoveryExists($this->name)) {
            return null;
        }
        if ($type == 'poller' && ! \ObzoraNMS\Util\Module::legacyPollingExists($this->name)) {
            return null;
        }
