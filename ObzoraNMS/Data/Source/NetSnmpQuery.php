...
class NetSnmpQuery implements SnmpQueryInterface
{
    ...
    private function buildCli(string $command, array $oids): array
    {
        ...
        $timeout = $this->device->timeout ?? ObzoraConfig::get('snmp.timeout');
        if ($timeout && $timeout !== 1) {
            array_push($cmd, '-t', $timeout);
        }

        $retries = $this->device->retries ?? ObzoraConfig::get('snmp.retries');
    ...
    private function exec(string $command, array $oids): SnmpResponse
    {
        ...
        return Cache::driver($driver)->rememberForever($key, function () use ($command, $oids) {
            ...
            $proc->setTimeout(ObzoraConfig::get('snmp.exec_timeout', 1200));
    ...
    private function initCommand(string $binary, array $oids): array
    {
        if ($binary == 'snmpwalk') {
            ...
            if (! empty(array_intersect($oids, ObzoraConfig::getCombined($this->device->os, 'oids.unordered', 'snmp.')))) {
                $this->allowUnordered();
            }

            // handle bulk settings
            if ($this->device->snmpver !== 'v1'
                && ObzoraConfig::getOsSetting($this->device->os, 'snmp_bulk', true)
                && empty(array_intersect($oids, ObzoraConfig::getCombined($this->device->os, 'oids.no_bulk', 'snmp.'))) // skip for oids that do not work with bulk
            ) {
                $snmpcmd = [ObzoraConfig::get('snmpbulkwalk', 'snmpbulkwalk')];

                $max_repeaters = $this->device->getAttrib('snmp_max_repeaters') ?: ObzoraConfig::getOsSetting($this->device->os, 'snmp.max_repeaters', ObzoraConfig::get('snmp.max_repeaters', false));
                if ($max_repeaters > 0) {
        ...
        return [ObzoraConfig::get($binary, $binary)];
    }
    ...
    private function mibDirectories(): string
    {
        ...
        // os group
        if ($os_group = ObzoraConfig::getOsSetting($this->device->os, 'group')) {
        ...
        // os directory
        $os_mibdir = ObzoraConfig::getOsSetting($this->device->os, 'mib_dir');
    ...
    private function limitOids(array $oids): array
    {
        ...
        $configured_max = $this->device->getAttrib('snmp_max_oid') ?: ObzoraConfig::getOsSetting($this->device->os, 'snmp_max_oid', ObzoraConfig::get('snmp.max_oid', 10));
