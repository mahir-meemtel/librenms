...
class InfluxDBv2 extends BaseDatastore
{
    ...
    public function __construct()
    {
        ...
        try {
            $host = ObzoraConfig::get('influxdbv2.host', 'localhost');
            $transport = ObzoraConfig::get('influxdbv2.transport', 'http');
            $port = ObzoraConfig::get('influxdbv2.port', 8086);
            $bucket = ObzoraConfig::get('influxdbv2.bucket', 'obzora');
            $organization = ObzoraConfig::get('influxdbv2.organization', '');
            $allow_redirects = ObzoraConfig::get('influxdbv2.allow_redirects', true);
            $token = ObzoraConfig::get('influxdbv2.token', '');
            $debug = ObzoraConfig::get('influxdbv2.debug', false);
            $log_file = ObzoraConfig::get('influxdbv2.log_file', ObzoraConfig::get('log_file'));
            $timeout = ObzoraConfig::get('influxdbv2.timeout', 1);
            $verify = ObzoraConfig::get('influxdbv2.verify', false);
            $batch_size = ObzoraConfig::get('influxdbv2.batch_size', 1000);
            $max_retry = ObzoraConfig::get('influxdbv2.max_retry', 3);
    ...
    {
        return ObzoraConfig::get('influxdbv2.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        ...
        $excluded_groups = ObzoraConfig::get('influxdbv2.groups-exclude');
        ...
        if (ObzoraConfig::get('influxdbv2.debug') === true) {
