...
use ObzoraNMS\Exceptions\FileExistsException;
use ObzoraNMS\Exceptions\RrdException;
...
class Rrd extends BaseDatastore
{
    ...
    {
        return ObzoraConfig::get('rrd.enable', true);
    }

    protected function loadConfig(): void
    {
        $this->rrdcached = ObzoraConfig::get('rrdcached', false);
        $this->rrd_dir = ObzoraConfig::get('rrd_dir', ObzoraConfig::get('install_dir') . '/rrd');
        $this->step = ObzoraConfig::get('rrd.step', 300);
        $this->rra = ObzoraConfig::get(
            'rrd_rra',
        ...
        $this->version = ObzoraConfig::get('rrdtool_version', '1.4');
        $this->rrdtool_executable = ObzoraConfig::get('rrdtool', 'rrdtool');
    }
    ...
    /**
     * rename an rrdfile, can only be done on the ObzoraNMS server hosting the rrd files
     *
    ...
    private function command($command, $filename, $options): array
    {
        ...
        if ($this->disabled && ! in_array($command, $ro_commands)) {
            if (! ObzoraConfig::get('hide_rrd_disabled')) {
    ...
    {
        $proc = new Process([ObzoraConfig::get('rrdtool', 'rrdtool'), '--version']);
        $proc->run();
