...
class Kafka extends BaseDatastore
{
    ...
    public function __construct(Producer $client)
    {
        ...
        $this->topicName = ObzoraConfig::get('kafka.topic', 'obzora');

        // Cache the flush timeout value early to avoid Config during shutdown
        if ($this->kafkaFlushTimeout == null) {
            $this->kafkaFlushTimeout = ObzoraConfig::get('kafka.flush.timeout', 100);
        }

        // Load excluded values from config
        foreach (ObzoraConfig::get('kafka.groups-exclude', []) as $exclude_group) {
        ...
        foreach (ObzoraConfig::get('kafka.measurement-exclude', []) as $exclude_measurement) {
    ...
    public static function getClient(): Producer
    {
        ...
        $conf->set('bootstrap.servers', ObzoraConfig::get('kafka.broker.list', '127.0.2.2:9092'));
        // Set the idempotence
        $conf->set('enable.idempotence', ObzoraConfig::get('kafka.idempotence', false) ? 'true' : 'false');
        // Max queue allowed messages in poller memory
        $conf->set('queue.buffering.max.messages', ObzoraConfig::get('kafka.buffer.max.message', 1_000));
        // Num of messages each call to kafka
        $conf->set('batch.num.messages', ObzoraConfig::get('kafka.batch.max.message', 200));
        // Max wait time to acumulate before sending the batch
        $conf->set('linger.ms', ObzoraConfig::get('kafka.linger.ms', default: 50));
        // Change ACK
        $conf->set(
            ...
            ObzoraConfig::get('kafka.idempotence', false) ? 'all' :
                (ObzoraConfig::get('kafka.request.required.acks', '-1'))
        );

        // check if debug for ssl was set and enable it
        $confKafkaSSLDebug = ObzoraConfig::get('kafka.security.debug', null);
        $confKafkaSSLDebug != null || strlen($confKafkaSSLDebug) !== 0 ? $conf->set('debug', $confKafkaSSLDebug) : null;

        // config ssl
        $isSslEnabled = ObzoraConfig::get('kafka.ssl.enable', false);
        if ($isSslEnabled) {
            $conf->set('security.protocol', ObzoraConfig::get('kafka.ssl.protocol', 'ssl'));
            $conf->set('ssl.endpoint.identification.algorithm', 'none');

            // prepare all necessary obzora kafka config with associated rdkafka key
            $kafkaSSLConfigs = [
            ...
            foreach ($kafkaSSLConfigs as $configKey => $kafkaKey) {
                $configValue = ObzoraConfig::get($configKey, null);
    ...
    {
        return ObzoraConfig::get('kafka.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        try {
            ...
            if (ObzoraConfig::get('kafka.debug') === true) {
