...
class SSOAuthorizer extends MysqlAuthorizer
{
    ...
    public function authenticate($credentials)
    {
        ...
        if (ObzoraConfig::get('sso.create_users') || ObzoraConfig::get('sso.update_users')) {
            $user = User::thisAuth()->firstOrNew(['username' => $credentials['username']]);

            $create = ! $user->exists && ObzoraConfig::get('sso.create_users');
            $update = $user->exists && ObzoraConfig::get('sso.update_users');

            if ($create || $update) {
                $user->auth_type = LegacyAuth::getType();
                $user->can_modify_passwd = 0;
                $user->realname = $this->authSSOGetAttr(ObzoraConfig::get('sso.realname_attr'));
                $user->email = $this->authSSOGetAttr(ObzoraConfig::get('sso.email_attr'));
                $user->descr = $this->authSSOGetAttr(ObzoraConfig::get('sso.descr_attr')) ?: 'SSO User';
                $user->save();
            }
        }
    ...
    {
        return $this->authSSOGetAttr(ObzoraConfig::get('sso.user_attr'), '');
    }
    ...
    public function authSSOGetAttr($attr, $prefix = 'HTTP_')
    {
        ...
        if ($this->authSSOProxyTrusted()) {
            ...
            if (ObzoraConfig::get('sso.mode') === 'header' && array_key_exists($header_key, $_SERVER)) {
                return $_SERVER[$header_key];
            } elseif (ObzoraConfig::get('sso.mode') === 'env' && array_key_exists($attr, $_SERVER)) {
                return $_SERVER[$attr];
            } else {
                return null;
            }
    ...
    public function authSSOProxyTrusted()
    {
        ...
        if (ObzoraConfig::get('sso.trusted_proxies')) {
            try {
                ...
                $proxies = ObzoraConfig::get('sso.trusted_proxies');
    ...
    {
        if (ObzoraConfig::get('sso.group_strategy') === 'attribute') {
            if (ObzoraConfig::get('sso.level_attr')) {
                if (is_numeric($this->authSSOGetAttr(ObzoraConfig::get('sso.level_attr')))) {
                    return Arr::wrap(LegacyAuthLevel::tryFrom((int) $this->authSSOGetAttr(ObzoraConfig::get('sso.level_attr')))?->getName());
                } else {
        ...
        } elseif (ObzoraConfig::get('sso.group_strategy') === 'map') {
            if (ObzoraConfig::get('sso.group_level_map') && is_array(ObzoraConfig::get('sso.group_level_map')) && ObzoraConfig::get('sso.group_delimiter') && ObzoraConfig::get('sso.group_attr')) {
        ...
        } elseif (ObzoraConfig::get('sso.group_strategy') === 'static') {
            if (ObzoraConfig::get('sso.static_level')) {
                return Arr::wrap(LegacyAuthLevel::tryFrom((int) ObzoraConfig::get('sso.static_level'))?->getName());
            } else {
    ...
    public function authSSOParseGroups()
    {
        ...
        $groups = explode(ObzoraConfig::get('sso.group_delimiter', ';'), $this->authSSOGetAttr(ObzoraConfig::get('sso.group_attr')) ?? '');

        $valid_groups = [];

        // Only consider groups that match the filter expression - this is an optimisation for sites with thousands of groups
        if (ObzoraConfig::get('sso.group_filter')) {
            foreach ($groups as $group) {
                if (preg_match(ObzoraConfig::get('sso.group_filter'), $group)) {
        ...
        $level = (int) ObzoraConfig::get('sso.static_level', 0);

        $config_map = ObzoraConfig::get('sso.group_level_map');
