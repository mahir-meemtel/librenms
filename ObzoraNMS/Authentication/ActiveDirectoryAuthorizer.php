...
class ActiveDirectoryAuthorizer extends AuthorizerBase
{
    ...
    public function authenticate($credentials)
    {
        ...
        if ($this->ldap_connection) {
            ...
            if (! empty($credentials['username']) && ! empty($credentials['password']) && ldap_bind($this->ldap_connection, $credentials['username'] . '@' . ObzoraConfig::get('auth_ad_domain'), $credentials['password'])) {
                $this->is_bound = true;
                // group membership in one of the configured groups is required
                if (ObzoraConfig::get('auth_ad_require_groupmembership', true)) {
                    // cycle through defined groups, test for memberOf-ship
                    foreach (ObzoraConfig::get('auth_ad_groups', []) as $group => $level) {
                    ...
                    // failed to find user
                    if (ObzoraConfig::get('auth_ad_debug', false)) {
        ...
        } elseif (ObzoraConfig::get('auth_ad_debug', false)) {
    ...
    protected function userInGroup($username, $groupname)
    {
        ...
        $search = ldap_search(
            $connection,
            ObzoraConfig::get('auth_ad_base_dn'),
            $search_filter,
            ['cn']
        );
    ...
    public function getRoles(string $username): array|false
    {
        $roles = [];
        if (! ObzoraConfig::get('auth_ad_require_groupmembership', true)) {
            if (ObzoraConfig::get('auth_ad_global_read', false)) {
                $roles[] = 'global-read';
            }
        }

        // cycle through defined groups, test for memberOf-ship
        foreach (ObzoraConfig::get('auth_ad_groups', []) as $group => $data) {
            try {
    ...
    protected function connect()
    {
        ...
        if (ObzoraConfig::has('auth_ad_check_certificates') &&
            ! ObzoraConfig::get('auth_ad_check_certificates')) {
            putenv('LDAPTLS_REQCERT=never');
        }

        if (ObzoraConfig::has('auth_ad_check_certificates') && ObzoraConfig::get('auth_ad_debug')) {
            ldap_set_option(null, LDAP_OPT_DEBUG_LEVEL, 7);
        }

        $this->ldap_connection = ldap_connect(ObzoraConfig::get('auth_ad_url'));
        if (empty($this->ldap_connection)) {
            throw new AuthenticationException('Fatal error while connecting to AD, uri not valid: ' . ObzoraConfig::get('auth_ad_url'));
        }
        ...
        $starttls = ObzoraConfig::get('auth_ad_starttls');
    ...
    public function bind($credentials = [])
    {
        ...
        if (ObzoraConfig::has('auth_ad_binduser') && ObzoraConfig::has('auth_ad_bindpassword')) {
            $username = ObzoraConfig::get('auth_ad_binduser');
            $password = ObzoraConfig::get('auth_ad_bindpassword');
        }
        $username .= '@' . ObzoraConfig::get('auth_ad_domain');

        ldap_set_option($this->ldap_connection, LDAP_OPT_NETWORK_TIMEOUT, ObzoraConfig::get('auth_ad_timeout', 5));
