...
trait ActiveDirectoryCommon
{
    ...
    protected function userFilter($username)
    {
        ...
        $extra = ObzoraConfig::get('auth_ad_user_filter');
        if ($extra) {
            $user_filter .= $extra;
        }
        $user_filter .= ')';
    ...
    protected function groupFilter($groupname)
    {
        ...
        $extra = ObzoraConfig::get('auth_ad_group_filter');
        if ($extra) {
    ...
    public function getGroupList()
    {
        ...
        if (ObzoraConfig::has('auth_ad_group')) {
            if (ObzoraConfig::get('auth_ad_group') !== $default_group) {
                $ldap_groups[] = ObzoraConfig::get('auth_ad_group');
            }
        }

        if (! ObzoraConfig::has('auth_ad_groups') && ! ObzoraConfig::has('auth_ad_group')) {
            $ldap_groups[] = $this->getDn($default_group);
        }

        foreach (ObzoraConfig::get('auth_ad_groups') as $key => $value) {
    ...
    public function getUser($user_id): array
    {
        ...
        $search = ldap_search($connection, ObzoraConfig::get('auth_ad_base_dn'), $search_filter, $attributes);
    ...
    protected function getDomainSid(): string
    {
        ...
        $dn_candidate = preg_replace('/^.*?DC=/i', 'DC=', ObzoraConfig::get('auth_ad_base_dn'));
