...
class LdapAuthorizationAuthorizer extends AuthorizerBase
{
    ...
    public function __construct()
    {
        ...
        if (ObzoraConfig::get('auth_ldap_version')) {
            ldap_set_option($this->ldap_connection, LDAP_OPT_PROTOCOL_VERSION, ObzoraConfig::get('auth_ldap_version'));
        }

        if (ObzoraConfig::get('auth_ldap_starttls') && (ObzoraConfig::get('auth_ldap_starttls') == 'optional' || ObzoraConfig::get('auth_ldap_starttls') == 'required')) {
            $tls = ldap_start_tls($this->ldap_connection);
            if (ObzoraConfig::get('auth_ldap_starttls') == 'required' && $tls === false) {
    ...
    public function authenticate($credentials)
    {
        ...
        $guest = ObzoraConfig::get('http_auth_guest');
    ...
    public function userExists($username, $throw_exception = false)
    {
        ...
        $search = ldap_search($this->ldap_connection, trim(ObzoraConfig::get('auth_ldap_suffix'), ','), $filter);
        if ($search === false) {
    ...
    public function getRoles(string $username): array|false
    {
        ...
        $filter = '(&(|(cn=' . implode(')(cn=', array_keys(ObzoraConfig::get('auth_ldap_groups'))) . '))(' . ObzoraConfig::get('auth_ldap_groupmemberattr') . '=' . $this->getMembername($username) . '))';
        $search = ldap_search($this->ldap_connection, ObzoraConfig::get('auth_ldap_groupbase'), $filter);
        if ($search === false) {
            throw new AuthenticationException('Role search failed: ' . ldap_error($this->ldap_connection));
        }
        $entries = ldap_get_entries($this->ldap_connection, $search);

        $authLdapGroups = ObzoraConfig::get('auth_ldap_groups');
        // Collect all roles
    ...
    public function getUserid($username)
    {
        ...
        $guest_username = ObzoraConfig::get('http_auth_guest');
    ...
    public function getUser($user_id)
    {
        ...
        if ($entries['count']) {
            ...
            $email = $entry[ObzoraConfig::get('auth_ldap_emailattr')][0];
            $ldap_groups = $this->getGroupList();
            foreach ($ldap_groups as $ldap_group) {
                $ldap_comparison = ldap_compare(
                    $this->ldap_connection,
                    $ldap_group,
                    ObzoraConfig::get('auth_ldap_groupmemberattr'),
                    $this->getMembername($username)
                );
                if (! ObzoraConfig::has('auth_ldap_group') || $ldap_comparison === true) {
                    return [
                        'username' => $username,
                        'realname' => $realname,
                        'user_id' => $user_id,
    ...
    {
        if (ObzoraConfig::get('auth_ldap_groupmembertype') == 'fulldn') {
            $membername = ObzoraConfig::get('auth_ldap_prefix') . $username . ObzoraConfig::get('auth_ldap_suffix');
        } elseif (ObzoraConfig::get('auth_ldap_groupmembertype') == 'puredn') {
    ...
    public function getGroupList()
    {
        ...
        if (ObzoraConfig::has('auth_ldap_group')) {
            if (ObzoraConfig::get('auth_ldap_group') !== $default_group) {
                $ldap_groups[] = ObzoraConfig::get('auth_ldap_group');
            }
        }

        foreach (ObzoraConfig::get('auth_ldap_groups') as $key => $value) {
            $dn = "cn=$key," . ObzoraConfig::get('auth_ldap_groupbase');
            $ldap_groups[] = $dn;
        }
