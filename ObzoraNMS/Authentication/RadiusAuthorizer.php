...
class RadiusAuthorizer extends MysqlAuthorizer
{
    ...
    {
        $this->radius = new Radius(ObzoraConfig::get('radius.hostname'), ObzoraConfig::get('radius.secret'), ObzoraConfig::get('radius.suffix'), ObzoraConfig::get('radius.timeout'), ObzoraConfig::get('radius.port'));
    }
    ...
    public function authenticate($credentials)
    {
        ...
        if ($this->radius->accessRequest($credentials['username'], $password) === true) {
            ...
            if ($filter_id_attribute && Str::startsWith($filter_id_attribute, 'obzora_role_')) {
                $this->roles[$credentials['username']] = [substr($filter_id_attribute, 14)];
            }

            if (ObzoraConfig::get('radius.enforce_roles') || $new_user) {
    ...
    private function getDefaultRoles(): array
    {
        ...
        return ObzoraConfig::get('radius.default_roles')
            ?: Arr::wrap(LegacyAuthLevel::from(ObzoraConfig::get('radius.default_level') ?? 1)->getName());
    }
}
