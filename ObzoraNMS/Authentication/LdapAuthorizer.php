...
use ErrorException;
use LDAP\Connection;
use ObzoraNMS\Enum\LegacyAuthLevel;
use ObzoraNMS\Exceptions\AuthenticationException;
use ObzoraNMS\Exceptions\LdapMissingException;
...
class LdapAuthorizer extends AuthorizerBase
{
    ...
    public function authenticate($credentials)
    {
        ...
        if (! empty($credentials['username'])) {
            ...
            if (ObzoraConfig::get('auth_ldap_wildcard_ou', false)) {
            ...
            if (! empty($credentials['password']) && ldap_bind($connection, $this->getFullDn($username), $credentials['password'])) {
                ...
                if ((ObzoraConfig::has('auth_ldap_binduser') || ObzoraConfig::has('auth_ldap_binddn')) && ObzoraConfig::has('auth_ldap_bindpassword')) {
                    $this->bind();
                }

                if (ObzoraConfig::get('auth_ldap_require_groupmembership') === false) {
                ...
                if (empty($ldap_groups)) {
                    ...
                    foreach ($ldap_groups as $ldap_group) {
                        if (ObzoraConfig::get('auth_ldap_userdn') === true) {
                            $ldap_comparison = ldap_compare(
                                $connection,
                                $ldap_group,
                                ObzoraConfig::get('auth_ldap_groupmemberattr', 'memberUid'),
    ...
    public function userExists($username, $throw_exception = false)
    {
        try {
            ...
            $filter = '(' . ObzoraConfig::get('auth_ldap_prefix') . $username . ')';
            $search = ldap_search($connection, trim(ObzoraConfig::get('auth_ldap_suffix'), ','), $filter);
    ...
    public function getRoles(string $username): array|false
    {
        try {
            $connection = $this->getLdapConnection();
            $groups = ObzoraConfig::get('auth_ldap_groups');
            ...
            if (ObzoraConfig::get('auth_ldap_userdn') === true) {
                $filter = "(&{$ldap_group_filter}(" . trim(ObzoraConfig::get('auth_ldap_groupmemberattr', 'memberUid')) . '=' . $this->getFullDn($username) . '))';
            } else {
                $filter = "(&{$ldap_group_filter}(" . trim(ObzoraConfig::get('auth_ldap_groupmemberattr', 'memberUid')) . '=' . $this->getMembername($username) . '))';
            }
            $search = ldap_search($connection, ObzoraConfig::get('auth_ldap_groupbase'), $filter);
            $entries = ldap_get_entries($connection, $search);
    ...
    public function getUserid($username)
    {
        try {
            ...
            if ($entries['count']) {
                $uid_attr = strtolower(ObzoraConfig::get('auth_ldap_uid_attribute', 'uidnumber'));
    ...
    public function getUser($user_id): false|array
    {
        ...
        $filter = '(' . ObzoraConfig::get('auth_ldap_prefix') . $this->userloginname . ')';
        if (ObzoraConfig::get('auth_ldap_userlist_filter') != null) {
            $filter = '(' . ObzoraConfig::get('auth_ldap_userlist_filter') . ')';
        }
    ...
    {
        $type = ObzoraConfig::get('auth_ldap_groupmembertype');
        ...
        if ($type == 'puredn') {
            try {
                $connection = $this->getLdapConnection();
                $filter = '(' . ObzoraConfig::get('auth_ldap_attr.uid') . '=' . $username . ')';
                $search = ldap_search($connection, ObzoraConfig::get('auth_ldap_groupbase'), $filter);
                $entries = ldap_get_entries($connection, $search);
    ...
    public function getGroupList(): array
    {
        ...
        if (ObzoraConfig::get('auth_ldap_group', $default_group) !== $default_group) {
            $ldap_groups[] = ObzoraConfig::get('auth_ldap_group');
        }

        foreach (ObzoraConfig::get('auth_ldap_groups') as $key => $value) {
            $ldap_groups[] = "cn=$key," . ObzoraConfig::get('auth_ldap_groupbase');
        }
    ...
    {
        return ObzoraConfig::get('auth_ldap_prefix', '') . $username . ObzoraConfig::get('auth_ldap_suffix', '');
    }

    /**
     * Set auth_ldap_suffix ou according to $username dn
     * useful if ObzoraConfig::get('auth_ldap_wildcard_ou) is set
     *
     * @internal
     *
     * @return false|true
    ...
    protected function setAuthLdapSuffixOu($username): bool
    {
        $connection = $this->getLdapConnection();
        $filter = '(' . ObzoraConfig::get('auth_ldap_attr.uid') . '=' . $username . ')';
        $base_dn = preg_replace('/,ou=[^,]+,/', ',', ObzoraConfig::get('auth_ldap_suffix'));
        ...
        foreach (ldap_get_entries($connection, $search) as $entry) {
            if (isset($entry['uid'][0]) && $entry['uid'][0] == $username) {
                ...
                $user_ou = $matches[1] ?? '';
                $new_auth_ldap_suffix = preg_replace('/,ou=[^,]+,/', ',ou=' . $user_ou . ',', ObzoraConfig::get('auth_ldap_suffix'));
                ObzoraConfig::set('auth_ldap_suffix', $new_auth_ldap_suffix);
    ...
    private function ldapToUser($entry): array
    {
        ...
        return [
            ...
            'email' => $entry[ObzoraConfig::get('auth_ldap_emailattr', 'mail')][0] ?? null,
        ];
    }
    ...
    private function connect(): void
    {
        ...
        $port = ObzoraConfig::get('auth_ldap_port');
        $uri = ObzoraConfig::get('auth_ldap_server');
        ...
        ldap_set_option($this->ldap_connection, LDAP_OPT_PROTOCOL_VERSION, ObzoraConfig::get('auth_ldap_version', 3));

        $use_tls = ObzoraConfig::get('auth_ldap_starttls');
    ...
    {
        if (ObzoraConfig::get('auth_ldap_debug')) {
         ...
         */
        if (ObzoraConfig::get('auth_ldap_cacertfile')) {
            ldap_set_option($this->ldap_connection, LDAP_OPT_X_TLS_CACERTFILE, ObzoraConfig::get('auth_ldap_cacertfile'));
        }
        if (ObzoraConfig::get('auth_ldap_ignorecert')) {
        ...
        if ((ObzoraConfig::has('auth_ldap_binduser') || ObzoraConfig::has('auth_ldap_binddn')) && ObzoraConfig::has('auth_ldap_bindpassword')) {
            if (ObzoraConfig::get('auth_ldap_binddn') == null) {
                ObzoraConfig::set('auth_ldap_binddn', $this->getFullDn(ObzoraConfig::get('auth_ldap_binduser')));
            }
            $username = ObzoraConfig::get('auth_ldap_binddn');
            $password = ObzoraConfig::get('auth_ldap_bindpassword');
        } elseif (! empty($credentials['username'])) {
            $username = $this->getFullDn($credentials['username']);
        }

        // With specified bind user
        ldap_set_option($this->ldap_connection, LDAP_OPT_NETWORK_TIMEOUT, ObzoraConfig::get('auth_ldap_timeout', 5));
