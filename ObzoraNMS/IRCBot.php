...
class IRCBot
{
    ...
    public function __construct()
    {
        ...
        $this->config = ObzoraConfig::getAll();
    ...
    private function _auth($params)
    {
        ...
        if (strlen($params[0]) == 64) {
            ...
            if ($user->email && $user->username == $params[0]) {
                ...
                try {
                    Mail::send($user->email, 'ObzoraNMS IRC-Bot Authtoken', "Your Authtoken for the IRC-Bot:\r\n\r\n" . $token . "\r\n\r\n");
    ...
    private function _reload($params)
    {
        if ($this->user['user']->can('irc.reload')) {
            ...
            if (ObzoraConfig::getAll() != $this->config) {
                $this->__construct();
