...
class AlertUtil
{
    ...
    public static function getContacts($results)
    {
        ...
        if (ObzoraConfig::get('alert.default_only') === true || ObzoraConfig::get('alerts.email.default_only') === true) {
            $email = ObzoraConfig::get('alert.default_mail', ObzoraConfig::get('alerts.email.default'));
        ...
        if (ObzoraConfig::get('alert.syscontact')) {
            $contacts = array_merge($contacts, self::findContactsSysContact($results));
        }

        if (ObzoraConfig::get('alert.users')) {
            $contacts = array_merge($contacts, self::findContactsOwners($results));
        }

        $roles = ObzoraConfig::get('alert.globals')
            ? ['admin', 'global-read']
            : (ObzoraConfig::get('alert.admins') ? ['admin'] : []);
        if ($roles) {
        ...
        $default_mail = ObzoraConfig::get('alert.default_mail');
        if (! isset($tmp_contacts[$default_mail]) && ObzoraConfig::get('alert.default_copy')) {
            $tmp_contacts[$default_mail] = '';
        }
        // Send email to default contact if no other contact found
        if (empty($tmp_contacts) && ObzoraConfig::get('alert.default_if_none') && $default_mail) {
            $tmp_contacts[$default_mail] = '';
        }
    ...
    {
        $macros = ObzoraConfig::get('alert.macros.rule', []);
        krsort($macros);
