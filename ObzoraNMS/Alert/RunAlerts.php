...
class RunAlerts
{
    ...
    {
        if (ObzoraConfig::get('alert.fixed-contacts') == false) {
        ...
        if (is_array($obj)) {
            ...
            if ($alert['state'] != AlertState::ACKNOWLEDGED || ObzoraConfig::get('alert.acknowledged') === true) {
    ...
    public function runAlerts()
    {
        foreach ($this->loadAlerts('alerts.state != ' . AlertState::ACKNOWLEDGED . ' && alerts.open = 1') as $alert) {
            ...
            $tolerence_window = ObzoraConfig::get('alert.tolerance_window');
    ...
    public function extTransports($obj)
    {
        ...
        if (ObzoraConfig::get('alert.transports.mail') === true && ! empty($obj['contacts'])) {
            $transport_maps[] = [
                'transport_id' => null,
                'transport_type' => 'mail',
