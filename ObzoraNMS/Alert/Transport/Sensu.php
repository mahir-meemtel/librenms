...
class Sensu extends Transport
{
    ...
    public function deliverAlert(array $alert_data): bool
    {
        ...
        if ($alert_data['state'] !== AlertState::RECOVERED && $alert_data['state'] !== AlertState::ACKNOWLEDGED && $alert_data['alerted'] === 0) {
            ...
            $data = $this->generateData($alert_data, self::OK, (int) round(ObzoraConfig::get('rrd.step', 300) / 2));
    ...
    private function generateData(array $alert_data, int $status, int $offset = 0): array
    {
        ...
        return [
            'check' => [
                ...
                'command' => sprintf('ObzoraNMS: %s', $alert_data['builder']),
                'executed' => time() - $offset,
                'interval' => ObzoraConfig::get('rrd.step', 300),
                'issued' => time() - $offset,
    ...
    private function generateAnnotations(array $alert_data): array
    {
        return array_filter([
            ...
            'obzora-notes' => $alert_data['notes'],
            'obzora-device-id' => strval($alert_data['device_id']),
            'obzora-rule-id' => strval($alert_data['rule_id']),
            'obzora-status-reason' => $alert_data['status_reason'],
    ...
    {
        // Sensu only has a single short (status) to indicate both severity and status, so we need to map ObzoraNMS' state and severity onto it

        if ($state === AlertState::RECOVERED) {
            // ObzoraNMS alert is resolved, send ok
            return self::OK;
        }
    ...
    {
        // Shrink the last domain components - e.g. obzora.corp.example.net becomes obzora.cen
