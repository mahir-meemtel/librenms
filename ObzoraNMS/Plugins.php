 ...
 *
 * @author     ObzoraNMS Group
 *
 * @link       https://www.obzora.meemtel.com
 *
 * @copyright  2016
 *
 * Supported hooks
...
class Plugins
{
    ...
    public static function start()
    {
        ...
        self::$plugins = [];
        $plugin_dir = ObzoraConfig::get('plugin_dir');
    ...
    {
        chdir(ObzoraConfig::get('install_dir') . '/html');
        ...
        chdir(ObzoraConfig::get('install_dir'));
        ...
        if (! empty(self::$plugins[$hook])) {
            foreach (self::$plugins[$hook] as $plugin) {
                try {
                    ...
                    Notifications::create("Plugin $name disabled", "$name caused an error and was disabled, please check with the plugin creator to fix the error. The error can be found in logs/obzora.log", 'plugins', 2);
    ...
    public static function scanNew()
    {
        ...
        if (file_exists(ObzoraConfig::get('plugin_dir'))) {
            $plugin_files = array_diff(scandir(ObzoraConfig::get('plugin_dir')), ['..', '.']);
            $plugin_files = array_diff($plugin_files, Plugin::versionOne()->pluck('plugin_name')->toArray());
            foreach ($plugin_files as $name) {
                if (is_dir(ObzoraConfig::get('plugin_dir') . '/' . $name)
                    && is_file(ObzoraConfig::get('plugin_dir') . '/' . $name . '/' . $name . '.php')) {
    ...
    public static function scanRemoved()
    {
        ...
        if (file_exists(ObzoraConfig::get('plugin_dir'))) {
            $plugin_files = array_diff(scandir(ObzoraConfig::get('plugin_dir')), ['.', '..', '.gitignore']);
