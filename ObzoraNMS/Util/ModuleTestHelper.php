...
use ObzoraNMS\Data\Source\SnmpResponse;
use ObzoraNMS\Exceptions\FileNotFoundException;
use ObzoraNMS\Exceptions\InvalidModuleException;
...
class ModuleTestHelper
{
    ...
    public function __construct($modules, $os, $variant = '')
    {
        ...
        $install_dir = ObzoraConfig::get('install_dir');
        ...
        ObzoraConfig::set('hide_rrd_disabled', true);
        ObzoraConfig::set('influxdb.enable', false);
        ObzoraConfig::set('influxdbv2.enable', false);
        ObzoraConfig::set('graphite.enable', false);
        ObzoraConfig::set('prometheus.enable', false);
    ...
    public static function findOsWithData($modules = [], ?string $os_filter = null)
    {
        ...
        foreach (glob(ObzoraConfig::get('install_dir') . '/tests/data/*.json') as $file) {
    ...
    {
        global $device;
        ObzoraConfig::set('rrd.enable', false); // disable rrd
        ObzoraConfig::set('rrdtool_version', '1.7.2'); // don't detect rrdtool version, rrdtool is not install on ci
    ...
    {
        if ($short) {
            return ltrim(str_replace(ObzoraConfig::get('install_dir'), '', $this->json_file), '/');
        }
