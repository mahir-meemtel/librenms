...
use ObzoraNMS\Data\Graphing\GraphImage;
...
class Graph
{
    ...
    public static function getSubtypes($type, $device = null): array
    {
        ...
        if ($device != null) {
            ...
            foreach (ObzoraConfig::get('graph_types') as $type => $type_data) {
    ...
    {
        return ObzoraConfig::get("graph_types.$type.$subtype.section") == 'mib';
    }

    public static function getOverviewGraphsForDevice(Device $device): array
    {
        if ($device->snmp_disable) {
            return Arr::wrap(ObzoraConfig::getOsSetting('ping', 'over'));
        }

        if ($graphs = ObzoraConfig::getOsSetting($device->os, 'over')) {
            return Arr::wrap($graphs);
        }

        $os_group = ObzoraConfig::getOsSetting($device->os, 'group');

        return Arr::wrap(ObzoraConfig::get("os_group.$os_group.over", ObzoraConfig::get('os.default.over')));
    }
    ...
    {
        $type = ObzoraConfig::get('webui.graph_type');
