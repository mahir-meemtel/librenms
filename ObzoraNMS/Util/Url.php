...
class Url
{
    ...
    public static function deviceLink($device, $text = '', $vars = [], $start = 0, $end = 0, $escape_text = 1, $overlib = 1)
    {
        ...
        if ($device->os) {
            $devinfo .= ($devinfo ? ' - ' : '') . ObzoraConfig::getOsSetting($device->os, 'text');
        }
    ...
    public static function generate($vars, $new_vars = [])
    {
        ...
        $url = url(ObzoraConfig::get('base_url', true) . $vars['page'] . '');
        unset($vars['page']);
    ...
    public static function lazyGraphTag($args)
    {
        ...
        if (ObzoraConfig::get('enable_lazy_load', true)) {
    ...
    public static function overlibLink($url, $text, $contents, $class = null)
    {
        ...
        if (ObzoraConfig::get('web_mouseover', true)) {
            $defaults = ObzoraConfig::get('overlib_defaults', ",FGCOLOR,'#ffffff', BGCOLOR, '#e5e5e5', BORDER, 5, CELLPAD, 4, CAPCOLOR, '#555555', TEXTCOLOR, '#3e3e3e'");
    ...
    public static function findOsImage($os, $feature, $icon = null, $dir = 'images/os/')
    {
        ...
        if ($os) {
            ...
            $os_icon = ObzoraConfig::getOsSetting($os, 'icon', $os);
            $possibilities[] = "$os_icon.svg";
            $possibilities[] = "$os_icon.png";
        }

        foreach ($possibilities as $file) {
            if (is_file(ObzoraConfig::get('html_dir') . "/$dir" . $file)) {
                return $file;
            }
        }
    ...
    public static function parseLegacyPathVars(?string $path = null): array
    {
        ...
        $base_url = parse_url(ObzoraConfig::get('base_url'))['path'] ?? '';
