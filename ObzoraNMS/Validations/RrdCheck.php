...
use ObzoraNMS\RRDRecursiveFilterIterator;
...
class RrdCheck extends BaseValidation
{
    ...
    public function validate(Validator $validator): void
    {
        ...
        $rrd_directory = new RecursiveDirectoryIterator(ObzoraConfig::get('rrd_dir'));
        ...
        echo "\nScanning " . $rrd_total . ' rrd files in ' . ObzoraConfig::get('rrd_dir') . "...\n";
    ...
    private function test($path, &$stdOutput, &$stdError)
    {
        ...
        $command = ObzoraConfig::get('rrdtool') . ' info ' . escapeshellarg($path);
        $process = proc_open(
            $command,
            [
                0 => ['pipe', 'r'],
