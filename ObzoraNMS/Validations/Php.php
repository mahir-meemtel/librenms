...
class Php extends BaseValidation
{
    ...
    private function checkVersion(Validator $validator)
    {
        ...
        if (ObzoraConfig::get('update') && version_compare(PHP_VERSION, self::PHP_MIN_VERSION, '<')) {
            $validator->warn('PHP version ' . self::PHP_MIN_VERSION . ' is the minimum supported version as of ' . self::PHP_MIN_VERSION_DATE . '. We recommend you update PHP to a supported version (' . self::PHP_RECOMMENDED_VERSION . ' suggested) to continue to receive updates. If you do not update PHP, ObzoraNMS will continue to function but stop receiving bug fixes and updates.');
        }
    ...
    private function checkExtensions(Validator $validator)
    {
        ...
        if (ObzoraConfig::get('distributed_poller') && Config::get('cache.default') == 'memcached') {
