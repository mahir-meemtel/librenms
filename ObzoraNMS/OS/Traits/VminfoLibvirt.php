...
trait VminfoLibvirt
{
    public function discoverVminfo(): Collection
    {
        ...
        if (! ObzoraConfig::get('enable_libvirt')) {
        ...
        if (ObzoraConfig::has('libvirt_username')) {
            $userHostname = ObzoraConfig::get('libvirt_username') . '@' . $userHostname;
        }

        foreach (ObzoraConfig::get('libvirt_protocols') as $method) {
            ...
            if ($ssh_ok || ! Str::contains($method, 'ssh')) {
                ...
                unset($domlist);
                exec(ObzoraConfig::get('virsh') . ' -rc ' . $uri . ' list', $domlist);
                ...
                foreach ($domlist as $dom) {
                    ...
                    if (is_numeric($dom_id)) {
                        ...
                        unset($vm_info_array);
                        exec(ObzoraConfig::get('virsh') . ' -rc ' . $uri . ' dumpxml ' . $dom_id, $vm_info_array);
                        ...
                        exec(ObzoraConfig::get('virsh') . ' -rc ' . $uri . ' domstate ' . $dom_id, $vm_state);
