...
if (! isset($options['q'])) {
    echo \App\Facades\ObzoraConfig::get('project_name') . " Discovery\n";
}
...
if (Debug::set(isset($options['d'])) || isset($options['v'])) {
    echo \ObzoraNMS\Util\Version::get()->header();
...
if (! empty(\App\Facades\ObzoraConfig::get('distributed_poller_group'))) {
    $where .= ' AND poller_group IN(' . \App\Facades\ObzoraConfig::get('distributed_poller_group') . ')';
}
...
$string = $argv[0] . " $doing " . date(\App\Facades\ObzoraConfig::get('dateformat.compact')) . " - $discovered_devices devices discovered in $proctime secs";
d_echo("$string\n");
