...
function shorthost($hostname, $len = 12)
{
    ...
    $len = ObzoraConfig::get('shorthost_target_length', $len);
...
function get_sensor_rrd_name($device, $sensor)
{
    ...
    if (ObzoraConfig::getOsSetting($device['os'], 'sensor_descr') || $sensor['poller_type'] == 'ipmi') {
...
{
    // fake a port model
    return \ObzoraNMS\Util\Url::portLinkDisplayClass((object) ['ifOperStatus' => $ifOperStatus, 'ifAdminStatus' => $ifAdminStatus]);
}
...
function get_graph_subtypes($type, $device = null)
{
    ...
    if ($handle = opendir(ObzoraConfig::get('install_dir') . "/includes/html/graphs/$type/")) {
...
{
    $smokeping = new \ObzoraNMS\Util\Smokeping(DeviceCache::get((int) $device['device_id']));
...
function format_hostname($device): string
{
    ...
    return \App\View\SimpleTemplate::parse(empty($device['display']) ? ObzoraConfig::get('device_display_default', '{{ $hostname }}') : $device['display'], [
        'hostname' => $hostname,
        'sysName' => $sysName,
...
function get_vm_parent_id($device)
{
    ...
    return dbFetchCell('SELECT `device_id` FROM `vminfo` WHERE `vmwVmDisplayName` = ? OR `vmwVmDisplayName` = ?', [$device['hostname'], $device['hostname'] . '.' . ObzoraConfig::get('mydomain')]);
}
