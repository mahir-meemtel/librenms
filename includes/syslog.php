...
function process_syslog($entry, $update)
{
    ...
    foreach (ObzoraConfig::get('syslog_filter') as $bi) {
    ...
    $syslog_xlate = ObzoraConfig::get('syslog_xlate');
    ...
    if ($entry['device_id']) {
        ...
        if (ObzoraConfig::get('enable_syslog_hooks') && is_array(ObzoraConfig::getOsSetting($os, 'syslog_hook'))) {
            foreach (ObzoraConfig::getOsSetting($os, 'syslog_hook') as $k => $v) {
