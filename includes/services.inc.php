...
function poll_service($service)
{
    ...
    $check_script = ObzoraConfig::get('install_dir') . '/includes/services/check_' . strtolower($service['service_type']) . '.inc.php';
    ...
    if ($check_cmd == '') {
        $check_cmd = ObzoraConfig::get('nagios_plugins') . '/check_' . $service['service_type'] . ' -H ' . ($service['service_ip'] ?: $service['hostname']);
...
{
    return \ObzoraNMS\Services::list();
}
