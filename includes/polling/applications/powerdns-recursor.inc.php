...
} elseif (ObzoraConfig::has('apps.powerdns-recursor.api-key')) {
    $port = ObzoraConfig::get('apps.powerdns-recursor.port', 8082);
    $scheme = ObzoraConfig::get('apps.powerdns-recursor.https') ? 'https://' : 'http://';

    d_echo("\nNo Agent Data. Attempting to connect directly to the powerdns-recursor server $scheme" . $device['hostname'] . ":$port\n");
    $context = stream_context_create(['http' => ['header' => 'X-API-Key: ' . ObzoraConfig::get('apps.powerdns-recursor.api-key')]]);
