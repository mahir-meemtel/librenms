...
use ObzoraNMS\Enum\Sensor;
use ObzoraNMS\Enum\Severity;
use ObzoraNMS\Exceptions\JsonAppBase64DecodeException;
use ObzoraNMS\Exceptions\JsonAppBlankJsonException;
use ObzoraNMS\Exceptions\JsonAppExtendErroredException;
use ObzoraNMS\Exceptions\JsonAppGzipDecodeException;
use ObzoraNMS\Exceptions\JsonAppMissingKeysException;
use ObzoraNMS\Exceptions\JsonAppParsingFailedException;
use ObzoraNMS\Exceptions\JsonAppPollingFailedException;
use ObzoraNMS\Exceptions\JsonAppWrongVersionException;
...
{
    $sensor_cache = [];
    if (file_exists(ObzoraConfig::get('install_dir') . '/includes/polling/sensors/pre-cache/' . $device['os'] . '.inc.php')) {
        include ObzoraConfig::get('install_dir') . '/includes/polling/sensors/pre-cache/' . $device['os'] . '.inc.php';
    }
...
function poll_sensor($device, $class)
{
    ...
    foreach ($sensors as $sensor) {
        ...
        if ($sensor['poller_type'] == 'snmp') {
            ...
            if (file_exists(ObzoraConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os'] . '.inc.php')) {
                require ObzoraConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os'] . '.inc.php';
            } elseif (isset($device['os_group']) && file_exists(ObzoraConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os_group'] . '.inc.php')) {
                require ObzoraConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os_group'] . '.inc.php';
            }
 ...
 * https://github.com/obzora/obzora-agent/blob/master/utils/lnms_return_optimizer
