...
if ($device['os_group'] == 'unix' || $device['os'] == 'windows') {
    echo \App\Facades\ObzoraConfig::get('project_name') . ' UNIX Agent: ';

    $agent_port = get_dev_attrib($device, 'override_Unixagent_port');
    if (empty($agent_port)) {
        $agent_port = \App\Facades\ObzoraConfig::get('unix-agent.port');
    }

    $agent_start = microtime(true);
    $agent = null;
    try {
        $poller_target = \ObzoraNMS\Util\Rewrite::addIpv6Brackets(Device::pollerTarget($device['hostname']));
        $agent = @fsockopen($poller_target, $agent_port, $errno, $errstr, \App\Facades\ObzoraConfig::get('unix-agent.connection-timeout'));
    ...
    if (! $agent) {
        ...
        stream_set_timeout($agent, \App\Facades\ObzoraConfig::get('unix-agent.read-timeout'));
