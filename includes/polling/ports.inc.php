...
if ($device['os'] === 'f5' && (version_compare($device['version'], '11.2.0', '>=') && version_compare($device['version'], '11.7', '<'))) {
    ...
    if ($selected_attrib !== null ? $selected_attrib == 'true' : ObzoraConfig::getOsSetting($device['os'], 'polling.selected_ports')) {
        ...
        $walk_base = false;
        if (! ObzoraConfig::has("os.{$device['os']}.polling.selected_ports") && $selected_attrib === null) {
        ...
        if (! in_array(strtolower($device['hardware'] ?? ''), array_map('strtolower', (array) ObzoraConfig::getOsSetting($device['os'], 'bad_ifXEntry', [])))) {
        ...
        if ($device['os'] != 'asa') {
            ...
            if (ObzoraConfig::get('enable_ports_poe') || ObzoraConfig::get('enable_ports_etherlike')) {
...
if (ObzoraConfig::get('enable_ports_poe')) {
    // Code by OS device
...
// By default obzoraNMS uses the ifIndex to associate ports on devices with ports discoverd/polled
...
$port_association_mode = ObzoraConfig::get('default_port_association_mode');
...
foreach ($port_stats as $ifIndex => $port) {
    ...
    if (is_port_valid($port, $device)) {
        ...
        if (! $port_id || empty($ports[$port_id])) {
             ...
             * value is empty().
             */
            if (ObzoraConfig::get('ignore_unmapable_port') === true && in_array($port[$port_association_mode], ['', null])) {
                continue;
            }
...
foreach ($ports as $port) {
    ...
    if ($port_stats[$ifIndex]) {
            ...
            if ($port[$oid] != $current_oid && ! isset($current_oid)) {
                ...
                if ($oid == 'ifSpeed') {
                    ...
                    if ($port_tune == 'true' ||
                        ...
                        (ObzoraConfig::get('rrdtool_tune') == 'true' && $port_tune != 'false' && $device_tune != 'false')) {
        ...
        if (ObzoraConfig::has('port_descr_parser') && is_file(ObzoraConfig::get('install_dir') . '/' . ObzoraConfig::get('port_descr_parser'))) {
            $port_attribs = [
                'type',
                'descr',
                'circuit',
            ...
            include ObzoraConfig::get('install_dir') . '/' . ObzoraConfig::get('port_descr_parser');
        ...
        if (! empty($port['skipped'])) {
            ...
            if (ObzoraConfig::get('debug_port.' . $port['port_id'])) {
            ...
            // End Update PAgP
            // Do EtherLike-MIB
            if (ObzoraConfig::get('enable_ports_etherlike')) {
