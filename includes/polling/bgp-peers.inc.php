...
if (! empty($peers)) {
    ...
    foreach ($peers as $peer) {
        ...
        if ($peer_data['bgpPeerFsmEstablishedTime']) {
            if (! (is_array(\App\Facades\ObzoraConfig::get('alerts.bgp.whitelist'))
                    && ! in_array($peer['bgpPeerRemoteAs'], \App\Facades\ObzoraConfig::get('alerts.bgp.whitelist')))
        ...
        // --- Update rrd data ---
        $peer_rrd_name = \ObzoraNMS\Data\Store\Rrd::safeName('bgp-' . $peer['bgpPeerIdentifier']);
        ...
        if ($device['os_group'] == 'vrp' || $device['os_group'] == 'cisco' || $device['os'] == 'junos' || $device['os'] == 'aos7' || $device['os_group'] === 'arista' || $device['os'] == 'dell-os10' || $device['os'] == 'firebrick') {
            ...
            foreach ($peer_afis as $peer_afi) {
                ...
                $cbgp_rrd_name = \ObzoraNMS\Data\Store\Rrd::safeName('cbgp-' . $peer['bgpPeerIdentifier'] . ".$afi.$safi");
