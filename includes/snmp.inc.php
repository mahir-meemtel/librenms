...
function prep_snmp_setting($device, $setting)
{
    ...
    return ObzoraConfig::get("snmp.$setting");
...
function get_mib_dir($device)
{
    ...
    if (isset($device['os']) && file_exists(ObzoraConfig::get('mib_dir') . '/' . $device['os'])) {
        $dirs[] = ObzoraConfig::get('mib_dir') . '/' . $device['os'];
    }

    if (isset($device['os_group'])) {
        if (file_exists(ObzoraConfig::get('mib_dir') . '/' . $device['os_group'])) {
            $dirs[] = ObzoraConfig::get('mib_dir') . '/' . $device['os_group'];
        }

        if ($group_mibdir = ObzoraConfig::get("os_groups.{$device['os_group']}.mib_dir")) {
            if (is_array($group_mibdir)) {
                foreach ($group_mibdir as $k => $dir) {
                    $dirs[] = ObzoraConfig::get('mib_dir') . '/' . $dir;
                }
            }
        }
    }

    if (isset($device['os']) && ($os_mibdir = ObzoraConfig::get("os.{$device['os']}.mib_dir"))) {
        $dirs[] = ObzoraConfig::get('mib_dir') . '/' . $os_mibdir;
    }
 ...
 *
 * @param  string  $mibdir  should be the name of the directory within \App\Facades\ObzoraConfig::get('mib_dir')
...
{
    $snmpcmd = [ObzoraConfig::get('snmpget')];
...
function gen_snmpwalk_cmd($device, $oids, $options = null, $mib = null, $mibdir = null)
{
    ...
    if ($device['snmpver'] == 'v1'
        || (isset($device['os']) && (ObzoraConfig::getOsSetting($device['os'], 'snmp_bulk', true) == false
                || ! empty(array_intersect($oids, ObzoraConfig::getCombined($device['os'], 'oids.no_bulk', 'snmp.'))))) // skip for oids that do not work with bulk
    ) {
        $snmpcmd = [ObzoraConfig::get('snmpwalk')];
    } else {
        $snmpcmd = [ObzoraConfig::get('snmpbulkwalk')];
    ...
    if (! empty(array_intersect($oids, ObzoraConfig::getCombined($device['os'], 'oids.unordered', 'snmp.')))) {
        $snmpcmd[] = '-Cc';
    }
...
function gen_snmp_cmd($cmd, $device, $oids, $options = null, $mib = null, $mibdir = null)
{
    ...
    $pollertarget = \ObzoraNMS\Util\Rewrite::addIpv6Brackets(Device::pollerTarget($device));
...
function snmp_getnext($device, $oid, $options = null, $mib = null, $mibdir = null)
{
    ...
    $snmpcmd = [ObzoraConfig::get('snmpgetnext', 'snmpgetnext')];
...
function get_device_max_repeaters($device)
{
    ...
    return $attrib ?? ObzoraConfig::getOsSetting($device['os'], 'snmp.max_repeaters', ObzoraConfig::get('snmp.max_repeaters', false));
}
