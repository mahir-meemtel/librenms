<?php
$check_cmd = \App\Facades\ObzoraConfig::get('nagios_plugins') . '/check_icmp ' . $service['service_param'] . ' ' . ($service['service_ip'] ? $service['service_ip'] : $service['hostname']);
...
if (isset($rrd_filename)) {
    ...
    $check_graph['rta'] .= ' LINE1.25:DS0#' . \App\Facades\ObzoraConfig::get('graph_colours.mixed.0') . ":'" . str_pad(substr('Round Trip Average', 0, 15), 15) . "' ";
    ...
    $check_graph['rtmax'] .= ' LINE1.25:DS1#' . \App\Facades\ObzoraConfig::get('graph_colours.mixed.1') . ":'" . str_pad(substr('Round Trip Max', 0, 15), 15) . "' ";
    ...
    $check_graph['rtmin'] .= ' LINE1.25:DS2#' . \App\Facades\ObzoraConfig::get('graph_colours.mixed.2') . ":'" . str_pad(substr('Round Trip Min', 0, 15), 15) . "' ";
    ...
    $check_graph['pl'] .= ' AREA:DS0#' . \App\Facades\ObzoraConfig::get('graph_colours.mixed.2') . ":'" . str_pad(substr('Packet Loss (%)', 0, 15), 15) . "' ";
