...
function parse_modules($type, $options)
{
    ...
    if (! empty($options['m'])) {
        ...
        $modules = array_map(fn ($v) => false, ObzoraConfig::get("{$type}_modules", []));

        foreach (explode(',', $options['m']) as $module) {
            // parse submodules (only supported by some modules)
            if (Str::contains($module, '/')) {
                [$module, $submodule] = explode('/', $module, 2);
                $existing_submodules = ObzoraConfig::get("{$type}_submodules.$module", []);
                $existing_submodules[] = $submodule;
                ObzoraConfig::set("{$type}_submodules.$module", $existing_submodules);
            }
        ...
        ObzoraConfig::set("{$type}_modules", array_filter($modules));

        // display selected modules
        $modules = array_map(function ($module) use ($type) {
            $submodules = ObzoraConfig::get("{$type}_submodules.$module");

            return $module . ($submodules ? '(' . implode(',', $submodules) . ')' : '');
        }, array_keys(ObzoraConfig::get("{$type}_modules", [])));
...
{
    $file = ObzoraConfig::get('log_file');
    $fd = fopen($file, 'a');
...
// FIXME port to ObzoraNMS\Util\IPv6 class
...
function is_port_valid($port, $device)
{
    ...
    if (empty($port['ifDescr'])) {
        ...
        if (! ObzoraConfig::getOsSetting($device['os'], 'empty_ifdescr', ObzoraConfig::get('empty_ifdescr', false))) {
    ...
    if (Str::contains($ifDescr, ObzoraConfig::getOsSetting($device['os'], 'good_if', ObzoraConfig::get('good_if')), ignoreCase: true)) {
        return true;
    }

    foreach (ObzoraConfig::getCombined($device['os'], 'bad_if') as $bi) {
    ...
    foreach (ObzoraConfig::getCombined($device['os'], 'bad_if_regexp') as $bir) {
    ...
    foreach (ObzoraConfig::getCombined($device['os'], 'bad_ifname_regexp') as $bnr) {
    ...
    foreach (ObzoraConfig::getCombined($device['os'], 'bad_ifalias_regexp') as $bar) {
    ...
    foreach (ObzoraConfig::getCombined($device['os'], 'bad_iftype') as $bt) {
    ...
    foreach (ObzoraConfig::getCombined($device['os'], 'bad_ifoperstatus') as $bos) {
...
function host_exists(string $hostname, ?string $sysName = null): bool
{
    return Device::where('hostname', $hostname)
        ->when(! empty($sysName), function ($query) use ($sysName) {
            $query->when(! ObzoraConfig::get('allow_duplicate_sysName'), fn ($q) => $q->orWhere('sysName', $sysName))
                  ->when(! empty(ObzoraConfig::get('mydomain')), fn ($q) => $q->orWhere('sysName', rtrim($sysName, '.') . '.' . ObzoraConfig::get('mydomain')));
        })->exists();
}
...
function hytera_h2f($number, $nd)
{
    if (strlen(str_replace(' ', '', $number)) == 4) {
        $number = \ObzoraNMS\Util\StringHelpers::asciiToHex($number, ' ');
    }
    $r = '';
    $y = explode(' ', $number);
    foreach ($y as $z) {
...
{
    if (ObzoraConfig::get('peeringdb.enabled') === true) {
        ...
        if ($cached == 0) {
            ...
            foreach (dbFetchRows('SELECT `bgpLocalAs` FROM `devices` WHERE `disabled` = 0 AND `ignore` = 0 AND `bgpLocalAs` > 0 AND (`bgpLocalAs` < 64512 OR `bgpLocalAs` > 65535) AND `bgpLocalAs` < 4200000000 GROUP BY `bgpLocalAs`') as $as) {
                $asn = $as['bgpLocalAs'];
                $get = \ObzoraNMS\Util\Http::client()->get($peeringdb_url . '/net?depth=2&asn=' . $asn);
                $json_data = $get->body();
                ...
                foreach ($ixs ?? [] as $ix) {
                    ...
                    $ix_keep[] = $pdb_ix_id;
                    $get_ix = \ObzoraNMS\Util\Http::client()->get("$peeringdb_url/netixlan?ix_id=$ixid");
                    $ix_json = $get_ix->body();
                    $ix_data = json_decode($ix_json);
                    $peers = $ix_data->{'data'};
                    foreach ($peers ?? [] as $index => $peer) {
                        $peer_name = \ObzoraNMS\Util\AutonomousSystem::get($peer->{'asn'})->name();
...
function get_device_oid_limit($device)
{
    ...
    // then os
    $os_max = ObzoraConfig::getOsSetting($device['os'], 'snmp_max_oid', 0);
    if ($os_max > 0) {
        return $os_max;
    }

    // then global
    $global_max = ObzoraConfig::get('snmp.max_oid', 10);
...
function lock_and_purge($table, $sql)
{
    ...
    if ($lock->get()) {
        $purge_days = ObzoraConfig::get($purge_name);
...
function lock_and_purge_query($table, $sql, $msg)
{
    ...
    $purge_duration = ObzoraConfig::get($purge_name);
...
{
    foreach (ObzoraConfig::getCombined($device['os'], 'bad_disk_regexp') as $bir) {
