...
function cleanPort($interface, $device = null)
{
    ...
    if (isset($device['os'])) {
        ...
        if (ObzoraConfig::get("os.$os.ifname")) {
            $interface['label'] = $interface['ifName'];

            if ($interface['ifName'] == '') {
                $interface['label'] = $interface['ifDescr'];
            }
        } elseif (ObzoraConfig::get("os.$os.ifalias")) {
            $interface['label'] = $interface['ifAlias'];
        } elseif (ObzoraConfig::get("os.$os.ifindex")) {
    ...
    if (is_array(ObzoraConfig::get('rewrite_if'))) {
        foreach (ObzoraConfig::get('rewrite_if') as $src => $val) {
    ...
    if (is_array(ObzoraConfig::get('rewrite_if_regexp'))) {
        foreach (ObzoraConfig::get('rewrite_if_regexp') as $reg => $val) {
...
{
    return \ObzoraNMS\Util\Rewrite::dslLineType($adslLineType);
}
