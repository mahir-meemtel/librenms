...
{
    $hosts = [];
    foreach (ObzoraConfig::get('datadirs') as $datadir) {
...
{
    if (! ObzoraConfig::get('collectd_sock')) {
        return false;
    }
    ...
    $u_errno = 0;
    $u_errmsg = '';
    if ($socket = @fsockopen(ObzoraConfig::get('collectd_sock'), 0, $u_errno, $u_errmsg)) {
...
function rrd_get_color($code, $line = true)
{
    ...
    if (! ObzoraConfig::has("rrd_colors.$name")) {
        $c_f = new CollectdColor('random');
        $c_h = new CollectdColor($c_f);
        $c_h->fade();
        ObzoraConfig::set("rrd_colors.f_$code", $c_f->toString());
        ObzoraConfig::set("rrd_colors.h_$code", $c_h->toString());
    }

    return ObzoraConfig::get("rrd_colors.$name");
}//end rrd_get_color()
...
{
    $timespan_def = null;
    $timespans = ObzoraConfig::get('timespan');
    ...
    // $rrd_cmd = array(RRDTOOL, 'graph', '-', '-E', '-a', 'PNG', '-w', ObzoraConfig::get('rrd_width'), '-h', ObzoraConfig::get('rrd_height'), '-t', $rrdfile);
    $rrd_cmd = [
        RRDTOOL,
        'graph',
        '-',
        ...
        'PNG',
        '-w',
        ObzoraConfig::get('rrd_width'),
        '-h',
        ObzoraConfig::get('rrd_height'),
    ];
    if (ObzoraConfig::get('rrd_width') <= '300') {
        $small_opts = [
            '--font',
            'LEGEND:7:mono',
            '--font',
    ...
    $rrd_cmd = array_merge($rrd_cmd, ObzoraConfig::get('rrd_opts_array'), $opts['rrd_opts'], $graph);
...
function collectd_draw_generic($timespan, $host, $plugin, $type, $pinst = null, $tinst = null)
{
    ...
    // $rrd_cmd  = array(RRDTOOL, 'graph', '-', '-E', '-a', 'PNG', '-w', ObzoraConfig::get('rrd_width'), '-h', ObzoraConfig::get('rrd_height'), '-t', $rrd_file);
    $rrd_cmd = [
        RRDTOOL,
        'graph',
        '-',
    ...
    $rrd_cmd = array_merge($rrd_cmd, ObzoraConfig::get('rrd_opts_array'));
...
function collectd_draw_meta_stack(&$opts, &$sources)
{
    ...
    // $cmd = array(RRDTOOL, 'graph', '-', '-E', '-a', 'PNG', '-w', ObzoraConfig::get('rrd_width'), '-h', ObzoraConfig::get('rrd_height'),
    // '-t', $opts['title']);
    $cmd = [
        RRDTOOL,
        'graph',
    ...
    $cmd = array_merge($cmd, ObzoraConfig::get('rrd_opts_array'), $opts['rrd_opts']);
    $max_inst_name = 0;
...
function collectd_draw_meta_line(&$opts, &$sources)
{
    ...
    // $cmd = array(RRDTOOL, 'graph', '-', '-E', '-a', 'PNG', '-w', ObzoraConfig::get('rrd_width'), '-h', ObzoraConfig::get('rrd_height'), '-t', $opts['title']);
    // $cmd = array_merge($cmd, ObzoraConfig::get('rrd_opts_array'), $opts['rrd_opts']);
    $cmd = [
        RRDTOOL,
        'graph',
