...
use ObzoraNMS\Util\Rewrite;
use ObzoraNMS\Util\Url;

function toner2colour($descr, $percent)
{
    $colour = \ObzoraNMS\Util\Color::percentage(100 - $percent, null);
...
{
    return \ObzoraNMS\Util\Html::percentageBar($width, $height, $percent, $left_text, $right_text, null, null, [
...
function generate_port_link($port, $text = null, $type = null, $overlib = 1, $single_graph = 0)
{
    ...
    $content = '<div class=list-large>' . $port['hostname'] . ' - ' . Rewrite::normalizeIfName(addslashes(\ObzoraNMS\Util\Clean::html($port['label'], []))) . '</div>';
    $content .= addslashes(\ObzoraNMS\Util\Clean::html($port['ifAlias'], [])) . '<br />';
    ...
    $graph_array['from'] = ObzoraConfig::get('time.day');
    $graph_array['id'] = $port['port_id'];
    $content .= Url::graphTag($graph_array);
    if ($single_graph == 0) {
        $graph_array['from'] = ObzoraConfig::get('time.week');
        $content .= Url::graphTag($graph_array);
        $graph_array['from'] = ObzoraConfig::get('time.month');
        $content .= Url::graphTag($graph_array);
        $graph_array['from'] = ObzoraConfig::get('time.year');
        $content .= Url::graphTag($graph_array);
    }
...
function generate_sensor_link($args, $text = null, $type = null)
{
    ...
    $graph_array = [
        ...
        'height' => '100',
        'width' => '340',
        'to' => ObzoraConfig::get('time.now'),
        'from' => ObzoraConfig::get('time.day'),
        'id' => $args['sensor_id'],
    ];
    $content .= Url::graphTag($graph_array);
    ...
    $url = Url::generate(['page' => 'graphs', 'id' => $args['sensor_id'], 'type' => $args['graph_type'], 'from' => \App\Facades\ObzoraConfig::get('time.day')], []);
...
function graph_error($text, $short = null, $color = [128, 0, 0])
{
    ...
    echo \ObzoraNMS\Util\Graph::error($text, $short, 300, null, $color);
}
...
function generate_ap_link($args, $text = null, $type = null)
{
    ...
    if ($args['ifAlias']) {
        $content .= \ObzoraNMS\Util\Clean::html($args['ifAlias'], []) . '<br />';
    }
...
function get_ports_from_type($given_types)
{
    ...
    foreach ($given_types as $type) {
        if (ObzoraConfig::has($type . '_descr')) {
            $type_descr = ObzoraConfig::get($type . '_descr');
...
function search_oxidized_config($search_in_conf_textbox)
{
    ...
    $oxidized_search_url = ObzoraConfig::get('oxidized.url') . '/nodes/conf_search?format=json';
    ...
    // Look up Oxidized node names to ObzoraNMS devices for a link
    foreach ($nodes as &$n) {
...
function get_oxidized_nodes_list()
{
    ...
    $data = json_decode(file_get_contents(ObzoraConfig::get('oxidized.url') . '/nodes?format=json', false, $context), true);
...
{
    if (ObzoraConfig::get('webui.graph_stacked') == true || $force_stack == true) {
...
function time_to_nfsen_subpath($time)
{
    ...
    $layout = ObzoraConfig::get('nfsen_subdirlayout');
...
{
    $nfsen_hostname = str_replace('.', ObzoraConfig::get('nfsen_split_char'), $hostname);

    if (! is_null(ObzoraConfig::get('nfsen_suffix'))) {
        $nfsen_hostname = str_replace(ObzoraConfig::get('nfsen_suffix'), '', $nfsen_hostname);
    }
...
function nfsen_live_dir($hostname)
{
    ...
    foreach (ObzoraConfig::get('nfsen_base') as $base_dir) {
