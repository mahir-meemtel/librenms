...
foreach ($ports as $port) {
    if (port_permitted($port['port_id'], $port['device_id'])) {
        ...
        $type = \ObzoraNMS\Util\Rewrite::normalizeIfType($port['ifType']);
        $port['in_rate'] = \ObzoraNMS\Util\Number::formatSi($port['ifInOctets_rate'] * 8, 2, 0, 'bps');
        $port['out_rate'] = \ObzoraNMS\Util\Number::formatSi($port['ifOutOctets_rate'] * 8, 2, 0, 'bps');
        $port = cleanPort($port, $device);
        $csv[] = [
            format_hostname($port),
            \ObzoraNMS\Util\Rewrite::normalizeIfName($port['label']),
            $speed,
            $port['in_rate'],
            $port['out_rate'],
            $type,
            \ObzoraNMS\Util\Clean::html($port['ifAlias'], []),
        ];
    }
}
