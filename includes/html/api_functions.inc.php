...
function api_get_graph(Request $request, array $additional = [])
{
    ...
    } catch (\ObzoraNMS\Exceptions\RrdGraphException $e) {
...
function maintenance_device(Illuminate\Http\Request $request)
{
    ...
    $behavior = MaintenanceBehavior::tryFrom((int) ($data['behavior'] ?? -1))
        ?? ObzoraConfig::get('alert.scheduled_maintenance_default_behavior');
...
function get_graphs(Illuminate\Http\Request $request)
{
    ...
    return check_device_permission($device_id, function ($device_id) {
        ...
        foreach (dbFetchRows('SELECT * FROM device_graphs WHERE device_id = ? ORDER BY graph', [$device_id]) as $graph) {
            $desc = ObzoraConfig::get("graph_types.device.{$graph['graph']}.descr");
            $graphs[] = [
                'desc' => $desc,
...
/**
 * @throws \ObzoraNMS\Exceptions\ApiException
 */
...
function ack_alert(Illuminate\Http\Request $request)
{
    ...
    $note .= date(ObzoraConfig::get('dateformat.long')) . ' - Ack (' . Auth::user()->username . ") {$data['note']}";
...
function list_oxidized(Illuminate\Http\Request $request)
{
    ...
    $devices = Device::query()
            ->with('attribs')
             ...
             ->whereNotIn('type', ObzoraConfig::get('oxidized.ignore_types', []))
             ->whereNotIn('os', ObzoraConfig::get('oxidized.ignore_os', []))
    ...
    foreach ($devices as $device) {
        ...
        if (ObzoraConfig::get('oxidized.group_support') === true && ! empty(ObzoraConfig::get('oxidized.default_group'))) {
            $output['group'] = ObzoraConfig::get('oxidized.default_group');
        }

        foreach (ObzoraConfig::get('oxidized.maps') as $maps_column => $maps) {
            // Based on Oxidized group support we can apply groups by setting group_support to true
            if ($maps_column == 'group' && ObzoraConfig::get('oxidized.group_support', true) !== true) {
                continue;
            }
        ...
        if (in_array($output['group'], ObzoraConfig::get('oxidized.ignore_groups'))) {
            continue;
        }
...
function validate_column_list(?string $columns, string $table, array $default = []): array
{
    ...
    static $schema;
    if (is_null($schema)) {
        $schema = new \ObzoraNMS\DB\Schema();
    }
...
/**
 * recieve syslog messages via json https://github.com/obzora/obzora/pull/14424
 */
...
/**
 * Display Obzora Instance Info
 */
function server_info()
{
    $version = \ObzoraNMS\Util\Version::get();
