...
if (session('preferences.timezone')) {
    $sql = "SELECT E.id AS alert_log_id, E.details AS alert_log_details, R.severity, D.device_id,name AS alert,rule_id,state,time_logged,DATE_FORMAT(IFNULL(CONVERT_TZ(time_logged, @@global.time_zone, ?),time_logged), '" . \App\Facades\ObzoraConfig::get('dateformat.mysql.compact') . "') as humandate,details $sql";
    $param = array_merge([session('preferences.timezone')], $param);
} else {
    $sql = "SELECT E.id AS alert_log_id, E.details AS alert_log_details, R.severity, D.device_id,name AS alert,rule_id,state,time_logged,DATE_FORMAT(time_logged, '" . \App\Facades\ObzoraConfig::get('dateformat.mysql.compact') . "') as humandate,details $sql";
}
