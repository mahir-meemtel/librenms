...
foreach (dbFetchRows($sql, $param) as $asn) {
    $astext = \ObzoraNMS\Util\AutonomousSystem::get($asn['bgpLocalAs'])->name();
    $response[] = [
        'bgpLocalAs' => $asn['bgpLocalAs'],
        'asname' => $astext,
        'action' => "<a class='btn btn-sm btn-primary' href='" . \ObzoraNMS\Util\Url::generate(['page' => 'peering', 'section' => 'ix-list', 'bgpLocalAs' => $asn['bgpLocalAs']]) . "' role='button'>Show connected IXes</a>",
    ];
}
