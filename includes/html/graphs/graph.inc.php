...
use ObzoraNMS\Data\Graphing\GraphParameters;
...
try {
    ...
    require ObzoraConfig::get('install_dir') . "/includes/html/graphs/$type/auth.inc.php";

    if ($auth && is_customoid_graph($type, $subtype)) {
        $unit = $vars['unit'];
        include ObzoraConfig::get('install_dir') . '/includes/html/graphs/customoid/customoid.inc.php';
    } elseif ($auth && is_file(ObzoraConfig::get('install_dir') . "/includes/html/graphs/$type/$subtype.inc.php")) {
        include ObzoraConfig::get('install_dir') . "/includes/html/graphs/$type/$subtype.inc.php";
    } elseif ($auth && is_file(ObzoraConfig::get('install_dir') . "/includes/html/graphs/$type/generic.inc.php")) {
        include ObzoraConfig::get('install_dir') . "/includes/html/graphs/$type/generic.inc.php";
    } else {
    ...
    if (isset($vars['device']) && empty($device['device_id'])) {
        throw new \ObzoraNMS\Exceptions\RrdGraphException('Device not found');
    }
    ...
    if (! empty($command_only)) {
        ...
        echo escapeshellcmd('rrdtool ' . Rrd::buildCommand('graph', ObzoraConfig::get('temp_dir') . '/' . Str::random(), $rrd_options));
        echo '</pre>';
        try {
