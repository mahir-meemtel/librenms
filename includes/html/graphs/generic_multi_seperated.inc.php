...
$units_descr = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($units_descr ?? '', $rrddescr_len + 5);
...
foreach ($rrd_list ?? [] as $rrd) {
    if (! ObzoraConfig::get("graph_colours.$colours_in.$iter") || ! ObzoraConfig::get("graph_colours.$colours_out.$iter")) {
        $iter = 0;
    }

    $colour_in = $rrd['colour_area_in'] ?? ObzoraConfig::get("graph_colours.$colours_in.$iter");
    $colour_out = $rrd['colour_area_out'] ?? ObzoraConfig::get("graph_colours.$colours_out.$iter");
    ...
    if (! $nodetails) {
        $descr = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($rrd['descr'], $rrddescr_len) . '  In';
        $descr_out = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr('', $rrddescr_len) . ' Out';
    }
...
if ($previous) {
    ...
    $rrd_options .= ' VDEF:dpercentile_outXperc=dpercentile_outXn,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
...
if (! $nototal && ! empty($rrd_list)) {
    ...
    $rrd_options .= " HRULE:999999999999999#FFFFFF:'" . \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr('Total', $rrddescr_len) . "  In'";
    ...
    $rrd_options .= " HRULE:999999999999990#FFFFFF:'" . \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr('', $rrddescr_len) . " Out'";
    ...
    $rrd_options .= " HRULE:999999999999990#FFFFFF:'" . \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr('', $rrddescr_len) . " Agg'";
