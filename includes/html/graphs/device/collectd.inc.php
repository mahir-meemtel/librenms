...
function error($code, $code_msg, $title, $msg)
{
    ...
    $w = (ObzoraConfig::get('rrd_width') + 81);
    $h = (ObzoraConfig::get('rrd_height') + 79);
    ...
    if (function_exists('imagettfbbox') && is_file(ObzoraConfig::get('error_font'))) {
        // Detailled error message
        $errorfont = ObzoraConfig::get('error_font');
...
$timespan = read_var('timespan', $_GET, ObzoraConfig::get('timespan.0.name'));
$timespan_ok = false;
foreach (ObzoraConfig::get('timespan') as &$ts) {
    if ($ts['name'] == $timespan) {
        $timespan_ok = true;
    }
}
...
if ($width <= '300') {
    $rrd_cmd .= ' --font LEGEND:7:' . ObzoraConfig::get('mono_font') . ' --font AXIS:6:' . ObzoraConfig::get('mono_font') . ' ';
} else {
    $rrd_cmd .= ' --font LEGEND:8:' . ObzoraConfig::get('mono_font') . ' --font AXIS:7:' . ObzoraConfig::get('mono_font') . ' ';
}
