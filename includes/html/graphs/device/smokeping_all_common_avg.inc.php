...
$i = 0;
$pings = ObzoraConfig::get('smokeping.pings');
$iter = 0;
$colourset = 'mixed';
...
foreach ($smokeping_files[$direction][$device['hostname']] as $source => $filename) {
    if (! ObzoraConfig::has("graph_colours.$colourset.$iter")) {
        $iter = 0;
    }

    $colour = ObzoraConfig::get("graph_colours.$colourset.$iter");
    $iter++;

    // FIXME: $descr unused? -- PDG 2015-11-14
    $descr = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($source, $descr_len);
...
$descr = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr('Average', $descr_len);
