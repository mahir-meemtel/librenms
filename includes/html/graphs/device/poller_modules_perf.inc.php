...
/*
 * ObzoraNMS per-module poller performance
 *
 * Copyright (c) 2016 Mike Rostermund <mike@kollegienet.dk>
 * Copyright (c) 2016 Paul D. Gear <paul@obzora.meemtel.com>
 *
...
$modules = ObzoraConfig::get('poller_modules');
ksort($modules);
...
foreach ($modules as $module => $module_status) {
    ...
    if ($attribs['poll_' . $module] || ($module_status && ! isset($attribs['poll_' . $module])) ||
        (ObzoraConfig::getOsSetting($device->os, 'poller_modules.' . $module) && ! isset($attribs['poll_' . $module]))) {
