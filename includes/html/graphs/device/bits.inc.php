...
if (empty($ports)) {
    throw new \ObzoraNMS\Exceptions\RrdGraphException('No Ports');
}

$i = 0;
foreach ($ports as $port) {
    $ignore = 0;
    if (is_array(\App\Facades\ObzoraConfig::get('device_traffic_iftype'))) {
        foreach (\App\Facades\ObzoraConfig::get('device_traffic_iftype') as $iftype) {
    ...
    if (is_array(\App\Facades\ObzoraConfig::get('device_traffic_descr'))) {
        foreach (\App\Facades\ObzoraConfig::get('device_traffic_descr') as $ifdescr) {
    ...
    if ($ignore != 1 && Rrd::checkRrdExists($rrd_filename)) {
        ...
        $rrd_list[$i]['descr'] = \ObzoraNMS\Util\Rewrite::shortenIfName($port['label']);
        $rrd_list[$i]['descr_in'] = $port['label'];
        $rrd_list[$i]['descr_out'] = \ObzoraNMS\Util\Clean::html($port['ifAlias'], []);
