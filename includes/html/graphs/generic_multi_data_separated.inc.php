...
foreach ($rrd_list as $rrd) {
    if (! \App\Facades\ObzoraConfig::get("graph_colours.$colours_in.$iter") || ! \App\Facades\ObzoraConfig::get("graph_colours.$colours_out.$iter")) {
        $iter = 0;
    }

    $colour_in = \App\Facades\ObzoraConfig::get("graph_colours.$colours_in.$iter");
    $colour_out = \App\Facades\ObzoraConfig::get("graph_colours.$colours_out.$iter");

    if (isset($rrd['descr_in'])) {
        $descr = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($rrd['descr_in'], $descr_len) . '  In';
    } else {
        $descr = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($rrd['descr'], $descr_len) . '  In';
    }

    $descr_out = \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($rrd['descr_out'], $descr_len) . ' Out';
...
if (! $nototal) {
    ...
    $rrd_options .= ' VDEF:percentile_in=inbits,' . \App\Facades\ObzoraConfig::get('percentile_value') . ',PERCENT';
    $rrd_options .= ' VDEF:percentile_out=outbits,' . \App\Facades\ObzoraConfig::get('percentile_value') . ',PERCENT';
    $rrd_options .= ' CDEF:dpercentile_outn=doutbits,' . $stacked['stacked'] . ',* VDEF:dpercentile_outnp=dpercentile_outn,' . \App\Facades\ObzoraConfig::get('percentile_value') . ',PERCENT CDEF:dpercentile_outnpn=doutbits,doutbits,-,dpercentile_outnp,' . $stacked['stacked'] . ',*,+ VDEF:dpercentile_out=dpercentile_outnpn,FIRST';
