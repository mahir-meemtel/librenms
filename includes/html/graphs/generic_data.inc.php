...
$stacked = generate_stacked_graphs(! empty($port['ifSpeed']) && ($vars['port_speed_zoom'] ?? ObzoraConfig::get('graphs.port_speed_zoom')));
...
if ($previous) {
    ...
    $rrd_options .= ' VDEF:dpercentile_outnpX=dpercentile_outnX,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
...
if (ObzoraConfig::get('rrdgraph_real_percentile')) {
    $rrd_options .= ' CDEF:highbits=inoctets,outoctets,MAX,8,*';
    $rrd_options .= ' VDEF:percentilehigh=highbits,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    if ($previous) {
        $rrd_options .= ' CDEF:highbitsX=inoctetsX,outoctetsX,MAX,8,*';
        $rrd_options .= ' VDEF:percentilehighX=highbitsX,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    }
}
...
$rrd_options .= ' VDEF:dpercentile_outnp=dpercentile_outn,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
...
if ($format == 'octets' || $format == 'bytes') {
    $rrd_options .= ' VDEF:percentile_in=inoctets,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    $rrd_options .= ' VDEF:percentile_out=outoctets,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    if ($previous) {
        $rrd_options .= ' VDEF:percentile_inX=inoctetsX,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
        $rrd_options .= ' VDEF:percentile_outX=outoctetsX,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    }
    $units = 'Bps';
    $format = 'octets';
} else {
    $rrd_options .= ' VDEF:percentile_in=inbits,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    $rrd_options .= ' VDEF:percentile_out=outbits,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    if ($previous) {
        $rrd_options .= ' VDEF:percentile_inX=inbitsX,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
        $rrd_options .= ' VDEF:percentile_outX=outbitsX,' . ObzoraConfig::get('percentile_value') . ',PERCENT';
    }
    $units = 'bps';
    $format = 'bits';
}

$rrd_options .= " COMMENT:'bps      Now       Ave      Max      " . ObzoraConfig::get('percentile_value') . "th %\\n'";
...
if (! empty($port['ifSpeed'])) {
    $speed_line_type = ($vars['port_speed_zoom'] ?? ObzoraConfig::get('graphs.port_speed_zoom')) ? 'LINE2' : 'HRULE';
