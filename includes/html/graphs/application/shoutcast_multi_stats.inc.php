...
foreach ($rrd_list as $rrd) {
    ...
    $descr = (isset($rrd['descr']) ? \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($rrd['descr'], $desc_len + $strlen) : 'Unknown');
    for ($z = 0; $z < $strlen; $z++) {
        $descr .= ' ';
    }
    ...
    $colour = \App\Facades\ObzoraConfig::get("graph_colours.$colours.$x");
    ...
    $x = (($x < count(\App\Facades\ObzoraConfig::get("graph_colours.$colours")) - 1) ? $x + 1 : 0);
    // $x++;
}//end foreach

if (! $nototal) {
    $strlen = ((strlen($total_text) < $descr_len) ? ($descr_len - strlen($total_text)) : '0');
    $descr = (isset($total_text) ? \ObzoraNMS\Data\Store\Rrd::fixedSafeDescr($total_text, $desc_len + $strlen) : 'Total');
    $colour = \App\Facades\ObzoraConfig::get("graph_colours.$colours.$x");
    for ($z = 0; $z < $strlen; $z++) {
        $descr .= ' ';
    }
