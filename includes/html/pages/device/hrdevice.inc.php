...
foreach (dbFetchRows('SELECT * FROM `hrDevice` WHERE `device_id` = ? ORDER BY `hrDeviceIndex`', [$device['device_id']]) as $hrdevice) {
    ...
    if ($hrdevice['hrDeviceType'] == 'hrDeviceProcessor') {
        ...
        $proc_popup .= "</div><img src=\'graph.php?id=" . $proc_id . '&amp;type=processor_usage&amp;from=' . \App\Facades\ObzoraConfig::get('time.month') . '&amp;to=' . \App\Facades\ObzoraConfig::get('time.now') . "&amp;width=400&amp;height=125\'>";
        $proc_popup .= "', RIGHT" . \App\Facades\ObzoraConfig::get('overlib_defaults') . ');" onmouseout="return nd();"';
        ...
        $mini_graph = \ObzoraNMS\Util\Url::overlibLink($proc_url, \ObzoraNMS\Util\Url::lazyGraphTag($graph_array), \ObzoraNMS\Util\Url::graphTag($graph_array_zoom));
        ...
        if ($interface['ifIndex']) {
            ...
            $mini_graph = \ObzoraNMS\Util\Url::overlibLink(generate_port_url($interface), \ObzoraNMS\Util\Url::lazyGraphTag($graph_array), \ObzoraNMS\Util\Url::graphTag($graph_array_zoom));
