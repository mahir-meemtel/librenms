    ...
    if (isset($app_data['oslvm_data'][$vars['oslvm']]['path'][0])) {
        ...
        foreach ($app_data['oslvm_data'][$vars['oslvm']]['path'] as $index => $path) {
            ...
            if (isset($storage_info)) {
                ...
                $mount_path = ObzoraNMS\Util\Url::overlibLink($path_link, $mount_path, $path_overlib_content);
                $mount_path_usage = round($storage_info['storage_perc']) . '% ';
                $mount_path_usage_graph = ObzoraNMS\Util\Url::overlibLink($path_link, $path_minigraph, $path_overlib_content);
            }
        }
        $table_info['rows'][] = [
            [
...
if ($app_data['has']['linux_mem_stats']) {
    if (ObzoraConfig::get('apps.oslv_monitor.linux_pg_memory_stats')) {
        $graphs[] = [
            'type' => 'cgroups_pg',
            'description' => 'Linux Pg Memory Stats',
        ];
    }
    if (ObzoraConfig::get('apps.oslv_monitor.misc_linux_memory_stats')) {
        $graphs[] = [
            'type' => 'cgroups_mem_misc',
            'description' => 'Misc Linux Memory Stats',
        ];
    }
    if (ObzoraConfig::get('apps.oslv_monitor.zswap_size')) {
        $graphs[] = [
            'type' => 'cgroups_zswap',
            'description' => 'Zswap Size',
        ];
    }
    if (ObzoraConfig::get('apps.oslv_monitor.zswap_activity')) {
        $graphs[] = [
            'type' => 'cgroups_zswap_activity',
            'description' => 'Zswap Activity',
        ];
    }
    if (ObzoraConfig::get('apps.oslv_monitor.workingset_stats')) {
        $graphs[] = [
            'type' => 'cgroups_workingset',
            'description' => 'Workingset Stats',
        ];
    }
    if (ObzoraConfig::get('apps.oslv_monitor.thp_activity')) {
        $graphs[] = [
