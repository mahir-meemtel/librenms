        ...
        if (isset($data['path'])) {
            if (! isset($path_cache[$data['host']][$data['path']])) {
                ...
                if (isset($storage_info)) {
                    ...
                    $path_graph_array['to'] = \App\Facades\ObzoraConfig::get('time.now');
                    $path_graph_array['id'] = $storage_info['storage_id'];
                    $path_graph_array['type'] = 'storage_usage';
                    $path_graph_array['from'] = \App\Facades\ObzoraConfig::get('time.day');
                    ...
                    $path_link = \ObzoraNMS\Util\Url::generate($path_link_array);
                    ...
                    $path_minigraph = \ObzoraNMS\Util\Url::lazyGraphTag($path_graph_array);

                    $new_path['data'] = \ObzoraNMS\Util\Url::overlibLink($path_link, $data['path'], $path_overlib_content) .
                        ' (' . round($storage_info['storage_perc']) . '%)' .
                        \ObzoraNMS\Util\Url::overlibLink($path_link, $path_minigraph, $path_overlib_content);
                    $new_path['raw'] = true;
        ...
        if (isset($data['rpath'])) {
            if (! isset($rpath_cache[$data['host']][$data['rpath']]) && isset($host_cache_device_id[$data['host']])) {
                ...
                if (isset($storage_info)) {
                    ...
                    $rpath_graph_array['to'] = \App\Facades\ObzoraConfig::get('time.now');
                    $rpath_graph_array['id'] = $storage_info['storage_id'];
                    $rpath_graph_array['type'] = 'storage_usage';
                    $rpath_graph_array['from'] = \App\Facades\ObzoraConfig::get('time.day');
                    ...
                    $rpath_link = \ObzoraNMS\Util\Url::generate($rpath_link_array);
                    ...
                    $rpath_minigraph = \ObzoraNMS\Util\Url::lazyGraphTag($rpath_graph_array);

                    $new_rpath['data'] = \ObzoraNMS\Util\Url::overlibLink($rpath_link, $storage_info['storage_descr'], $rpath_overlib_content) .
                        ' (' . round($storage_info['storage_perc']) . '%)' .
                        \ObzoraNMS\Util\Url::overlibLink($rpath_link, $rpath_minigraph, $rpath_overlib_content);
                    $new_rpath['raw'] = true;
