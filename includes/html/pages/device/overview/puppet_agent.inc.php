...
if ($app) {
    ...
    $link_last_run = \ObzoraNMS\Util\Url::generate($link_array_last_run);

    $overlib_content_last_run = generate_overlib_content($graph_array_last_run, $device['hostname'] . ' - ' . $title_last_run);
    $overlib_link_last_run = \ObzoraNMS\Util\Url::overlibLink($link_last_run, 'Last run', $overlib_content_last_run);
    ...
    $link_runtime = \ObzoraNMS\Util\Url::generate($link_array_runtime);

    $overlib_content_runtime = generate_overlib_content($graph_array_runtime, $device['hostname'] . ' - ' . $title_runtime);
    $overlib_link_runtime = \ObzoraNMS\Util\Url::overlibLink($link_runtime, $title_runtime, $overlib_content_runtime);
    ...
    $link_resources = \ObzoraNMS\Util\Url::generate($link_array_resources);

    $overlib_content_resources = generate_overlib_content($graph_array_resources, $device['hostname'] . ' - ' . $title_resources);
    $overlib_link_resources = \ObzoraNMS\Util\Url::overlibLink($link_resources, $title_resources, $overlib_content_resources);
    ...
    $link_events = \ObzoraNMS\Util\Url::generate($link_array_events);

    $overlib_content_events = generate_overlib_content($graph_array_events, $device['hostname'] . ' - ' . $title_events);
    $overlib_link_events = \ObzoraNMS\Util\Url::overlibLink($link_events, $title_events, $overlib_content_events);
