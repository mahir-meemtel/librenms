...
if ($sensors->isNotEmpty()) {
    ...
    echo '<a href="device/device=' . $device['device_id'] . '/tab=health/metric=' . strtolower($sensor_type) . '/"><i class="fa ' . $sensor_fa_icon . ' fa-lg icon-theme" aria-hidden="true"></i><strong> ' . \ObzoraNMS\Util\StringHelpers::niceCase($sensor_type) . '</strong></a>';
    echo '      </div>
    ...
    foreach ($sensors as $sensor) {
        ...
        foreach (['day', 'week', 'month', 'year'] as $period) {
            $graph_array['from'] = \App\Facades\ObzoraConfig::get("time.$period");
            $overlib_content .= str_replace('"', "\'", \ObzoraNMS\Util\Url::graphTag($graph_array));
        }
        ...
        $graph_array['from'] = \App\Facades\ObzoraConfig::get('time.day');
        $sensor_minigraph = \ObzoraNMS\Util\Url::lazyGraphTag($graph_array);
        ...
        echo '<tr><td><div style="display: grid; grid-gap: 10px; grid-template-columns: 3fr 1fr 1fr;">
            <div>' . \ObzoraNMS\Util\Url::overlibLink($link, \ObzoraNMS\Util\Rewrite::shortenIfName($sensor->sensor_descr), $overlib_content, $sensor_class) . '</div>
            <div>' . \ObzoraNMS\Util\Url::overlibLink($link, $sensor_minigraph, $overlib_content, $sensor_class) . '</div>
            <div>' . \ObzoraNMS\Util\Url::overlibLink($link, $sensor_current, $overlib_content, $sensor_class) . '</div>
            </div></td></tr>';
    }//end foreach
