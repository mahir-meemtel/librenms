...
if ($mempools->isNotEmpty()) {
    ...
    $graph = \App\Http\Controllers\Device\Tabs\OverviewController::setGraphWidth([
        ...
        'type' => 'device_mempool',
        'from' => \App\Facades\ObzoraConfig::get('time.day'),
        'legend' => 'no',
        'popup_title' => DeviceCache::getPrimary()->hostname . ' - Memory Usage',
    ]);
    echo \ObzoraNMS\Util\Url::graphPopup($graph, \ObzoraNMS\Util\Url::lazyGraphTag($graph), $mempools_url);
    echo '  </td>
            </tr>';
    ...
    foreach ($mempools as $mempool) {
        ...
        $graph_array = [
            ...
            'width' => 210,
            'from' => \App\Facades\ObzoraConfig::get('time.day'),
            'to' => \App\Facades\ObzoraConfig::get('time.now'),
            'legend' => 'no',
        ];
        ...
        switch ($mempool->mempool_class) {
            case 'system':
                $percentageBar = Html::percentageBar(400, 20, $mempool->mempool_perc, "$used / $total ($mempool->mempool_perc%)", $free, $mempool->mempool_perc_warn, $available_used_all, null, 'memory');
                break;
            case 'virtual':
            case 'swap':
                $percentageBar = Html::percentageBar(400, 20, $mempool->mempool_perc, "$used / $total ($mempool->mempool_perc%)", $free, $mempool->mempool_perc_warn, null, null, 'memory');
                break;
            default:
                $percentageBar = Html::percentageBar(400, 20, $mempool->mempool_perc, "$used ($mempool->mempool_perc%)", '', $mempool->mempool_perc_warn, null, null, 'memory');
                break;
        }

        echo '<tr>
            <td class="col-md-4">' . \ObzoraNMS\Util\Url::overlibLink($link, $mempool->mempool_descr, $overlib_content) . '</td>
            <td class="col-md-4">' . \ObzoraNMS\Util\Url::overlibLink($link, $minigraph, $overlib_content) . '</td>
            <td class="col-md-4">' . \ObzoraNMS\Util\Url::overlibLink($link, $percentageBar, $overlib_content) . '
            </a></td>
            </tr>';
    }//end foreach
