...
foreach ($supplies as $type => $supply) {
    if (! empty($supply)) {
        ...
        foreach ($supply as $toner) {
            ...
            echo '<tr>
            <td class="col-md-4">' . \ObzoraNMS\Util\Url::overlibLink($link, $toner['supply_descr'], $overlib_content) . '</td>
            <td class="col-md-4">' . \ObzoraNMS\Util\Url::overlibLink($link, $minigraph, $overlib_content) . '</td>
            <td class="col-md-4">' . \ObzoraNMS\Util\Url::overlibLink($link, print_percentage_bar(200, 20, $percent, null, 'ffffff', $background['left'], $percent . '%', 'ffffff', $background['right']), $overlib_content) . '
           </a></td>
         </tr>';
        }//end foreach
