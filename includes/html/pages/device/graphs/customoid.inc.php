...
foreach (dbFetchRows('SELECT * FROM `customoids` WHERE `device_id` = ? ORDER BY `customoid_descr`', [$device['device_id']]) as $customoid) {
    ...
    $customoid_current = \ObzoraNMS\Util\Number::formatSi($customoid['customoid_current'], 2, 0, '') . $customoid_unit;
    $customoid_limit = \ObzoraNMS\Util\Number::formatSi($customoid['customoid_limit'], 2, 0, '') . $customoid_unit;
    $customoid_limit_low = \ObzoraNMS\Util\Number::formatSi($customoid['$customoid_limit_low'], 2, 0, '') . $customoid_unit;
