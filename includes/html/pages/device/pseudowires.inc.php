...
foreach (dbFetchRows('SELECT * FROM pseudowires AS P, ports AS I WHERE P.port_id = I.port_id AND I.device_id = ? ORDER BY I.ifDescr', [$device['device_id']]) as $pw_a) {
    ...
    if ($vars['view'] == 'minigraphs') {
        ...
        if ($pw_a) {
            $pw_a['width'] = '150';
            $pw_a['height'] = '30';
            $pw_a['from'] = \App\Facades\ObzoraConfig::get('time.day');
            $pw_a['to'] = \App\Facades\ObzoraConfig::get('time.now');
            $pw_a['bg'] = $bg;
            $types = [
                'bits',
                'upkts',
        ...
        if ($pw_b) {
            $pw_b['width'] = '150';
            $pw_b['height'] = '30';
            $pw_b['from'] = \App\Facades\ObzoraConfig::get('time.day');
            $pw_b['to'] = \App\Facades\ObzoraConfig::get('time.now');
            $pw_b['bg'] = $bg;
