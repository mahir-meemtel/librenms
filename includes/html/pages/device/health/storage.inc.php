...
foreach (dbFetchRows('SELECT * FROM `storage` WHERE device_id = ? ORDER BY storage_descr', [$device['device_id']]) as $drive) {
    ...
    $fs_popup .= "</div><img src=\'graph.php?id=" . $drive['storage_id'] . '&amp;type=' . $graph_type . '&amp;from=' . \App\Facades\ObzoraConfig::get('time.month') . '&amp;to=' . \App\Facades\ObzoraConfig::get('time.now') . "&amp;width=400&amp;height=125\'>";
