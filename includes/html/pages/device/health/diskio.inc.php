...
foreach (dbFetchRows('SELECT * FROM `ucd_diskio` WHERE device_id = ? ORDER BY diskio_descr', [$device['device_id']]) as $drive) {
    ...
    $graph_array_zoom['from'] = \App\Facades\ObzoraConfig::get('time.twoday');
    $graph_array_zoom['to'] = \App\Facades\ObzoraConfig::get('time.now');

    $overlib_link = \ObzoraNMS\Util\Url::overlibLink($fs_url, $drive['diskio_descr'], \ObzoraNMS\Util\Url::graphTag($graph_array_zoom));
