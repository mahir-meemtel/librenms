...
foreach (dbFetchRows('SELECT * FROM device_graphs WHERE device_id = ? ORDER BY graph', [$device['device_id']]) as $graph) {
    $section = \App\Facades\ObzoraConfig::get("graph_types.device.{$graph['graph']}.section");
    if ($section != '') {
        ...
        if ($graph_enable[$graph]) {
            if ($graph == 'customoid') {
                foreach (dbFetchRows('SELECT * FROM `customoids` WHERE `device_id` = ? ORDER BY `customoid_descr`', [$device['device_id']]) as $graph_entry) {
                    $graph_title = \App\Facades\ObzoraConfig::get("graph_types.device.$graph.descr") . ': ' . $graph_entry['customoid_descr'];
                ...
                $graph_title = \App\Facades\ObzoraConfig::get("graph_types.device.$graph.descr");
