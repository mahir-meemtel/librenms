...
echo '<form action = "' . \ObzoraNMS\Util\Url::generate($link_array, ['nfsen' => 'stats']) . '" iOCd = "FlowStats" method = "SUBMIT">';

echo 'Top N:
<select name = "topN" id = "topN" size = 1>
';

$option_default = $vars['topN'] ?? \App\Facades\ObzoraConfig::get('nfsen_top_default');

$option_int = 0;
foreach (\App\Facades\ObzoraConfig::get('nfsen_top_N') as $option) {
...
$option_keys = array_keys(\App\Facades\ObzoraConfig::get('nfsen_lasts'));
$options = \App\Facades\ObzoraConfig::get('nfsen_lasts');
...
$option_default = $vars['stattype'] ?? \App\Facades\ObzoraConfig::get('nfsen_stats_default');
...
$option_default = \App\Facades\ObzoraConfig::get('nfsen_order_default');
...
if (isset($vars['process'])) {
    ...
    if (isset($vars['lastN']) &&
         ...
         ($vars['lastN'] <= \App\Facades\ObzoraConfig::get('nfsen_last_max'))
    ) {
        $lastN = $vars['lastN'];
    }
    ...
    if (isset($vars['topN']) &&
         ...
         ($vars['topN'] <= \App\Facades\ObzoraConfig::get('nfsen_top_max'))
    ) {
        $topN = $vars['topN'];
    }
    ...
    $command = \App\Facades\ObzoraConfig::get('nfdump') . ' -M ' . nfsen_live_dir($device['hostname']) . ' -T -R ' .
