...
foreach (dbFetchRows("SELECT * FROM `bgpPeers` WHERE `device_id` = ? $extra_sql ORDER BY `bgpPeerRemoteAs`, `bgpPeerIdentifier`", [$device['device_id']]) as $peer) {
    ...
    $peeraddresslink = '<span class=list-large>' . \ObzoraNMS\Util\Url::overlibLink($link, $peerIdentifierIp?->compressed(), \ObzoraNMS\Util\Url::graphTag($graph_array_zoom)) . '</span>';
    ...
    echo '
        ...
        <td>' . $last_error . '</td>
        <td>' . \ObzoraNMS\Util\Time::formatInterval($peer['bgpPeerFsmEstablishedTime']) . "<br />
