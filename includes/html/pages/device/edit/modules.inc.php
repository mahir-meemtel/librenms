...
use ObzoraNMS\Util\Module;

$language = \config('app.locale');
$settings = (include ObzoraConfig::get('install_dir') . '/lang/' . $language . '/settings.php')['settings'];
...
$poller_modules = ObzoraConfig::get('poller_modules');
...
foreach ($poller_modules as $module => $module_status) {
    ...
    if (ObzoraConfig::has("os.{$device['os']}.poller_modules.$module")) {
        if (ObzoraConfig::get("os.{$device['os']}.poller_modules.$module")) {
            echo '<span class="text-success">Enabled</span>';
            $module_status = 1;
        } else {
            echo '<span class="text-danger">Disabled</span>';
...
$discovery_modules = ObzoraConfig::get('discovery_modules');
...
foreach ($discovery_modules as $module => $module_status) {
    ...
    if (ObzoraConfig::has("os.{$device['os']}.discovery_modules.$module")) {
        if (ObzoraConfig::get("os.{$device['os']}.discovery_modules.$module")) {
            echo '<span class="text-success">Enabled</span>';
            $module_status = 1;
        } else {
            echo '<span class="text-danger">Disabled</span>';
