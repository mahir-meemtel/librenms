...
if (isset($_POST['editing'])) {
    if (Auth::user()->hasGlobalAdmin()) {
        ...
        if ($snmp_enabled) {
            ...
            $device->port = $_POST['port'] ?: ObzoraConfig::get('snmp.port');
        ...
        if ($force_save !== true && $snmp_enabled) {
            $helper = new \ObzoraNMS\Polling\ConnectivityHelper($device);
...
echo "
    ...
    <div class='col-sm-4'>
    <input id='os' class='form-control' name='os' value='" . htmlspecialchars(ObzoraConfig::get("os.{$device->os}.text")) . "'/>
    ...
    <div class='col-sm-2'>
    <input type='number' name='port' placeholder='port' class='form-control input-sm' value='" . htmlspecialchars($device->port == ObzoraConfig::get('snmp.port') ? '' : $device->port) . "'>
    </div>
    <div class='col-sm-1'>
    <select name='transport' id='transport' class='form-control input-sm'>";
foreach (ObzoraConfig::get('snmp.transports') as $transport) {
...
if (! \ObzoraNMS\SNMPCapabilities::supportsSHA2()) {
    echo '<label class="text-left"><small>Some options are disabled. <a href="https://docs.obzora.meemtel.com/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>';
}
echo "
    </div>
    </div>
...
echo '</select>
    ';
if (! \ObzoraNMS\SNMPCapabilities::supportsAES256()) {
    echo '<label class="text-left"><small>Some options are disabled. <a href="https://docs.obzora.meemtel.com/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>';
}
    echo '
    </div>
