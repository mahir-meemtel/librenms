...
<div class="row">
    ...
    <div class="col-md-2 text-center">
        <?php
        if (\App\Facades\ObzoraConfig::get('enable_clear_discovery') == 1 && ! $device['snmp_disable']) {
            ?>
...
<input type=hidden name="editing" value="yes">
    ...
    <div class="form-group" data-toggle="tooltip" data-container="body" data-placement="bottom" title="Use this IP instead of resolved one for polling" >
     ...
     <div class="form-group">
        ...
        <div class="col-sm-6">
            <textarea id="descr" name="descr" class="form-control"><?php echo \ObzoraNMS\Util\Clean::html($device_model->purpose, []); ?></textarea>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <select id="type" name="type" class="form-control">
                ...
                foreach (\App\Facades\ObzoraConfig::get('device_types') as $type) {
...
<?php
if (\App\Facades\ObzoraConfig::get('distributed_poller') === true) {
                    ?>
   <div class="form-group">
       <label for="poller_group" class="col-sm-2 control-label">Poller Group</label>
       <div class="col-sm-6">
           <select name="poller_group" id="poller_group" class="form-control input-sm">
           <option value="0">General<?=\App\Facades\ObzoraConfig::get('distributed_poller_group') == 0 ? ' (default Poller)' : ''?></option>
    <?php
    foreach (dbFetchRows('SELECT `id`,`group_name` FROM `poller_groups` ORDER BY `group_name`') as $group) {
        ...
        echo \App\Facades\ObzoraConfig::get('distributed_poller_group') == $group['id'] ? ' (default Poller)' : '';
        echo '</option>';
    } ?>
           </select>
       </div>
...
echo 'Size on Disk: <b>' . \ObzoraNMS\Util\Number::formatBi($sizeondisk, 2, 0) . '</b> in <b>' . $numrrds . ' RRD files</b>.';
