...
$applications = [];
foreach (glob(ObzoraConfig::get('install_dir') . '/includes/polling/applications/*.inc.php') as $file) {
...
foreach ($applications as $app) {
    $modifiers = '';
    $app_text = \ObzoraNMS\Util\StringHelpers::niceCase($app);
    // check if the app exists in the enable apps array and check if it was automatically enabled
    if (isset($enabled_apps[$app])) {
        $modifiers = ' checked';
        if ($enabled_apps[$app]
            && (get_dev_attrib($device, 'poll_applications') || ObzoraConfig::getOsSetting($device['os'], 'poller_modules.applications'))
        ) {
