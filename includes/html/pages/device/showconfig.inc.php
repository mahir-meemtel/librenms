...
if (Auth::user()->hasGlobalAdmin()) {
    if (! empty($rancid_file)) {
        ...
        if (ObzoraConfig::get('rancid_repo_type') == 'svn') {
            $svn_binary = ObzoraConfig::locateBinary('svn');
            if (is_executable($svn_binary)) {
                ...
                foreach ($svnlogs as $svnlog) {
                    ...
                    $linktext = 'r' . $svnlog['rev'] . ' <small>' . date(ObzoraConfig::get('dateformat.byminute'), $svnlog['date']) . '</small>';
        ...
        if (ObzoraConfig::get('rancid_repo_type') == 'git') {
            $sep = ' | ';
            ...
            foreach ($gitlogs as $gitlog) {
                ...
                $linktext = 'r' . $gitlog['rev'] . ' <small>' . date(ObzoraConfig::get('dateformat.byminute'), $gitlog['date']) . '</small>';
        ...
        } elseif (ObzoraConfig::get('rancid_repo_type') == 'git') {
        ...
        if (ObzoraConfig::get('rancid_ignorecomments')) {
            $lines = explode("\n", $text);
    ...
    } elseif (ObzoraConfig::get('oxidized.enabled') === true && ObzoraConfig::has('oxidized.url')) {
        // Try with hostname as set in obzora first
        ...
        if (! empty($node_info['last']['start'])) {
            $node_info['last']['start'] = date(ObzoraConfig::get('dateformat.long'), strtotime($node_info['last']['start']));
        }
        if (! empty($node_info['last']['end'])) {
            $node_info['last']['end'] = date(ObzoraConfig::get('dateformat.long'), strtotime($node_info['last']['end']));
        }
        ...
        if (! $node_info) {
            ...
            } elseif (ObzoraConfig::get('mydomain')) {
                $oxidized_hostname = $device['hostname'] . '.' . ObzoraConfig::get('mydomain');
            }
        ...
        if (ObzoraConfig::get('oxidized.features.versioning') === true) { // fetch a list of versions
    ...
    if (! empty($text)) {
        $language = isset($previous_config) ? 'diff' : ObzoraConfig::getOsSetting($device['os'], 'config_highlighting', 'ios');
