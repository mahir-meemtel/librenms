    ...
    foreach ($components as $id => $array) {
        ...
        if (($array['parent'] == $components[$parent]['sp-obj']) && ($array['sp-id'] == $components[$parent]['sp-id'])) {
            ...
            if ($array['qos-type'] == 1) {
                ...
                echo '<a href="' . \ObzoraNMS\Util\Url::generate($linkvars) . '">' . $array['label'] . '</a>';
            } elseif ($array['qos-type'] == 2) {
                // Its a class, we need to make it a link.
                echo '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['policy' => $parent, 'class' => $id]) . '">' . $array['label'] . '</a>';
            } else {
                // Unknown, no link
                echo $array['label'];
            }
...
foreach ($components as $id => $array) {
    if (($array['qos-type'] == 1) && ($array['ifindex'] == $port['ifIndex']) && ($array['direction'] == 1) && ($array['parent'] == 0)) {
        ...
        echo '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['policy' => $id]) . '">' . $array['label'] . '</a>';
