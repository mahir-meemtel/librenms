...
if (! Auth::user()->hasGlobalRead()) {
    ...
    if ($vars['view'] == 'basic' || $vars['view'] == 'graphs') {
        ...
        foreach (dbFetchRows('SELECT `vrf_name`, `mplsVpnVrfRouteDistinguisher`, `mplsVpnVrfDescription` FROM `vrfs` GROUP BY `mplsVpnVrfRouteDistinguisher`, `mplsVpnVrfDescription`,`vrf_name`') as $vrf) {
            echo '<tr><td></td>';
            echo '<td width=240>';
            echo '<a class=list-large href=' . \ObzoraNMS\Util\Url::generate($vars, ['view' => 'detail', 'vrf' => $vrf['vrf_name']]) . '>';
            ...
            foreach ($vrf_devices[$vrf['vrf_name']][$vrf['mplsVpnVrfRouteDistinguisher']] as $device) {
                if ($i % 2) {
                    if ($x % 2) {
                        $dev_colour = ObzoraConfig::get('list_colour.even_alt');
                    } else {
                        $dev_colour = ObzoraConfig::get('list_colour.even_alt2');
                    }
                } else {
                    if ($x % 2) {
                        $dev_colour = ObzoraConfig::get('list_colour.odd_alt2');
                    } else {
                        $dev_colour = ObzoraConfig::get('list_colour.odd_alt');
                    }
                }

                echo "<tr bgcolor='$dev_colour'><td width=200><a href='";
                echo \ObzoraNMS\Util\Url::generate(['page' => 'device'], ['device' => $device['device_id'], 'tab' => 'routing', 'view' => 'basic', 'proto' => 'vrf']);
                ...
                foreach ($ports[$device['vrf_id']][$device['device_id']] as $port) {
                    ...
                    switch ($vars['graph']) {
                        ...
                        case 'errors':
                            $port['width'] = '130';
                            $port['height'] = '30';
                            $port['from'] = ObzoraConfig::get('time.day');
                            $port['to'] = ObzoraConfig::get('time.now');
                            $port['bg'] = '#' . $bg;
                            ...
                            text-align: center; float: left; background-color: " . ObzoraConfig::get('list_colour.odd_alt2') . ";'>
