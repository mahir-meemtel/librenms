...
$ctypes = collect(\App\Facades\ObzoraConfig::get('custom_descr', []))->keyBy(function ($descr) {
    if (is_array($descr)) {
...
if ($if_list) {
    ...
    foreach ($ports as $port) {
        ...
        if (dbFetchCell('SELECT count(*) FROM mac_accounting WHERE port_id = ?', [$port['port_id']])) {
            echo "<span style='float: right;'><a href='" . \ObzoraNMS\Util\Url::generate(['page' => 'device', 'device' => $port['device_id'], 'tab' => 'port', 'port' => $port['port_id'], 'view' => 'macaccounting']) . "'><i class='fa fa-pie-chart fa-lg icon-theme' aria-hidden='true'></i> MAC Accounting</a></span>";
        }
