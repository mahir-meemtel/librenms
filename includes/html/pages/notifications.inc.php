...
if (Auth::user()->hasGlobalAdmin()) {
    echo '<button class="btn btn-success pull-right new-notif" data-toggle="tooltip" data-placement="bottom" title="Create new notification" style="margin-top:-10px;"><i class="fa fa-plus"></i></button>';
}

if ($total > 0 && ! isset($vars['archive'])) {
    echo '<button class="btn btn-success pull-right read-all-notif tw:mr-2" data-toggle="tooltip" data-placement="bottom" title="Mark all as Read" style="margin-top:-10px;"><i class="fa fa-eye"></i></button>';
}
?>
      </h4>
      <hr/>
    ...
    foreach (Auth::user()->getNotifications('sticky') as $notif) {
      $notif_found[] = $notif->notifications_id;
        ...
        if ($notif->user_id != Auth::id()) {
            ...
            echo '<button class="btn btn-primary unstick-notif" data-toggle="tooltip" data-placement="bottom" title="Remove Sticky" style="margin-top:-10px;"><i class="fa fa-bell-slash-o"></i></button>';
        }
    ...
    <div class="row">
      <div class="col-md-12">
        <blockquote<?php echo $notif->severity == 2 ? ' style="border-color: darkred;"' : '' ?>>
          <p><?php echo \ObzoraNMS\Util\Clean::html($notif->body, ['HTML.Allowed' => 'br']); ?></p>
...
<hr/>
    ...
    foreach (Auth::user()->getNotifications('unread') as $notif) {
        ...
        if (Auth::user()->hasGlobalAdmin()) {
            echo '<button class="btn btn-primary stick-notif" data-toggle="tooltip" data-placement="bottom" title="Mark as Sticky" style="margin-top:-10px;"><i class="fa fa-bell-o"></i></button>';
        } ?>

<button class="btn btn-primary read-notif" data-toggle="tooltip" data-placement="bottom" title="Mark as Read" style="margin-top:-10px;"><i class="fa fa-eye"></i></button>
</span>
</h4>
      </div>
    </div>
  ...
  <div class="row">
    ...
    foreach ($read as $notif) {
        ...
        if (Auth::user()->isAdmin()) {
            echo '<span class="pull-right"><button class="btn btn-primary stick-notif" data-toggle="tooltip" data-placement="bottom" title="Mark as Sticky" style="margin-top:-10px;"><i class="fa fa-bell-o"></i></button></span>';
        } ?>
        </h4>
      </div>
    </div>
