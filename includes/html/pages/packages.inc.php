...
foreach (dbFetchRows("SELECT * FROM `packages` WHERE 1 $where GROUP BY `name`", $param) as $entry) {
    echo '<tr class="list">';
    echo '<td width=200><a href="' . \ObzoraNMS\Util\Url::generate($vars, ['name' => $entry['name']]) . '">' . $entry['name'] . '</a></td>';
    ...
    foreach ($entry['blah'] as $version => $bleu) {
        ...
        $content .= '</div>';
        if (empty($vars['name'])) {
            echo "<span style='margin:5px;'>" . \ObzoraNMS\Util\Url::overlibLink('', $version, $content) . '</span>';
        } else {
