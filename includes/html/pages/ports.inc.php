...
foreach ($menu_options as $option => $text) {
    ...
    $displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['format' => 'list_' . $option]) . '">' . $text . '</a>';
    if ($vars['format'] == 'list_' . $option) {
        $displayLists .= '</span>';
    }
    $sep = ' | ';
...
foreach ($menu_options as $option => $text) {
    ...
    $displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['format' => 'graph_' . $option]) . '">' . $text . '</a>';
    if ($vars['format'] == 'graph_' . $option) {
        $displayLists .= '</span>';
    }
    $sep = ' | ';
...
$displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['format' => '', 'page' => 'csv.php', 'report' => 'ports']) . '" title="Export as CSV" target="_blank" rel="noopener">Export CSV</a> | <a href="' . \ObzoraNMS\Util\Url::generate($vars) . '" title="Update the browser URL to reflect the search criteria.">Update URL</a> | ';

if (isset($vars['searchbar']) && $vars['searchbar'] == 'hide') {
    $displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['searchbar' => '']) . '">Search</a>';
} else {
    $displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['searchbar' => 'hide']) . '">Search</a>';
}

$displayLists .= ' | ';

if (isset($vars['bare']) && $vars['bare'] == 'yes') {
    $displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['bare' => '']) . '">Header</a>';
} else {
    $displayLists .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['bare' => 'yes']) . '">Header</a>';
}
...
if ((isset($vars['searchbar']) && $vars['searchbar'] != 'hide') || ! isset($vars['searchbar'])) {
    ...
    foreach ($ifSpeed as $data) {
        if ($data['ifSpeed']) {
            ...
            $output .= "<option value='" . $data['ifSpeed'] . "'" . $speedselected . '>' . \ObzoraNMS\Util\Number::formatSi($data['ifSpeed'], 2, 0, 'bps') . '</option>';
        }
    }
    ...
    $output .= "<a class='btn btn-default btn-sm' href='" . \ObzoraNMS\Util\Url::generate(['page' => 'ports', 'section' => $vars['section'] ?? '', 'bare' => $vars['bare'] ?? '']) . "' title='Reset critera to default.'>Reset</a>";
