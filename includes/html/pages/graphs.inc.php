...
$vars['from'] = Time::parseAt($vars['from'] ?? '') ?: ObzoraConfig::get('time.day');
$vars['to'] = Time::parseAt($vars['to'] ?? '') ?: ObzoraConfig::get('time.now');
...
if (! $auth) {
    ...
    if (ObzoraConfig::has("graph_types.$type.$subtype.descr")) {
        $title .= ' :: ' . ObzoraConfig::get("graph_types.$type.$subtype.descr");
    } elseif ($type == 'device' && $subtype == 'collectd') {
        $title .= ' :: ' . \ObzoraNMS\Util\StringHelpers::niceCase($subtype) . ' :: ' . $vars['c_plugin'];
        ...
        $title .= ' :: ' . \ObzoraNMS\Util\StringHelpers::niceCase($subtype);
    }
    ...
    if ($type != 'sensor') {
        ...
        foreach (get_graph_subtypes($type, $device) as $avail_type) {
            echo "<option value='" . \ObzoraNMS\Util\Url::generate($vars, ['type' => $type . '_' . $avail_type, 'page' => 'graphs']) . "'";
    ...
    if (! $show_command) {
        $thumb_array = ObzoraConfig::get('graphs.row.normal');
        ...
        foreach ($thumb_array as $period => $text) {
            ...
            echo '<b>' . $text . '</b>';
            echo '<a href="' . $link . '">';
            echo \ObzoraNMS\Util\Url::lazyGraphTag($graph_array);
            echo '</a>';
            echo '</td>';
        }
    ...
    $graph_array = $vars;
    $graph_array['height'] = ObzoraConfig::get('webui.min_graph_height');
    ...
    if ($vars['type'] == 'port_bits') {
        echo ' | ';
        if ($vars['port_speed_zoom'] ?? ObzoraConfig::get('graphs.port_speed_zoom')) {
    ...
    if (ObzoraConfig::get('webui.dynamic_graphs', false) === true) {
    ...
    if (ObzoraConfig::has('graph_descr.' . $vars['type'])) {
        print_optionbar_start();
        ...
        echo ObzoraConfig::get('graph_descr.' . $vars['type']);
        print_optionbar_end();
    }
