...
$fromtext = dbFetchCell("SELECT DATE_FORMAT($datefrom, '" . \App\Facades\ObzoraConfig::get('dateformat.mysql.date') . "')");
$totext = dbFetchCell("SELECT DATE_FORMAT($dateto, '" . \App\Facades\ObzoraConfig::get('dateformat.mysql.date') . "')");
...
$total['bg'] = \ObzoraNMS\Util\Color::percentage($total['per'], null);
...
$in['bg'] = \ObzoraNMS\Util\Color::percentage($in['per'], null);
...
$out['bg'] = \ObzoraNMS\Util\Color::percentage($out['per'], null);

$ousage = [];
$ousage['over'] = ($bill_data['total_data'] - ($bill_data['bill_quota']));
$ousage['over'] = (($ousage['over'] < 0) ? '0' : $ousage['over']);
$ousage['data'] = \ObzoraNMS\Util\Number::formatBase($ousage['over'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');
...
$ousage['bg'] = \ObzoraNMS\Util\Color::percentage($ousage['per'], null);

function showPercent($per)
{
    $background = \ObzoraNMS\Util\Color::percentage($per, null);
...
$di .= '&amp;from=' . \App\Facades\ObzoraConfig::get('time.day') . '&amp;to=' . \App\Facades\ObzoraConfig::get('time.now');
$di .= '&amp;imgtype=hour';
