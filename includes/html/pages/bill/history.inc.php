...
echo '<table class="table table-striped">
    ...
    <tr style="font-weight: bold; ">
        ...
        <th colspan="2" style="text-align: right;"><a href="' . \ObzoraNMS\Util\Url::generate($vars, ['detail' => 'all']) . '">
...
foreach (dbFetchRows('SELECT * FROM `bill_history` WHERE `bill_id` = ? ORDER BY `bill_datefrom` DESC LIMIT 24', [$bill_id]) as $history) {
    if (bill_permitted($history['bill_id'])) {
        ...
        $total_data = Number::formatBase($history['traf_total'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');

        $background = \ObzoraNMS\Util\Color::percentage($percent, null);
        ...
        if ($type == 'CDR') {
            ...
            $allowed = Number::formatBase($history['bill_allowed'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');
            $used = Number::formatBase($history['total_data'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');
            $in = Number::formatBase($history['traf_in'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');
            $out = Number::formatBase($history['traf_out'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');
            $overuse = (($history['bill_overuse'] <= 0) ? '-' : '<span style="color: #' . $background['left'] . '; font-weight: bold;">' . Number::formatBase($history['bill_overuse'], \App\Facades\ObzoraConfig::get('billing.base')) . '</span>');
        }
        $peakOut = Number::formatBase($history['bill_peak_out'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');
        $peakIn = Number::formatBase($history['bill_peak_in'], \App\Facades\ObzoraConfig::get('billing.base'), 2, 0, '');

        $total_data = (($type == 'Quota') ? '<b>' . $total_data . '</b>' : $total_data);
        $rate_95th = (($type == 'CDR') ? '<b>' . $rate_95th . '</b>' : $rate_95th);

        $url = \ObzoraNMS\Util\Url::generate($vars, ['detail' => $history['bill_hist_id']]);
