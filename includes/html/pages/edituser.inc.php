...
if (! Auth::user()->hasGlobalAdmin()) {
    ...
    if ($vars['user_id'] && empty($vars['edit'])) {
        ...
        $allow_dynamic = \App\Facades\ObzoraConfig::get('permission.device_group.allow_dynamic');
        if (! $allow_dynamic) {
        ...
        foreach ($interface_perms as $interface_perm) {
            echo '<tr>
              <td>
                <strong>' . $interface_perm['hostname'] . ' - ' . $interface_perm['ifDescr'] . '</strong>' . '' . \ObzoraNMS\Util\Clean::html($interface_perm['ifAlias'], []) . "
              </td>
              <td>
