...
foreach (Application::query()->hasAccess(Auth::user())->with('device')->get()->sortBy('show_name', SORT_NATURAL | SORT_FLAG_CASE)->groupBy('app_type') as $type => $groupedApps) {
    ...
    foreach ($groupedApps as $app) {
        ...
        $app_state = \ObzoraNMS\Util\Html::appStateIcon($app->app_state);
        ...
        $overlib_link .= '<br/>';
        $overlib_link .= \ObzoraNMS\Util\Url::graphTag($graph_array);
        ...
        echo \ObzoraNMS\Util\Url::overlibLink($overlib_url, $overlib_link, $overlib_content);
        echo '</div>';
    } //end foreach
    echo '</div>';
}//end foreach
