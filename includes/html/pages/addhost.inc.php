...
use ObzoraNMS\Exceptions\HostUnreachableException;
...
if (! empty($_POST['hostname'])) {
    ...
    if (! \ObzoraNMS\Util\Validate::hostname($hostname) && ! IP::isValid($hostname)) {
        ...
        if (Auth::user()->hasGlobalRead()) {
            ...
            if (! $snmp_enabled) {
                ...
                $communities = ObzoraConfig::get('snmp.community');
                if ($_POST['community']) {
            ...
            try {
                ...
                if ($result) {
                    $link = \ObzoraNMS\Util\Url::deviceUrl($new_device->device_id);
        ...
        <div class="form-group">
          ...
          <div class="col-sm-3">
            <select name="snmpver" id="snmpver" class="form-control input-sm" onChange="changeForm();">
                <?php
                $snmpver_pref = ObzoraConfig::get('snmp.version.0', 'v2c');
...
<?php
var_dump(ObzoraConfig::get('snmp.transports', ['udp']));
foreach (ObzoraConfig::get('snmp.transports', 'udp') as $transport) {
    echo '<option value="' . $transport . '"'. (ObzoraConfig::get('snmp.transports.0') == $transport ? ' selected' : '') . '>' . $transport . '</option>';
}
?>
            </select>
          </div>
...
foreach (PortAssociationMode::getModes() as $mode) {
    $selected = $mode == ObzoraConfig::get('default_port_association_mode') ? ' selected' : '';
          ...
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <select name="authlevel" id="authlevel" class="form-control input-sm">
                  ...
                  $authlevel_pref = ObzoraConfig::get('snmp.v3.0.authlevel', 'noAuthNoPriv');
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <select name="authalgo" id="authalgo" class="form-control input-sm">
                  <?php
                  $algo_pref = ObzoraConfig::get('snmp.v3.0.authalgo');
                  foreach (\ObzoraNMS\SNMPCapabilities::authAlgorithms() as $algo => $enabled) {
                      echo "<option value=\"$algo\"" . ($enabled ? '' : ' disabled') . ($algo == $algo_pref ? ' selected' : '') . ">$algo</option>";
                  }
                  ?>
              </select>
              <?php if (! \ObzoraNMS\SNMPCapabilities::supportsSHA2()) {?>
              <label class="text-left"><small>Some options are disabled. <a href="https://docs.obzora.meemtel.com/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>
              <?php } ?>
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <select name="cryptoalgo" id="cryptoalgo" class="form-control input-sm">
                  <?php
                  $algo_pref = ObzoraConfig::get('snmp.v3.0.cryptoalgo');
                  foreach (\ObzoraNMS\SNMPCapabilities::cryptoAlgoritms() as $algo => $enabled) {
                      echo "<option value=\"$algo\"" . ($enabled ? '' : ' disabled') . ($algo == $algo_pref ? ' selected' : '') . ">$algo</option>";
                  }
                  ?>
              </select>
              <?php if (! \ObzoraNMS\SNMPCapabilities::supportsAES256()) {?>
              <label class="text-left"><small>Some options are disabled. <a href="https://docs.obzora.meemtel.com/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>
              <?php } ?>
            </div>
          </div>
...
<?php
if (ObzoraConfig::get('distributed_poller') === true) {
                      echo '
          <div class="form-group">
