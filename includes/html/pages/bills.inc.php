...
if (isset($_POST['addbill']) && $_POST['addbill'] == 'yes') {
        ...
        if ($_POST['bill_type'] == 'quota') {
            if (isset($_POST['bill_quota_type'])) {
                if ($_POST['bill_quota_type'] == 'MB') {
                    $multiplier = (1 * \App\Facades\ObzoraConfig::get('billing.base'));
                }

                if ($_POST['bill_quota_type'] == 'GB') {
                    $multiplier = (1 * \App\Facades\ObzoraConfig::get('billing.base') * \App\Facades\ObzoraConfig::get('billing.base'));
                }

                if ($_POST['bill_quota_type'] == 'TB') {
                    $multiplier = (1 * \App\Facades\ObzoraConfig::get('billing.base') * \App\Facades\ObzoraConfig::get('billing.base') * \App\Facades\ObzoraConfig::get('billing.base'));
                }

                $bill_quota = (is_numeric($_POST['bill_quota']) ? $_POST['bill_quota'] * \App\Facades\ObzoraConfig::get('billing.base') * $multiplier : 0);
                $bill_cdr = 0;
            }
        }
    ...
    header('Location: ' . \ObzoraNMS\Util\Url::generate(['page' => 'bill', 'bill_id' => $bill_id, 'view' => 'edit']));
    exit();
}
