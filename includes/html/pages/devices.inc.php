...
foreach ($menu_options as $option => $text) {
    ...
    $listoptions .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['format' => 'list_' . $option]) . '">' . $text . '</a>';
    if ($vars['format'] == 'list_' . $option) {
        $listoptions .= '</span>';
    }
    $sep = ' | ';
...
foreach ($menu_options as $option => $text) {
    ...
    $listoptions .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['format' => 'graph_' . $option, 'from' => '-24hour', 'to' => 'now']) . '">' . $text . '</a>';
    if ($vars['format'] == 'graph_' . $option) {
        $listoptions .= '</span>';
    }
    $sep = ' | ';
...
foreach (get_graph_subtypes($type) as $avail_type) {
    $display_type = \ObzoraNMS\Util\StringHelpers::niceCase($avail_type);
    ...
    $headeroptions .= '<option value="' .
        \ObzoraNMS\Util\Url::generate($vars, [
            'format' => 'graph_' . $avail_type,
            'from' => $vars['from'] ?? \App\Facades\ObzoraConfig::get('time.day'),
            'to' => $vars['to'] ?? \App\Facades\ObzoraConfig::get('time.now'),
        ]) . '" ' . $is_selected . '>' . $display_type . '</option>';
}
$headeroptions .= '</select>';

if (isset($vars['searchbar']) && $vars['searchbar'] == 'hide') {
    $headeroptions .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['searchbar' => '']) . '">Restore Search</a>';
} else {
    $headeroptions .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['searchbar' => 'hide']) . '">Remove Search</a>';
}

$headeroptions .= ' | ';

if (isset($vars['bare']) && $vars['bare'] == 'yes') {
    $headeroptions .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['bare' => '']) . '">Restore Header</a>';
} else {
    $headeroptions .= '<a href="' . \ObzoraNMS\Util\Url::generate($vars, ['bare' => 'yes']) . '">Remove Header</a>';
}
...
if ($format == 'graph') {
    ...
    foreach (dbFetchRows($query, $sql_param) as $device) {
        if (is_integer($row / 2)) {
            $row_colour = \App\Facades\ObzoraConfig::get('list_colour.even');
        } else {
            $row_colour = \App\Facades\ObzoraConfig::get('list_colour.odd');
        }
        ...
        if (device_permitted($device['device_id'])) {
            ...
            $overlib_link = \ObzoraNMS\Util\Url::generate($link_array);
            echo '<div class="devices-overlib-box" style="min-width:' . ($width + 90) . '; max-width: ' . ($width + 90) . '">';
            echo '<div class="panel panel-default">';
            echo \ObzoraNMS\Util\Url::overlibLink($overlib_link, \ObzoraNMS\Util\Url::lazyGraphTag($graph_array_new), \ObzoraNMS\Util\Url::graphTag($graph_array_zoom));
            echo "</div></div>\n\n";
        }
    }
    echo '</div>';
    ...
    $os_selected = '""';
    if (isset($vars['os'])) {
        $os_selected = json_encode(['id' => $vars['os'], 'text' => \App\Facades\ObzoraConfig::getOsSetting($vars['os'], 'text', $vars['os'])]);
    }
    ...
    <script>
        ...
        $(".devices-headers-table-menu").append(
            ...
            "<a href='<?php echo \ObzoraNMS\Util\Url::generate(array_diff_key($vars, ['_token' => 1])) ?>' title='Update the browser URL to reflect the search criteria.' class='btn btn-default'>Update URL</a>" +
            "<a href='<?php echo \ObzoraNMS\Util\Url::generate(['page' => 'devices', 'section' => $vars['section'] ?? '', 'bare' => $vars['bare'] ?? '']) ?>' title='Reset criteria to default.' class='btn btn-default'>Reset</a>" +
            "</form>" +
            "</div>"
        );
            <?php
