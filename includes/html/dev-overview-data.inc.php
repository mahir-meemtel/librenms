...
if (ObzoraConfig::get('overview_show_sysDescr')) {
    echo '<i class="fa fa-id-card fa-lg icon-theme" aria-hidden="true"></i> <strong>';
    echo ObzoraConfig::get('overview_show_sysDescr', true) ? Clean::html($device['sysDescr'], []) : 'System';
    echo '</strong>';
}
...
if ($device['os'] == 'ios' || $device['os'] == 'iosxe') {
    \ObzoraNMS\Util\Rewrite::ciscoHardware($device, false);
}
...
$device['os_text'] = ObzoraConfig::getOsSetting($device['os'], 'text');
...
if ($device['location_id'] && $location = Location::find($device['location_id'])) {
    $maps_api = ObzoraConfig::get('geoloc.api_key');
    $maps_engine = $maps_api ? ObzoraConfig::get('geoloc.engine') : '';
    ...
    if (ObzoraConfig::get('device_location_map_show_devices')) {
        // Get a list of devices we have access to and add them to the map
        echo'
                device_marker_cluster = L.markerClusterGroup({
                    maxClusterRadius: ' . ObzoraConfig::get('leaflet.group_radius', 80) . ',
        ...
        if (ObzoraConfig::get('device_location_map_show_device_dependencies')) {
            echo'
    ...
    if (ObzoraConfig::get('device_location_map_open')) {
