            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" role="form" id="transports" class="form-horizontal transports-form">
                        ...
                        <input type="hidden" name="type" id="alert-transport-type" value="alert-transports">
...
// Create list of transport
    ...
    foreach ($transports_list as $transport => $name) {
        ...
        foreach ($tmp['config'] as $item) {
            if ($item['type'] !== 'hidden') {
                // Create unique ID by prefixing with transport type
                $unique_id = htmlentities($transport) . '-form-' . htmlentities($item['name']);
                $field_name = htmlentities($item['name']);
                
                echo '<div class="form-group" title="' . htmlentities($item['descr'] ?? '') . '">';
                echo '<label for="' . $unique_id . '" class="col-sm-3 col-md-2 control-label">' . htmlentities($item['title']) . ': </label>';
                if ($item['type'] == 'text' || $item['type'] == 'password') {
                    echo '<div class="col-sm-9 col-md-10">';
                    echo '<input type="' . htmlentities($item['type']) . '" id="' . $unique_id . '" name="' . $field_name . '" class="form-control" ';
                    ...
                    echo '<input type="checkbox" name="' . $field_name . '" id="' . $unique_id . '">';
                    echo '</div>';
                    $switches[$item['name']] = htmlentities($item['default']);
                } elseif ($item['type'] == 'select') {
                    echo '<div class="col-sm-3">';
                    echo '<select name="' . $field_name . '" id="' . $unique_id . '" class="form-control">';
                    ...
                    echo '<div class="col-sm-9 col-md-10">';
                    echo '<textarea name="' . $field_name . '" id="' . $unique_id . '" class="form-control" placeholder="' . htmlentities($item['descr']) . '">';
                    echo '</textarea>';
                    echo '</div>';
    ...
    <script>
        ...
        function loadTransport(transport) {
            ...
            transport.details.forEach(function(config) {
                // Try to find field by unique ID first (transport-form-fieldname), then fallback to name attribute
                var $field = transport_form.find("#" + form_id + "-" + config.name);
                if ($field.length === 0) {
                    // Fallback to finding by name attribute
                    $field = transport_form.find("[name='" + config.name + "']");
                }
                if ($field.length > 0) {
