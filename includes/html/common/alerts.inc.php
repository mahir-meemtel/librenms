...
';
    ...
    // Fix column alignment between header and body
    function syncColumnWidths() {
        var $table = alerts_grid;
        var $thead = $table.find(\'thead\');
        var $tbody = $table.find(\'tbody\');
        
        if ($thead.length && $tbody.length) {
            // First, ensure table has proper width
            var tableWidth = $table.width();
            if (tableWidth > 0) {
                $table.css(\'width\', tableWidth + \'px\');
            }
            
            // Get header column widths and sync to body
            $thead.find(\'th\').each(function(index) {
                var $th = $(this);
                var colIndex = index + 1;
                var thWidth = $th.outerWidth();
                var thMinWidth = $th.css(\'min-width\');
                var thMaxWidth = $th.css(\'max-width\');
                var thPadding = parseInt($th.css(\'padding-left\')) + parseInt($th.css(\'padding-right\'));
                var thBorder = parseInt($th.css(\'border-left-width\')) + parseInt($th.css(\'border-right-width\'));
                
                // Calculate content width
                var contentWidth = thWidth - thPadding - thBorder;
                
                // Sync to all tbody cells in this column
                $tbody.find(\'tr td:nth-child(\' + colIndex + \')\').each(function() {
                    var $td = $(this);
                    var tdPadding = parseInt($td.css(\'padding-left\')) + parseInt($td.css(\'padding-right\'));
                    var tdBorder = parseInt($td.css(\'border-left-width\')) + parseInt($td.css(\'border-right-width\'));
                    
                    // Ensure padding and border match
                    if (tdPadding !== thPadding) {
                        $td.css({
                    ...
                    $td.css({
                        \'width\': thWidth + \'px\',
                        \'min-width\': thMinWidth,
                        \'max-width\': thMaxWidth !== \'none\' ? thMaxWidth : \'auto\'
                    });
                });
            });
    ...
    // Sync widths immediately and after delays to handle bootgrid rendering
    syncColumnWidths();
    setTimeout(syncColumnWidths, 50);
    setTimeout(syncColumnWidths, 200);
    setTimeout(syncColumnWidths, 500);
    
    // Re-sync on window resize and table reload
    var resizeTimer;
    var uniqueId = \'' . $unique_id . '\';
    $(window).on(\'resize.alerts-table-\' + uniqueId, function() {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(syncColumnWidths, 100);
    });
    
    // Also sync on any table updates
    alerts_grid.on(\'reload.rs.jquery.bootgrid\', function() {
