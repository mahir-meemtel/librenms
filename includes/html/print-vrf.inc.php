...
foreach (dbFetchRows('SELECT * FROM ports WHERE `device_id` = ? AND `ifVrf` = ?', [$device['device_id'], $vrf['vrf_id']]) as $port) {
    ...
    ' . $port['ifAlias'] . " \
    <img src=\'graph.php?type=$graph_type&amp;id=" . $port['port_id'] . '&amp;from=' . \App\Facades\ObzoraConfig::get('time.twoday') . '&amp;to=' . \App\Facades\ObzoraConfig::get('time.now') . "&amp;width=450&amp;height=150\'>\
    ', CENTER, LEFT, FGCOLOR, '#e5e5e5', BGCOLOR, '#e5e5e5', WIDTH, 400, HEIGHT, 150);\" onmouseout=\"return nd();\"  >" . "<img src='graph.php?type=$graph_type&amp;id=" . $port['port_id'] . '&amp;from=' . \App\Facades\ObzoraConfig::get('time.twoday') . '&amp;to=' . \App\Facades\ObzoraConfig::get('time.now') . "&amp;width=132&amp;height=40&amp;legend=no'>
    </a>
