...
$query = "SELECT DATE_FORMAT(time_logged, '" . \App\Facades\ObzoraConfig::get('alert_graph_date_format') . "') Date, COUNT(alert_log.rule_id) totalCount, alert_rules.severity Severity FROM alert_log,alert_rules WHERE alert_log.rule_id=alert_rules.id AND `alert_log`.`state` != 0 $sql GROUP BY DATE_FORMAT(time_logged, '" . \App\Facades\ObzoraConfig::get('alert_graph_date_format') . "'),alert_rules.severity";
