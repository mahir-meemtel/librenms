...
//
if (file_exists(ObzoraConfig::get('install_dir') . "/includes/discovery/bgp-peers/{$device['os']}.inc.php")) {
    include ObzoraConfig::get('install_dir') . "/includes/discovery/bgp-peers/{$device['os']}.inc.php";
}
...
foreach (DeviceCache::getPrimary()->getVrfContexts() as $context_name) {
    ...
    if (! empty($peerlist)) {
        ...
        foreach ($peerlist as $peer) {
            $peer['astext'] = \ObzoraNMS\Util\AutonomousSystem::get($peer['as'])->name();
