    ...
    foreach ($fdp_array as $key => $fdp_if_array) {
        ...
        foreach ($fdp_if_array as $entry_key => $fdp) {
            ...
            if (! $remote_device_id &&
                ...
                if (ObzoraConfig::get('autodiscovery.xdp') === true) {
    ...
    foreach ($cdp_array as $key => $cdp_if_array) {
        ...
        foreach ($cdp_if_array as $entry_key => $cdp) {
            ...
            if (
                ...
                ObzoraConfig::get('autodiscovery.xdp') === true
            ) {
                $remote_device_id = discover_new_device($cdp['cdpCacheDeviceId'], $device, 'CDP', $interface);

                if ($cdp_ip && ! $remote_device_id && ObzoraConfig::get('discovery_by_ip', false)) {
...
if (($device['os'] == 'routeros') && version_compare($device['version'], '7.7', '<')) {
    ...
    if (! empty($lldp_array)) {
        ...
        foreach ($lldp_array as $key => $lldp) {
            ...
            if (! $remote_device_id &&
                    \ObzoraNMS\Util\Validate::hostname($lldp['lldpRemSysName']) &&
    ...
    foreach ($lldp_array as $key => $lldp_array_inner) {
        foreach ($lldp_array_inner as $ifIndex => $lldp) {
            ...
            if (ObzoraConfig::get('autodiscovery.xdp') && isset($lldp['lldpRemSysName']) && ! $remote_device_id &&
                    \ObzoraNMS\Util\Validate::hostname($lldp['lldpRemSysName']) &&
                    ! can_skip_discovery($lldp['lldpRemSysName'], $lldp['lldpRemSysDesc'])
            ) {
                $remote_device_id = discover_new_device($lldp['lldpRemSysName'], $device, 'LLDP', $interface);
        ...
        foreach ($sub_lldp_1 as $ifIndex => $sub_lldp_2) {
            foreach ($sub_lldp_2 as $macIndex => $sub_lldp_3) {
                foreach ($sub_lldp_3 as $remIndex => $lldp) {
                    ...
                    if (! $remote_device_id &&
                            \ObzoraNMS\Util\Validate::hostname($lldp['tmnxLldpRemSysName']) &&
    ...
    if (isset($lldp['lldpNeighborPortIndexId']) && is_array($lldp['lldpNeighborPortIndexId'])) {
        foreach ($lldp['lldpNeighborPortIndexId'] as $IndexId => $lldp_data) {
            ...
            if (! $remote_device_id &&
                    \ObzoraNMS\Util\Validate::hostname($remote_device_name) &&
    ...
    foreach ($lldp_array as $key => $lldp_if_array) {
        foreach ($lldp_if_array as $entry_key => $lldp_instance) {
            ...
            foreach ($lldp_instance as $entry_instance => $lldp) {
                ...
                if (! $remote_device_id && ObzoraConfig::get('autodiscovery.xdp') && ! can_skip_discovery($lldp['lldpRemSysName'] ?? null, $lldp['lldpRemSysDesc'] ?? null)) {
                    if (isset($lldp['lldpRemSysName'])) {
                        $remote_device_id = discover_new_device($lldp['lldpRemSysName'], $device, 'LLDP', $interface);
                    }
                    if (! $remote_device_id && ObzoraConfig::get('discovery_by_ip', false)) {
...
if (ObzoraConfig::get('autodiscovery.ospf') === true) {
    echo ' OSPF Discovery: ';
    ...
    foreach (dbFetchRows($sql, [$device['device_id']]) as $nbr) {
        ...
        } catch (\ObzoraNMS\Exceptions\InvalidIpException $e) {
            //
        }
    }
    echo PHP_EOL;
...
if (ObzoraConfig::get('autodiscovery.ospfv3') === true) {
