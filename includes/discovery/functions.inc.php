...
function discover_new_device($hostname, $device, $method, $interface = null)
{
    ...
    if (empty(ObzoraConfig::get('nets'))) {
    ...
    if (IP::isValid($hostname)) {
        $ip = $hostname;
        if (! ObzoraConfig::get('discovery_by_ip', false)) {
    ...
    } elseif (\ObzoraNMS\Util\Validate::hostname($hostname)) {
        if ($mydomain = ObzoraConfig::get('mydomain')) {
    ...
    $ip = IP::parse($ip, true);
    if ($ip->inNetworks(ObzoraConfig::get('autodiscovery.nets-exclude'))) {
        Log::debug("$ip in an excluded network - skipping\n");

        return false;
    }

    if (! $ip->inNetworks(ObzoraConfig::get('nets'))) {
...
function discover_device(&$device, $force_module = false)
{
    ...
    $helper = new \ObzoraNMS\Polling\ConnectivityHelper(DeviceCache::getPrimary());
    ...
    $discovery_modules = ['core' => true] + ObzoraConfig::get('discovery_modules', []);
    ...
    foreach ($discovery_modules as $module => $module_status) {
        $os_module_status = ObzoraConfig::getOsSetting($device['os'], "discovery_modules.$module");
...
{
    $fringe = array_merge(ObzoraConfig::get('bad_entity_sensor_regex', []), ObzoraConfig::getOsSetting($device['os'], 'bad_entity_sensor_regex', []));
...
function sensors($types, $os, $pre_cache = [])
{
    ...
    foreach ((array) $types as $sensor_class) {
        ...
        $dir = ObzoraConfig::get('install_dir') . '/includes/discovery/sensors/' . $sensor_class . '/';
        ...
        if (ObzoraConfig::getOsSetting($device['os'], 'rfc1628_compat', false)) {
...
function add_bgp_peer($device, $peer)
{
    if (dbFetchCell('SELECT COUNT(*) from `bgpPeers` WHERE device_id = ? AND bgpPeerIdentifier = ?', [$device['device_id'], $peer['ip']]) < '1') {
        ...
        if (ObzoraConfig::get('autodiscovery.bgp')) {
...
{
    if ($sysName) {
        foreach ((array) ObzoraConfig::get('autodiscovery.xdp_exclude.sysname_regexp') as $needle) {
    ...
    if ($sysDescr) {
        foreach ((array) ObzoraConfig::get('autodiscovery.xdp_exclude.sysdesc_regexp') as $needle) {
    ...
    if ($platform) {
        foreach ((array) ObzoraConfig::get('autodiscovery.cdp_exclude.platform_regexp') as $needle) {
...
function find_device_id($name = '', $ip = '', $mac_address = '')
{
    ...
    if ($name && \ObzoraNMS\Util\Validate::hostname($name)) {
        $where[] = '`hostname`=?';
        $params[] = $name;
