...
if ($ipmi['host'] = get_dev_attrib($device, 'ipmi_hostname')) {
    ...
    $cmd = [ObzoraConfig::get('ipmitool', 'ipmitool')];
    if (ObzoraConfig::get('own_hostname') != $device['hostname'] || $ipmi['host'] != 'localhost') {
    ...
    foreach (ObzoraConfig::get('ipmi.type', []) as $ipmi_type) {
    ...
    foreach ($results as $sensor) {
        ...
        $index++;
        if ($current != 'na' && ObzoraConfig::has("ipmi_unit.$unit")) {
            discover_sensor(
                null,
                ObzoraConfig::get("ipmi_unit.$unit"),
                $device,
                $desc,
                $index,
                'ipmi',
