...
$scheduler = ObzoraConfig::get('schedule_type.billing');
...
$query = \ObzoraNMS\DB\Eloquent::DB()->table('bills');
...
foreach ($query->get(['bill_id', 'bill_name']) as $bill) {
    ...
    if (ObzoraConfig::get('distributed_poller') && ObzoraConfig::get('distributed_billing')) {
        $port_list = dbFetchRows('SELECT * FROM `bill_ports` as P, `ports` as I, `devices` as D WHERE P.bill_id=? AND I.port_id = P.port_id AND I.ifOperStatus="up" AND D.device_id = I.device_id AND D.status=1 AND D.poller_group IN (' . ObzoraConfig::get('distributed_poller_group') . ')', [$bill_id]);
    } else {
    ...
    if (! empty($period) && $period < '0') {
        ...
        // NOTE: casting to string for mysqli bug (fixed by mysqlnd)
        if (ObzoraConfig::get('distributed_poller') && ObzoraConfig::get('distributed_billing')) {
            $port_count = dbFetchCell('SELECT COUNT(*) FROM `bill_ports` as P, `ports` as I, `devices` as D WHERE P.bill_id=? AND I.port_id = P.port_id AND D.device_id = I.device_id AND D.poller_group IN (' . ObzoraConfig::get('distributed_poller_group') . ')', [$bill_id]);
        } else {
