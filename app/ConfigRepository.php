...
class ConfigRepository
{
    ...
    public function __construct()
    {
        ...
        $cache_ttl = config('obzora.config_cache_ttl');
        $this->config = Cache::driver($cache_ttl == 0 ? 'null' : 'file')->remember('obzora-config', $cache_ttl, function () {
            $this->config = [];
    ...
    {
        Cache::driver('file')->forget('obzora-config');
    }
...
//        $this->setDefault('email_from', '"%s" <%s@' . php_uname('n') . '>', ['project_name', 'email_user']);  // FIXME email_from set because alerting config
    ...
    public function populateLegacyDbCredentials(): void
    {
        ...
        $this->set('db_name', config("database.connections.$db.database", 'obzora'));
        $this->set('db_user', config("database.connections.$db.username", 'obzora'));
