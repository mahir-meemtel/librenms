...
class Sensor
{
    ...
    {
        if (! empty($sensor->sensor_class) && (ObzoraConfig::getOsSetting($this->device->os, "disabled_sensors.$sensor->sensor_class") || ObzoraConfig::get("disabled_sensors.$sensor->sensor_class"))) {
            return true;
        }
        foreach (ObzoraConfig::getCombined($this->device->os, 'disabled_sensors_regex') as $skipRegex) {
            if (! empty($sensor->sensor_descr) && preg_match($skipRegex, $sensor->sensor_descr)) {
                return true;
            }
        }
        foreach (ObzoraConfig::getCombined($this->device->os, "disabled_sensors_regex.$sensor->sensor_class") as $skipRegex) {
            if (! empty($sensor->sensor_descr) && preg_match($skipRegex, $sensor->sensor_descr)) {
                return true;
            }
        }
