...
use ObzoraNMS\OS;
use ObzoraNMS\Polling\ConnectivityHelper;
...
class PollDevice implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($measurement->getDuration() > ObzoraConfig::get('rrd.step')) {
            Eventlog::log('Polling took longer than ' . round(ObzoraConfig::get('rrd.step') / 60, 2) .
        ...
        foreach ($this->getModules() as $module => $status) {
            ...
            try {
                ...
                if ($should_poll) {
                    ...
                    if (is_array($status)) {
                        ObzoraConfig::set('poller_submodules.' . $module, $status);
                    }
    ...
    private function initDevice(): void
    {
        ...
        if ($os_group = ObzoraConfig::get("os.{$this->device->os}.group")) {
    ...
    private function initRrdDirectory(): void
    {
        ...
        if (ObzoraConfig::get('rrd.enable', true) && ! is_dir($host_rrd)) {
            try {
                mkdir($host_rrd);
...
EOH, $this->device->hostname, $group ? "($group)" : '', $this->device->status ? '' : '%RDOWN%n', $this->device->device_id, $this->device->os, $this->device->ip), ['color' => true]);
    ...
    {
        $default_modules = ObzoraConfig::get('poller_modules', []);
