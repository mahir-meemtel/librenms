...
class Port extends DeviceRelatedModel
{
    ...
    public function getLabel()
    {
        ...
        if (\App\Facades\ObzoraConfig::getOsSetting($os, 'ifname')) {
            $label = $this->ifName;
        } elseif (\App\Facades\ObzoraConfig::getOsSetting($os, 'ifalias')) {
            $label = $this->ifAlias;
        }
        ...
        if (empty($label)) {
            ...
            if (\App\Facades\ObzoraConfig::getOsSetting($os, 'ifindex')) {
                $label .= " $this->ifIndex";
            }
        }

        foreach ((array) \App\Facades\ObzoraConfig::get('rewrite_if', []) as $src => $val) {
            if (Str::contains(strtolower($label), strtolower($src))) {
                $label = $val;
            }
        }

        foreach ((array) \App\Facades\ObzoraConfig::get('rewrite_if_regexp', []) as $reg => $val) {
