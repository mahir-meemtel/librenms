...
use ObzoraNMS\Exceptions\InsufficientDataException;
...
class Storage extends DeviceRelatedModel implements Keyable
{
    ...
    public function isValid(string $os): bool
    {
        ...
        foreach (\App\Facades\ObzoraConfig::getCombined($os, 'ignore_mount') as $im) {
        ...
        foreach (\App\Facades\ObzoraConfig::getCombined($os, 'ignore_mount_string') as $ims) {
        ...
        foreach (\App\Facades\ObzoraConfig::getCombined($os, 'ignore_mount_regexp') as $imr) {
        ...
        // filter by type
        foreach (\App\Facades\ObzoraConfig::getCombined($os, 'ignore_mount_type') as $imt) {
