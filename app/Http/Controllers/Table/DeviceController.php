...
class DeviceController extends TableController
{
    ...
    {
        $os_text = htmlspecialchars(ObzoraConfig::getOsSetting($device->os, 'text'));
    ...
    private function getActions(Device $device): array
    {
        ...
        if ($server = ObzoraConfig::get('gateone.server')) {
            $ssh_href = ObzoraConfig::get('gateone.use_obzora_user')
        ...
        foreach (array_values(Arr::wrap(ObzoraConfig::get('html.device.links'))) as $index => $custom) {
    ...
    protected function formatExportRow($device)
    {
        ...
        return [
            ...
            'os' => ObzoraConfig::getOsSetting($device->os, 'text', $device->os),
