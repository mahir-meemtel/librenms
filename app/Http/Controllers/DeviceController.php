...
class DeviceController extends Controller
{
    ...
    public function index(Request $request, $device, $current_tab = 'overview', $vars = '')
    {
        ...
        $primary_device_link_name = ObzoraConfig::get('html.device.primary_link', 'edit');
    ...
    private function deviceLinkMenu(Device $device, string $current_tab): array
    {
        ...
        foreach (array_values(Arr::wrap(ObzoraConfig::get('html.device.links'))) as $index => $link) {
        ...
        $ssh_url = ObzoraConfig::get('gateone.server')
            ? ObzoraConfig::get('gateone.server') . '?ssh=ssh://' . (ObzoraConfig::get('gateone.use_obzora_user') ? Auth::user()->username . '@' : '') . $device['hostname'] . '&location=' . $device['hostname']
