...
class OverviewController extends Controller
{
    ...
    {
        $view = ObzoraConfig::get('front_page');
    ...
    public function simple(Request $request)
    {
        ...
        $devices_down = Device::hasAccess(Auth::user())
            ->isDown()
            ->limit(ObzoraConfig::get('front_page_down_box_limit'))
            ->get();

        if (ObzoraConfig::get('warn.ifdown')) {
        ...
        if (filter_var(ObzoraConfig::get('uptime_warning'), FILTER_VALIDATE_FLOAT) !== false
            && ObzoraConfig::get('uptime_warning') > 0
        ) {
            $devices_uptime = Device::hasAccess(Auth::user())
                ->isUp()
                ->whereUptime(ObzoraConfig::get('uptime_warning'))
                ->limit(ObzoraConfig::get('front_page_down_box_limit'))
                ->get();

            $devices_uptime = $devices_uptime->reject(function ($device) {
                return ObzoraConfig::getOsSetting($device->os, 'bad_uptime') == true;
            });
        }

        if (ObzoraConfig::get('enable_syslog')) {
