...
class MapDataController extends Controller
{
    ...
    protected function linkUseColour(float $link_pct): string
    {
        ...
        return ObzoraConfig::get("network_map_legend.$link_pct", '#000000');
    }
    ...
    protected function nodeDisabledStyle(): array
    {
        return [
            'color' => [
                'highlight' => [
                    'background' => ObzoraConfig::get('network_map_legend.di.node'),
                ],
                'border' => ObzoraConfig::get('network_map_legend.di.border'),
                'background' => ObzoraConfig::get('network_map_legend.di.node'),
            ],
            'borderWidth' => null,
        ];
    ...
    protected function nodeDownStyle(): array
    {
        return [
            'color' => [
                'highlight' => [
                    'background' => ObzoraConfig::get('network_map_legend.dn.node'),
                    'border' => ObzoraConfig::get('network_map_legend.dn.border'),
                ],
                'border' => ObzoraConfig::get('network_map_legend.dn.border'),
                'background' => ObzoraConfig::get('network_map_legend.dn.node'),
            ],
    ...
    public function getDevices(Request $request): JsonResponse
    {
        ...
        foreach (self::deviceList($request) as $device) {
            if ($device->status) {
                $updowntime = \ObzoraNMS\Util\Time::formatInterval($device->uptime);
            } elseif ($device->last_polled) {
                $updowntime = \ObzoraNMS\Util\Time::formatInterval(time() - strtotime($device->last_polled));
            } else {
                $updowntime = '';
            }
            ...
            $device_list[$device->device_id] = [
                'id' => $device->device_id,
                'icon' => $device->icon ? asset($device->icon) : $device->logo(),
                ...
                foreach ($port->{$remote_port_attr} as $remote_port) {
                    ...
                    if ($port->device->status == 0 && $remote_port->device->status == 0) {
                        ...
                        $link_style = [
                            ...
                            'color' => [
                                'border' => ObzoraConfig::get('network_map_legend.dn.border'),
                                'highlight' => ObzoraConfig::get('network_map_legend.dn.edge'),
                                'color' => ObzoraConfig::get('network_map_legend.dn.edge'),
                            ],
                        ];
    ...
    public function getServices(Request $request): JsonResponse
    {
        ...
        foreach ($services->get() as $service) {
            if ($service->device->status) {
                $updowntime = \ObzoraNMS\Util\Time::formatInterval($service->device->uptime);
            } elseif ($service->device->last_polled) {
                $updowntime = \ObzoraNMS\Util\Time::formatInterval(time() - strtotime($service->device->last_polled));
            } else {
                $updowntime = '';
            }
            ...
            $service_list[] = [
                ...
                'icon' => $service->device->icon ? asset($service->device->icon) : $service->device->logo(),
