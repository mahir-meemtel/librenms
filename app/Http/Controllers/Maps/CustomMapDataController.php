...
class CustomMapDataController extends Controller
{
    public function get(Request $request, CustomMap $map): JsonResponse
    {
        ...
        foreach ($map->nodes as $node) {
            ...
            if ($node->device) {
                $warning_time = ObzoraConfig::get('uptime_warning', 86400);
    ...
    {
        $node_data_array['colour_bg_view'] = ObzoraConfig::get('network_map_legend.di.border');
        $node_data_array['colour_bdr_view'] = ObzoraConfig::get('network_map_legend.di.node');
    }

    protected function setNodeWarningStyle(array &$node_data_array, Request $request): void
    {
        $node_data_array['colour_bg_view'] = ObzoraConfig::get('network_map_legend.wn.node');
        $node_data_array['colour_bdr_view'] = ObzoraConfig::get('network_map_legend.wn.border');
    ...
    {
        $node_data_array['colour_bg_view'] = ObzoraConfig::get('network_map_legend.dn.node');
        $node_data_array['colour_bdr_view'] = ObzoraConfig::get('network_map_legend.dn.border');
        // Change the text colour as long as we have not been requested by the editor
        if ($request->headers->get('referer') && ! str_ends_with(parse_url($request->headers->get('referer'), PHP_URL_PATH), '/edit')) {
