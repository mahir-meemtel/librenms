...
use ObzoraNMS\Interfaces\Plugins\Hooks\MenuEntryHook;
use ObzoraNMS\Plugins;
...
class MenuComposer
{
    ...
    public function compose(View $view)
    {
        ...
        $vars['project_name'] = ObzoraConfig::get('project_name', 'ObzoraNMS');
        ...
        $vars['locations'] = (ObzoraConfig::get('show_locations') && ObzoraConfig::get('show_locations_dropdown')) ?
        ...
        // Service menu
        if (ObzoraConfig::get('show_services')) {
        ...
        $vars['port_counts']['pseudowire'] = ObzoraConfig::get('enable_pseudowires') ? ObjectCache::portCounts(['pseudowire'])['pseudowire'] : 0;

        $vars['port_counts']['alerted'] = 0; // not actually supported on old...

        $custom_descr = [];
        foreach ((array) ObzoraConfig::get('custom_descr', []) as $descr) {
        ...
        $vars['port_groups_exist'] = ObzoraConfig::get('int_customers') ||
            ObzoraConfig::get('int_transit') ||
            ObzoraConfig::get('int_peering') ||
            ObzoraConfig::get('int_core') ||
            ObzoraConfig::get('int_l2tp') ||
        ...
        // Wireless menu
        $wireless_menu_order = array_keys(\ObzoraNMS\Device\WirelessSensor::getTypes());
        ...
        if ($user->hasGlobalRead()) {
            ...
            if ($routing_count['bgp']) {
                $vars['show_peeringdb'] = ObzoraConfig::get('peeringdb.enabled', false);
        ...
        // Search bar
        $vars['typeahead_limit'] = ObzoraConfig::get('webui.global_search_result_limit');
