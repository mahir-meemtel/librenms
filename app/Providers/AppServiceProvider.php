...
use ObzoraNMS\Cache\PermissionsCache;
use ObzoraNMS\Util\IP;
use ObzoraNMS\Util\Validate;
...
class AppServiceProvider extends ServiceProvider
{
	...
	View::composer('*', function ($view) {
        	$systemStatus = DB::table('config')->where('config_name', 'system_status')->value('config_value') ?? '';
    ...
    private function bootCustomBladeDirectives(): void
    {
        Blade::if('config', function ($key, $value = true) {
            return ObzoraConfig::get($key) == $value;
        });
        Blade::if('notconfig', function ($key) {
            return ! ObzoraConfig::get($key);
        });
        ...
        Blade::directive('deviceUrl', function ($arguments) {
            return "<?php echo \ObzoraNMS\Util\Url::deviceUrl($arguments); ?>";
        });

        // Graphing
        Blade::directive('signedGraphUrl', function ($vars) {
            return "<?php echo \ObzoraNMS\Util\Url::forExternalGraph($vars); ?>";
        });

        Blade::directive('signedGraphTag', function ($vars) {
            return "<?php echo '<img class=\"obzora-graph\" src=\"' . \ObzoraNMS\Util\Url::forExternalGraph($vars) . '\" />'; ?>";
        });

        Blade::directive('graphImage', function ($vars, $flags = 0) {
            return "<?php echo \ObzoraNMS\Util\Graph::getImageData($vars, $flags); ?>";
        });
 ...
 ?>";
    ...
    {
        $sensor_types = [];
        foreach (\ObzoraNMS\Enum\Sensor::values() as $sensor_type) {
    ...
    {
        $this->app->alias(\ObzoraNMS\Interfaces\Geocoder::class, 'geocoder');
        $this->app->bind(\ObzoraNMS\Interfaces\Geocoder::class, function ($app) {
