...
class SmokepingGenerateCommand extends LnmsCommand
{
    ...
    {
        $probes = $this->assembleProbes(ObzoraConfig::get('smokeping.probes'));
    ...
    public function buildTargetsConfiguration($devices)
    {
        ...
        $targets = $this->buildTargets($smokelist, ObzoraConfig::get('smokeping.probes'), $this->option('single-process'));
        $header = $this->buildHeader($this->option('no-header'), $this->option('compat'));
    ...
    public function assembleProbes($probeCount)
    {
        ...
        return array_merge(
            $this->buildProbes('FPing', self::DEFAULTIP4PROBE, self::IP4PROBE, ObzoraConfig::get('fping'), $probeCount),
            $this->buildProbes('FPing6', self::DEFAULTIP6PROBE, self::IP6PROBE, ObzoraConfig::get('fping6'), $probeCount)
        );
    }
    ...
    {
        if (! ObzoraConfig::has('smokeping.probes') ||
            ! ObzoraConfig::has('fping') ||
            ! ObzoraConfig::has('fping6')
        ) {
        ...
        if (ObzoraConfig::get('smokeping.probes') < 1) {
     ...
     *
     * @param  string  $entry  The ObzoraNMS device hostname to rewrite
     * @return string
     */
