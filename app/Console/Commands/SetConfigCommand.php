...
class SetConfigCommand extends LnmsCommand
{
    ...
    public function handle(DynamicConfig $definition): int
    {
        ...
        if (Str::endsWith($setting, '.+')) {
            ...
            $sub_data = ObzoraConfig::get($setting, []);
        ...
        if ($parent && $parent !== $setting) {
            $parent_data = ObzoraConfig::get($parent);
        ...
        if (ObzoraConfig::persist($setting, $value)) {
            return 0;
        }
    ...
    private function erase($setting, $parent = null)
    {
        if ($parent) {
            ...
            return ObzoraConfig::persist($parent, $data);
        }

        return ObzoraConfig::erase($setting);
    }
    ...
    private function validateOsSetting(string $os, string $setting, $value)
    {
        ...
        $os_data = ObzoraConfig::get("os.$os");
        if ($os_data === null) {
