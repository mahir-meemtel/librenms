...
class KeyRotate extends LnmsCommand
{
    ...
    {
        if (! \App\Facades\ObzoraConfig::has($key)) {
            return true;
        }

        try {
            $data = $this->decrypt->decryptString(\App\Facades\ObzoraConfig::get($key));
            \App\Facades\ObzoraConfig::persist($key, $this->encrypt->encryptString($data));

            return true;
        } catch (DecryptException $e) {
            try {
                $this->encrypt->decryptString(\App\Facades\ObzoraConfig::get($key));
