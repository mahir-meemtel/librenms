...
use ObzoraNMS\Util\Notifications;
...
if ($options['f'] === 'update') {
    if (! ObzoraConfig::get('update')) {
        exit(0);
    }

    if (ObzoraConfig::get('update_channel') == 'master') {
        exit(1);
    } elseif (ObzoraConfig::get('update_channel') == 'release') {
        exit(3);
    }
    exit(0);
}
...
if ($options['f'] === 'rrd_purge') {
    ...
    if ($lock->get()) {
        $rrd_purge = ObzoraConfig::get('rrd_purge');
...
if ($options['f'] === 'syslog') {
    ...
    if ($lock->get()) {
        $syslog_purge = ObzoraConfig::get('syslog_purge');
...
if ($options['f'] === 'callback') {
    \ObzoraNMS\Util\Stats::submit();
}

if ($options['f'] === 'ports_purge') {
    if (ObzoraConfig::get('ports_purge')) {
...
if ($options['f'] === 'handle_notifiable') {
    if ($options['t'] === 'update') {
        ...
        $poller_name = ObzoraConfig::get('distributed_poller_name');
        ...
        if ($options['r']) {
            ...
            Notifications::create($title, "The daily update script (daily.sh) has failed on $poller_name."
                ...
                . 'visit the <a href="https://obzora.meemtel.com/#support" title="ObzoraNMS">ObzoraNMS Website</a> to find out how.',
                'daily.sh',
                2
            );
        }
        ...
        if (ObzoraConfig::get('update') && $options['r']) {
            if (preg_match('/^php\d{2}/', $options['r'])) {
                ...
                Notifications::create($error_title,
                    "PHP version $phpver is the minimum supported version as of $eol_date.  We recommend you update to PHP a supported version of PHP (" . Php::PHP_RECOMMENDED_VERSION . ' suggested) to continue to receive updates.  If you do not update PHP, ObzoraNMS will continue to function but stop receiving bug fixes and updates.',
                    'daily.sh',
                    2
                );
                exit(1);
        ...
        if (ObzoraConfig::get('update') && $options['r']) {
            if ($options['r'] === 'python3-missing') {
                Notifications::create($error_title,
                    'Python 3 is required to run ObzoraNMS as of May, 2020. You need to install Python 3 to continue to receive updates.  If you do not install Python 3 and required packages, ObzoraNMS will continue to function but stop receiving bug fixes and updates.',
                    'daily.sh',
                    2
                );
                exit(1);
                ...
                Notifications::create($error_title,
                    'Python 3 dependencies are missing. You need to install them via pip3 install -r requirements.txt or system packages to continue to receive updates.  If you do not install Python 3 and required packages, ObzoraNMS will continue to function but stop receiving bug fixes and updates.',
                    'daily.sh',
                    2
                );
                exit(1);
...
if ($options['f'] === 'purgeusers') {
    ...
    if ($lock->get()) {
        $purge = 0;
        if (is_numeric(\App\Facades\ObzoraConfig::get('radius.users_purge')) && ObzoraConfig::get('auth_mechanism') === 'radius') {
            $purge = \App\Facades\ObzoraConfig::get('radius.users_purge');
        }
        if (is_numeric(\App\Facades\ObzoraConfig::get('active_directory.users_purge')) && ObzoraConfig::get('auth_mechanism') === 'active_directory') {
            $purge = \App\Facades\ObzoraConfig::get('active_directory.users_purge');
        }
        if ($purge > 0) {
...
if ($options['f'] === 'notify') {
    if (\App\Facades\ObzoraConfig::has('alert.default_mail')) {
        try {
            \ObzoraNMS\Util\Mail::send(\App\Facades\ObzoraConfig::get('alert.default_mail'), '[ObzoraNMS] Auto update has failed for ' . ObzoraConfig::get('distributed_poller_name'), "We just attempted to update your install but failed. The information below should help you fix this.\r\n\r\n" . $options['o'], false);
        } catch (Exception $e) {
...
if ($options['f'] === 'refresh_os_cache') {
    ...
    if (is_file(ObzoraConfig::get('install_dir') . '/cache/os_defs.cache')) {
        unlink(ObzoraConfig::get('install_dir') . '/cache/os_defs.cache');
    }
}
