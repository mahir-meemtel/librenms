<td>
    ...
    @if($dslPort->port->ifInErrors_delta > 0 || $dslPort->port->ifOutErrors_delta > 0)
        <a href="{{ \ObzoraNMS\Util\Url::portUrl($dslPort->port, ['graph_type' => 'port_errors']) }}"><i class='fa fa-flag fa-lg tw:text-red-600'></i></a>
    @endif
...
<td>
    {{ \ObzoraNMS\Util\Number::formatSi($dslPort->port->ifInOctets_rate * 8, 2, 0, 'bps') }}
    <i class='fa fa-arrows-v fa-lg icon-theme' aria-hidden='true'></i>
    {{ \ObzoraNMS\Util\Number::formatSi($dslPort->port->ifOutOctets_rate * 8, 2, 0, 'bps') }}
    <br />
