<tr>
    <td>
        <div>
            ...
            @foreach($port->transceivers as $transceiver)
                ...
                @endphp
                <x-popup>
                    <a href="{{ \ObzoraNMS\Util\Url::generate(['page' => 'device', 'device' => $port->device_id, 'tab' => 'port','port' => $port->port_id], ['view' => 'transceiver']) }}" class="tw:text-current">
    ...
    <td class="tw:whitespace-nowrap">
        <div>
            ...
            <span style="color: {{ \ObzoraNMS\Util\Color::percent($port->in_rate, $port->ifSpeed) }}">{{ \ObzoraNMS\Util\Number::formatSi($port->ifInOctets_rate * 8, 2, 0, 'bps') }}</span>
        </div>
        <div>
            <i class="fa fa-long-arrow-right fa-lg" style="color:blue" aria-hidden="true"></i>
            <span style="color: {{ \ObzoraNMS\Util\Color::percent($port->out_rate, $port->ifSpeed) }}">{{ \ObzoraNMS\Util\Number::formatSi($port->ifOutOctets_rate * 8, 2, 0, 'bps') }}</span>
        </div>
        <div>
            <i class="fa fa-long-arrow-left fa-lg" style="color:purple" aria-hidden="true"></i>
            {{ \ObzoraNMS\Util\Number::formatBi($port->ifInUcastPkts_rate, 2, 0, 'pps') }}
        </div>
        <div>
            <i class="fa fa-long-arrow-right fa-lg" style="color:darkorange" aria-hidden="true"></i>
            {{ \ObzoraNMS\Util\Number::formatBi($port->ifOutUcastPkts_rate, 2, 0, 'pps') }}
        </div>
    </td>
    <td class="tw:whitespace-nowrap">
        @if($port->ifSpeed)
            <div>{{ \ObzoraNMS\Util\Number::formatSi($port->ifSpeed, 2, 0, 'bps') }}</div>
        @endif
        ...
        @if($port->vlans->isNotEmpty())
            <div class="tw:text-blue-800">
                <a href="{{ \ObzoraNMS\Util\Url::deviceUrl($port->device_id, ['tab' => 'vlans']) }}">
    ...
    <td @if($collapsing)class="tw:hidden tw:sm:table-cell"@endif>
        @if($port->adsl)
            <div>{{ $port->adsl->adslLineCoding }}/{{ \ObzoraNMS\Util\Rewrite::dslLineType($port->adsl->adslLineType) }}</div>
            <div>{{ __('port.xdsl.sync_stat', ['down' => \ObzoraNMS\Util\Number::formatSi($port->adsl->adslAtucChanCurrTxRate, 2, 0, 'bps'), 'up' => \ObzoraNMS\Util\Number::formatSi($port->adsl->adslAturChanCurrTxRate, 2, 0, 'bps')]) }}</div>
            <div>{{ __('port.xdsl.attainable_stat', ['down' => \ObzoraNMS\Util\Number::formatSi($port->adsl->adslAtucCurrAttainableRate, 2, 0, 'bps'), 'up' => \ObzoraNMS\Util\Number::formatSi($port->adsl->adslAturCurrAttainableRate, 2, 0, 'bps')]) }}</div>
        ...
        @elseif($port->vdsl)
            <div>{{ __('port.xdsl.sync_stat', ['down' => \ObzoraNMS\Util\Number::formatSi($port->vdsl->xdsl2ChStatusActDataRateXtuc, 2, 0, 'bps'), 'up' => \ObzoraNMS\Util\Number::formatSi($port->vdsl->xdsl2ChStatusActDataRateXtur, 2, 0, 'bps')]) }}</div>
            <div>{{ __('port.xdsl.attainable_stat', ['down' => \ObzoraNMS\Util\Number::formatSi($port->vdsl->xdsl2LineStatusAttainableRateDs, 2, 0, 'bps'), 'up' => \ObzoraNMS\Util\Number::formatSi($port->vdsl->xdsl2LineStatusAttainableRateUs, 2, 0, 'bps')]) }}</div>
        @else
            <div>{{ \ObzoraNMS\Util\Rewrite::normalizeIfType($port->ifType) }}</div>
        @endif
