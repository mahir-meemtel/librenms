    ...
    <table class="table table-condensed table-striped table-hover tw:mt-1 tw:mb-0!">
        ...
        @foreach($data['adsl'] as $dslPort)
            <tr>
                ...
                <td>
                    {{ \ObzoraNMS\Util\Number::formatSi($dslPort->adslAtucChanCurrTxRate, 2, 0, 'bps') }}/{{ \ObzoraNMS\Util\Number::formatSi($dslPort->adslAturChanCurrTxRate, 2, 0, 'bps') }}
                    <br />
                    <x-graph :port="$dslPort->port" type="port_adsl_speed" width="120" height="40" legend="no"></x-graph>
                </td>
                <td>
                    {{ \ObzoraNMS\Util\Number::formatSi($dslPort->adslAtucCurrAttainableRate, 2, 0, 'bps') }}/{{ \ObzoraNMS\Util\Number::formatSi($dslPort->adslAturCurrAttainableRate, 2, 0, 'bps') }}
                    <br />
        ...
        @foreach($data['vdsl'] as $dslPort)
            @include('device.tabs.ports.includes.xdsl_base_columns')
            <td>
                {{ \ObzoraNMS\Util\Number::formatSi($dslPort->xdsl2ChStatusActDataRateXtuc, 2, 0, 'bps') }}/{{ \ObzoraNMS\Util\Number::formatSi($dslPort->xdsl2ChStatusActDataRateXtur, 2, 0, 'bps') }}
                <br />
                <x-graph :port="$dslPort->port" type="port_vdsl_speed" width="120" height="40" legend="no"></x-graph>
            </td>
            <td>
                {{ \ObzoraNMS\Util\Number::formatSi($dslPort->xdsl2LineStatusAttainableRateDs, 2, 0, 'bps') }}/{{ \ObzoraNMS\Util\Number::formatSi($dslPort->xdsl2LineStatusAttainableRateUs, 2, 0, 'bps') }}
                <br />
