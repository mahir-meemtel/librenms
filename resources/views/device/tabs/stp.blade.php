        ...
        <x-panel class="stp-panel">
            ...
            <table class="table table-condensed table-striped table-hover">
                ...
                <tr>
                    ...
                    <td>
                        {{ \ObzoraNMS\Util\Mac::parse($instance['bridgeAddress'])->readable() }}
                        @if($url = \ObzoraNMS\Util\Url::deviceLink(\App\Facades\DeviceCache::get(\App\Models\Stp::where('bridgeAddress', $instance['bridgeAddress'])->value('device_id'))))
                            ({!! $url !!})
                        @elseif($brVendor = \ObzoraNMS\Util\Mac::parse($instance['bridgeAddress'])->vendor())
                            ({{ $brVendor }})
                        @endif
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>{{ \ObzoraNMS\Util\Time::formatInterval($instance['timeSinceTopologyChange']) }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        {{ \ObzoraNMS\Util\Mac::parse($instance['designatedRoot'])->readable() }}
                        @if($url = \ObzoraNMS\Util\Url::deviceLink(\App\Facades\DeviceCache::get(\App\Models\Stp::where('bridgeAddress', $instance['designatedRoot'])->whereNot('bridgeAddress', '')->value('device_id'))))
                            ({!! $url !!})
                        @elseif($drVendor = \ObzoraNMS\Util\Mac::parse($instance['designatedRoot'])->vendor())
                            ({{ $drVendor }})
                        @endif
                    </td>
                </tr>
