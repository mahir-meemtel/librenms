...
<div class="row">
    ...
    @foreach ($devices_down as $device)
        <div class="front-box device-down">
            {!! \ObzoraNMS\Util\Url::deviceLink($device, $device->shortDisplayName()) !!}
            <br />
    ...
    @foreach ($ports_down as $port)
        <div class="front-box alert alert-danger">
            {!! \ObzoraNMS\Util\Url::deviceLink($port->device, $port->device->shortDisplayName()) !!}
            <br />
            <span class="interface-updown">{{ __('Port Down') }}</span>
            <br />
            {!! \ObzoraNMS\Util\Url::PortLink($port) !!}
            @if($port->ifAlias)
                <br />
    ...
    @foreach ($services_down as $service)
        <div class="front-box service-down">
            {!! \ObzoraNMS\Util\Url::deviceLink($service->device, $service->device->shortDisplayName()) !!}
    ...
    @foreach ($bgp_down as $bgp)
        <div class="front-box bgp-down">
            {!! \ObzoraNMS\Util\Url::deviceLink($bgp->device, $bgp->device->shortDisplayName()) !!}
            ...
            <x-panel title="{{ __('Syslog entries') }}">
                <x-slot name="table">
                    ...
                    @foreach ($syslog as $entry)
                        <tr>
                            <td>{{ $entry->date }}</td>
                            <td><strong>{!! \ObzoraNMS\Util\Url::deviceLink($entry->device) !!}</strong></td>
