...
class SyslogTest extends TestCase
{
    ...
    public function testLinuxSyslog(): void
    {
        ...
        // ---- PAM ----
        $this->checkSyslog(
            '1.1.1.1||authpriv||info||info||56||2016-02-28 00:23:34||pam_unix(cron:session): session opened for user obzora by (uid=0)||CRON',
            ['device_id' => 1, 'program' => 'CRON', 'msg' => 'pam_unix(cron:session): session opened for user obzora by (uid=0)']
        );
        $this->checkSyslog(
            '1.1.1.1||authpriv||info||info||55||2016-02-28 00:23:34||pam_unix(sudo:session): session opened for user obzora by root (uid=0)||sudo',
            ['device_id' => 1, 'program' => 'SUDO', 'msg' => 'pam_unix(sudo:session): session opened for user obzora by root (uid=0)']
        );
        $this->checkSyslog(
        ...
        $this->checkSyslog(
            '1.1.1.1||mail||info||info||16||2016-02-28 00:23:34||D7256400EF: from=<obzora@obzora.example.com>, size=882, nrcpt=1 (queue active)||postfix/qmgr',
            ['device_id' => 1, 'program' => 'POSTFIX/QMGR', 'msg' => 'D7256400EF: from=<obzora@obzora.example.com>, size=882, nrcpt=1 (queue active)']
        );
        ...
        // ---- Other ----
        $this->checkSyslog(
            '1.1.1.1||cron||info||info||4e||2016-02-28 00:23:34||(obzora) CMD (   /opt/obzora/alerts.php >> /var/log/obzora_alert.log 2>&1)||CRON',
            ['device_id' => 1, 'program' => 'CRON', 'msg' => '(obzora) CMD (   /opt/obzora/alerts.php >> /var/log/obzora_alert.log 2>&1)']
        );
        $this->checkSyslog(
            '1.1.1.1||authpriv||notice||notice||55||2016-02-28 00:23:34||    root : TTY=pts/1 ; PWD=/opt/obzora ; USER=obzora ; COMMAND=/usr/bin/git status||sudo',
            ['device_id' => 1, 'program' => 'SUDO', 'msg' => 'root : TTY=pts/1 ; PWD=/opt/obzora ; USER=obzora ; COMMAND=/usr/bin/git status']
        );
    }
