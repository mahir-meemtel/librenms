...
class CommonFunctionsTest extends TestCase
{
    ...
    public function testRrdDescriptions(): void
    {
        ...
        $this->assertEquals('Toner, S/N CR_UM-16021314488.', \ObzoraNMS\Data\Store\Rrd::safeDescr($data));
    }
    ...
    {
        $this->assertSame('ObzoraNMS\OS\Os', StringHelpers::toClass('OS', 'ObzoraNMS\\OS\\'));
        ...
        $this->assertSame('ObzoraNMS\\AllOfThemName', StringHelpers::toClass('all OF-thEm_NaMe', 'ObzoraNMS\\'));
    }
...
//        $this->assertSame($expected, ResolveGlues(array('bill_data'), 'device_id'));
    ...
    {
        $device_dns = [
            'hostname' => 'test.obzora.meemtel.com',
            'sysName' => 'Testing DNS',
        ];
        $invalid_dns = [
            'hostname' => 'Not DNS',
        ...
        // default {{ $hostname }}
        ObzoraConfig::set('device_display_default', null);
        $this->assertEquals('test.obzora.meemtel.com', format_hostname($device_dns));
        ...
        $this->assertEquals('Custom Display (test.obzora.meemtel.com sysName)', format_hostname($custom_display));

        // ip to sysname
        ObzoraConfig::set('device_display_default', '{{ $sysName_fallback }}');
        $this->assertEquals('test.obzora.meemtel.com', format_hostname($device_dns));
        $this->assertEquals('Not DNS', format_hostname($invalid_dns));
        ...
        // sysname
        ObzoraConfig::set('device_display_default', '{{ $sysName }}');
