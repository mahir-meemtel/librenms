...
class TestLocks(unittest.TestCase):
    ...
    def test_redis_lock(self):
        if "redis" not in sys.modules:
            ...
            lm = ObzoraNMS.RedisLock(namespace="lock")
            thread = threading.Thread(
    ...
    def test_redis_queue(self):
        if "redis" not in sys.modules:
            ...
            qm = ObzoraNMS.RedisUniqueQueue("testing", namespace="queue")
...
class TestTimer(unittest.TestCase):
    ...
    def test_recurring_timer(self):
        ...
        timer = ObzoraNMS.RecurringTimer(0.5, self.count)
        timer.start()
        self.assertEqual(0, self.counter)
        sleep(0.5)
