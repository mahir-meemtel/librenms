...
function submitCustomRange(frmdata) {
    ...
    // Get dates from Flatpickr or fallback to bootstrap-datetimepicker
    var tsto, tsfrom;
    ...
    // Try to get Flatpickr instance (returns existing instance or undefined)
    var toPicker, fromPicker;
    try {
        // Check if element has Flatpickr instance attached
        if ($toInput[0]._flatpickr) {
            toPicker = $toInput[0]._flatpickr;
        } else if (typeof flatpickr !== 'undefined') {
            // Try to get instance using flatpickr function
            var toEl = document.getElementById("dtpickerto");
            if (toEl && toEl._flatpickr) {
                toPicker = toEl._flatpickr;
            }
        }
        
        if ($fromInput[0]._flatpickr) {
            fromPicker = $fromInput[0]._flatpickr;
        } else if (typeof flatpickr !== 'undefined') {
            var fromEl = document.getElementById("dtpickerfrom");
            if (fromEl && fromEl._flatpickr) {
                fromPicker = fromEl._flatpickr;
            }
        }
    } catch (e) {
        console.warn("Error accessing Flatpickr instances:", e);
    }
    
    // Get 'to' date
    if (toPicker && toPicker.selectedDates && toPicker.selectedDates.length > 0) {
        // Use Flatpickr date
        tsto = Math.floor(toPicker.selectedDates[0].getTime() / 1000);
    } else if ($toInput.data("DateTimePicker")) {
        // Fallback to bootstrap-datetimepicker
        try {
            tsto = $toInput.data("DateTimePicker").date().unix();
        } catch (e) {
            console.warn("Error getting DateTimePicker 'to' date:", e);
            tsto = null;
        }
    } else {
        // Fallback: parse input value with moment
        var toValue = $toInput.val();
        if (toValue) {
            var toMoment = moment(toValue, "YYYY-MM-DD HH:mm", true);
            if (toMoment.isValid()) {
                tsto = toMoment.unix();
            } else {
                // Try other formats
                ...
                if (toMoment.isValid()) {
                    ...
                    console.error("Invalid 'to' date value:", toValue);
                    return false;
                }
            }
        } else {
            console.error("No 'to' date selected");
            return false;
        }
    }
    
    // Get 'from' date
    if (fromPicker && fromPicker.selectedDates && fromPicker.selectedDates.length > 0) {
        // Use Flatpickr date
        tsfrom = Math.floor(fromPicker.selectedDates[0].getTime() / 1000);
    } else if ($fromInput.data("DateTimePicker")) {
        // Fallback to bootstrap-datetimepicker
        try {
            tsfrom = $fromInput.data("DateTimePicker").date().unix();
        } catch (e) {
            console.warn("Error getting DateTimePicker 'from' date:", e);
            tsfrom = null;
        }
    } else {
        // Fallback: parse input value with moment
        ...
        if (fromValue) {
            var fromMoment = moment(fromValue, "YYYY-MM-DD HH:mm", true);
            if (fromMoment.isValid()) {
                tsfrom = fromMoment.unix();
            } else {
                // Try other formats
                ...
                if (fromMoment.isValid()) {
                    ...
                    console.error("Invalid 'from' date value:", fromValue);
                    return false;
                }
            }
        } else {
            console.error("No 'from' date selected");
            return false;
        }
    }
    
    // Validate timestamps
    if (!tsto || !tsfrom || isNaN(tsto) || isNaN(tsfrom)) {
        console.error("Invalid timestamps - to:", tsto, "from:", tsfrom);
        return false;
    }
