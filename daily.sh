...
# CONSTANTS
#######################################
# define DAILY_SCRIPT as the full path to this script and OBZORA_DIR as the directory this script is in
DAILY_SCRIPT=$(readlink -f "$0")
OBZORA_DIR=$(dirname "$DAILY_SCRIPT")
COMPOSER="php ${OBZORA_DIR}/scripts/composer_wrapper.php --no-interaction"

# set log_file, using obzora 'log_dir' config setting, if set
# otherwise we default to <ObzoraNMS Install Directory>/logs
LOG_DIR=$(php -r "@include '${OBZORA_DIR}/config.php'; echo isset(\$config['log_dir']) ? \$config['log_dir'] : '${OBZORA_DIR}/logs';")

# get the obzora user
# shellcheck source=.env.example
source "${OBZORA_DIR}/.env"
OBZORA_USER="${OBZORA_USER:-obzora}"
OBZORA_USER_ID=$(id -u "$OBZORA_USER")
...
status_run() {
    ...
    # set log_file, using obzora $config['log_dir'], if set
    ...
    if [[ "${exit_code}" == "0" ]]; then
        ...
        if [[ "${arg_option}" == "update" ]]; then
            php "${OBZORA_DIR}/daily.php" -f notify -o "${tmp}"
        fi
        if [[ -n "${tmp}" ]]; then
...
call_daily_php() {
    ...
    for arg in "${args[@]}"; do
        php "${OBZORA_DIR}/daily.php" -f "${arg}"
    done
}
...
set_notifiable_result() {
    ...
    php "${OBZORA_DIR}/daily.php" -f handle_notifiable -t "${arg_type}" -r "${arg_result}"
}
...
check_dependencies() {
    ...
    python_deps=$("${OBZORA_DIR}/scripts/check_requirements.py" > /dev/null 2>&1; echo $?)
    phpver="master"
    pythonver="master"
...
main () {
    ...
    cd "${OBZORA_DIR}" || exit 1

    # if not running as $OBZORA_USER (unless $OBZORA_USER = root), relaunch
    if [[ "$OBZORA_USER" != "root" ]]; then
        # only try to su if we are root (or sudo)
        if [[ "$EUID" -eq 0 ]]; then
            echo "Re-running ${DAILY_SCRIPT} as ${OBZORA_USER} user"
            sudo -u "$OBZORA_USER" "$DAILY_SCRIPT" "$@"
            exit
        fi

        if [[ "$EUID" -ne "$OBZORA_USER_ID" ]]; then
    ...
    if [ ! -f "${OBZORA_DIR}/vendor/autoload.php" ]; then
